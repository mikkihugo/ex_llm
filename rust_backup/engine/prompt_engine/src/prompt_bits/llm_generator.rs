...
impl SPARCEngineLLMClient {
  ...
  async fn generate_internal(&self, trigger: &PromptBitTrigger, category: &PromptBitCategory, context: &GenerationContext) -> Result<String> {
    ...
    let generated = self.call_llm_engine(&prompt, trigger.clone(), category.clone()).await?;

    Ok(generated)
  }

  /// Call the real LLM engine (Claude/Gemini) via sparc-engine integration
  async fn call_llm_engine(&self, prompt: &str, trigger: PromptBitTrigger, category: PromptBitCategory) -> Result<String> {
    ...
    let generated_content = llm_response["choices"][0]["message"]["content"].as_str().ok_or_else(|| anyhow::anyhow!("Invalid LLM response format"))?;

    // Validate and clean the response
    let cleaned_response = self.validate_and_clean_response(generated_content, trigger, category)?;

    Ok(cleaned_response)
  }

  /// Validate and clean LLM response to ensure quality
  fn validate_and_clean_response(&self, response: &str, trigger: PromptBitTrigger, category: PromptBitCategory) -> Result<String> {
...
impl LLMClient for SPARCEngineLLMClient {
  async fn generate_prompt_bit(&self, trigger: &PromptBitTrigger, category: &PromptBitCategory, context: &GenerationContext) -> Result<String> {
    self.generate_internal(trigger, category, context).await
  }
}

/// Helper to build generation context
pub fn build_generation_context(task: &str, similar_bits: Vec<String>, repo_info: &str) -> GenerationContext {
  let existing_bits =
    if similar_bits.is_empty() { "No similar examples available.".to_string() } else { format!("Similar examples:\n{}", similar_bits.join("\n---\n")) };

  GenerationContext {
    task_description: task.to_string(),
    existing_bits: similar_bits,
    repo_context: format!("Repository context: {}\n\nSimilar bits: {}", repo_info, existing_bits),
  }
}
...
mod tests {
  ...
  async fn test_llm_generation() {
    ...
    let context = build_generation_context("Add Spring Boot service", vec![], "Monorepo with Maven build system");

    let result = client.generate_prompt_bit(&trigger, &category, &context).await;
    assert!(result.is_ok());
  }
}
