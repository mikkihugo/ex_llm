...
pub mod test;
pub use github_graphql_client::{
  GitHubGraphQLClient as InnerGitHubGraphQLClient, GitHubVersionAnalysis,
};
...
pub struct GitHubGraphQLClient {
  inner_client: InnerGitHubGraphQLClient,
}
...
impl GitHubGraphQLClient {
  pub fn new(token: Option<String>) -> Self {
    Self {
      inner_client: InnerGitHubGraphQLClient::new(token),
    }
  }

  /// Analyze version-specific repository using direct GraphQL queries.
  pub async fn analyze_version_specific(
    &self,
    analysis: &VersionSpecificAnalysis,
  ) -> Result<GitHubVersionAnalysis> {
    self.inner_client.analyze_version_specific(analysis).await
  }
  ...
  pub async fn discover_organization_repos(
    ...
    org_name: &str,
    limit: usize,
  ) -> Result<Vec<github_graphql_client::OrganizationRepo>> {
    self
      .inner_client
      .discover_organization_repos(org_name, limit)
      .await
  }
}
