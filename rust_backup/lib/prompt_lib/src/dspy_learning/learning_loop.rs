...
use crate::{
  ...
  prompt_tracking::{ABTestResultFact, EvolutionType, FactQuery, FactStorage, PromptEvolutionFact, PromptFactType, TestVariant},
};
...
impl ContinuousLearningLoop {
  ...
  pub async fn get_performance_report(&self, prompt_id: &str) -> Result<String> {
    ...
    if executions.len() > 1 {
      ...
      report.push_str(&format!("Improvement over time: {:.1}%\n", improvement * 100.0));
    }
  ...
  async fn run_learning_cycle(&mut self) -> Result<()> {
    ...
    let execution_summary = self.execution_tracker.get_performance_summary("test_prompt").await?;
    ...
    let confidence_adjustment = self.confidence_scorer.calculate_adjustment(&execution_result).await?;
    ...
    for prompt_id in optimization_candidates {
      ...
      let candidates = self.copro_optimizer.generate_candidate_variations(&prompt_id, 5).await?;
      ...
      if let Some(candidate) = best_candidate {
        tracing::info!("Selected candidate for {}: score={}, depth={}", prompt_id, candidate.score, candidate.depth);

        // Store the optimized prompt using available method
        self
          .fact_store
          .store(crate::prompt_tracking::PromptFactType::PromptExecution(crate::prompt_tracking::PromptExecutionFact {
    ...
    let coordination_result = self.sparc_coordinator.execute_request(sparc_request).await?;
  ...
  async fn identify_optimization_candidates(&self) -> Result<Vec<String>> {
    ...
    // Group by prompt ID
    let mut prompt_stats: std::collections::HashMap<String, PromptStats> = std::collections::HashMap::new();
  ...
  pub async fn optimize_prompt(&self, prompt_id: &str) -> Result<()> {
    ...
    let evolved_executions = self.fact_store.query(FactQuery::PromptExecutions(evolved_prompt_id.clone())).await.unwrap_or_default();
    ...
    metadata.insert("baseline_success".to_string(), format!("{:.2}", baseline_metrics.success_rate));
    metadata.insert("baseline_confidence".to_string(), format!("{:.2}", baseline_metrics.avg_confidence));
    ...
    self.fact_store.store(PromptFactType::PromptEvolution(evolution_fact)).await?;

    tracing::info!("Created evolved prompt: {} (improvement: {:.1}%)", evolved_prompt_id, performance_improvement * 100.0);
  ...
  async fn run_ab_tests(&self) -> Result<()> {
    ...
    for fact in evolutions {
      if let PromptFactType::PromptEvolution(evolution) = fact {
        ...
        let a_executions = self.fact_store.query(FactQuery::PromptExecutions(evolution.original_prompt_id.clone())).await?;

        let b_executions = self.fact_store.query(FactQuery::PromptExecutions(evolution.evolved_prompt_id.clone())).await?;

        if a_executions.len() >= self.config.ab_test_sample_size / 2 && b_executions.len() >= self.config.ab_test_sample_size / 2 {
          ...
          let test_duration = if let (Some(first_a), Some(first_b)) = (
            a_executions.first().and_then(|f| if let PromptFactType::PromptExecution(e) = f { Some(e.timestamp) } else { None }),
            b_executions.first().and_then(|f| if let PromptFactType::PromptExecution(e) = f { Some(e.timestamp) } else { None }),
          ) {
            (chrono::Utc::now() - first_a.min(first_b)).to_std().unwrap_or(Duration::from_secs(3600))
          } else {
          ...
          metrics.insert("improvement_pct".to_string(), (b_success - a_success) / a_success * 100.0);

          // Store A/B test result
          let ab_test = ABTestResultFact {
            test_id: format!("ab_test_{}_{}", evolution.original_prompt_id, chrono::Utc::now().timestamp()),
            variant_a_prompt_id: evolution.original_prompt_id.clone(),
            variant_b_prompt_id: evolution.evolved_prompt_id.clone(),
            winner,
            confidence_level: self.calculate_significance(a_success, b_success, a_executions.len(), b_executions.len()),
          ...
          self.fact_store.store(PromptFactType::ABTestResult(ab_test)).await?;
        }
      }
    }
  ...
  pub fn calculate_performance_metrics(&self, executions: &[PromptFactType]) -> PerformanceMetrics {
  ...
  pub fn calculate_improvement(&self, baseline: PerformanceMetrics, evolved: PerformanceMetrics) -> f64 {
    ...
    let success_improvement = if baseline.success_rate > 0.0 { (evolved.success_rate - baseline.success_rate) / baseline.success_rate } else { 0.0 };

    let confidence_improvement = if baseline.avg_confidence > 0.0 { (evolved.avg_confidence - baseline.avg_confidence) / baseline.avg_confidence } else { 0.0 };

    // Speed improvement is inverted (lower is better)
    let speed_improvement = if baseline.avg_execution_time_ms > 0 && evolved.avg_execution_time_ms > 0 {
      (baseline.avg_execution_time_ms as f64 - evolved.avg_execution_time_ms as f64) / baseline.avg_execution_time_ms as f64
    } else {
      0.0
    };

    // Weighted average improvement
    let total_improvement = success_improvement * SUCCESS_WEIGHT + confidence_improvement * CONFIDENCE_WEIGHT + speed_improvement * SPEED_WEIGHT;
  ...
  async fn cleanup_old_data(&self) -> Result<()> {
    ...
    if deleted_count > 0 {
      tracing::info!("Marked {} execution facts for cleanup (older than {} days)", deleted_count, RETENTION_DAYS);
    }
...
impl PerformanceMetrics {
  ...
  pub fn new(success_rate: f64, avg_confidence: f64, avg_execution_time_ms: u64, sample_count: usize) -> Self {
    Self { success_rate, avg_confidence, avg_execution_time_ms, sample_count }
  }
