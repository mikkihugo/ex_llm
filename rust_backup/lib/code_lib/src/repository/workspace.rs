...
impl WorkspaceDetector {
  ...
  pub fn detect_package_manager(&self) -> Result<PackageManager> {
    ...
    } else if self.root_path.join("Pipfile.lock").exists() || self.root_path.join("requirements.txt").exists() {
...
mod tests {
  ...
  fn test_detect_monorepo_moon() {
    ...
    assert_eq!(detector.detect_workspace_type().unwrap(), WorkspaceType::Monorepo);
  ...
  fn test_detect_single_repo() {
    ...
    let detector = WorkspaceDetector::new(temp.path().to_path_buf());
    assert_eq!(detector.detect_workspace_type().unwrap(), WorkspaceType::SingleRepo);
  }
