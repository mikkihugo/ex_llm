defmodule ExLLM.ModelDiscovery.OpenRouterPriceSyncer do
  @moduledoc """
  ...
  ```
  # Sync on demand
  ExLLM.ModelDiscovery.OpenRouterPriceSyncer.sync_prices()
  # => {:ok, 150} - synced 150 models

  # Check if cache is fresh
  ExLLM.ModelDiscovery.OpenRouterPriceSyncer.cache_fresh?()
  # => true

  # Merge with models.dev data
  {:ok, merged} = ExLLM.ModelDiscovery.OpenRouterPriceSyncer.merge_with_models_dev()
  ```
  """
  ...
  defp get_models_dev_data do
    ...
    case ExLLM.ModelDiscovery.ModelsDevSyncer.get_cached_models() do
      {:ok, data} -> {:ok, data}
  ...
  defp store_to_postgres(models) do
    ...
    try do
      # Check if ExLLM.Repo is available
      case Code.ensure_compiled(ExLLM.Repo) do
        ...
        {:error, _} ->
          Logger.debug("ExLLM.Repo not available, using file cache only")
  ...
  defp cache_path do
    cache_dir = Application.get_env(:ex_llm, :cache_dir, "/tmp/.ex_llm_cache")
