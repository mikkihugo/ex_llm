defmodule SingularityLLM.Infrastructure.StartupValidator do
  @moduledoc """
  Validates SingularityLLM configuration during application startup.

  This module performs comprehensive validation of the SingularityLLM configuration
  ...
  ## Examples
      ...
      case SingularityLLM.Infrastructure.StartupValidator.validate() do
        {:ok, results} ->
      ...
      {:ok, _} = SingularityLLM.Infrastructure.StartupValidator.validate_providers()
      {:ok, _} = SingularityLLM.Infrastructure.StartupValidator.validate_cache()
  """

  require Logger
  alias SingularityLLM.Infrastructure.{Config.ModelConfig, Logger}
  ...
  def validate do
    ...
    if config.enabled do
      Logger.info("Starting SingularityLLM configuration validation...")
      ...
      cond do
        ...
        Enum.empty?(all_issues) ->
          Logger.info("✓ SingularityLLM configuration validation passed")
          {:ok, results}

        true ->
          Logger.warning("⚠ SingularityLLM configuration validation completed with issues")
          {:ok, Map.put(results, :issues, all_issues)}
      end
    else
      Logger.debug("SingularityLLM startup validation disabled")
  ...
  def validate_application_config do
    ...
    # Validate log level
    issues =
      case Application.get_env(:singularity_llm, :log_level) do
    ...
    issues =
      case Application.get_env(:singularity_llm, :log_components) do
  ...
  def validate_cache do
    ...
    cache_enabled = Application.get_env(:singularity_llm, :cache_enabled, false)

    issues =
      if cache_enabled do
        # Validate cache strategy
        issues =
          case Application.get_env(:singularity_llm, :cache_strategy) do
            nil ->
              [
                %{
                  category: :cache,
        ...
        persist_disk = Application.get_env(:singularity_llm, :cache_persist_disk, false)

        if persist_disk do
          cache_path = Application.get_env(:singularity_llm, :cache_disk_path, "~/.cache/ex_llm_cache")
    ...
    case validate() do
      ...
      {:error, issues} ->
        ...
        if config.fail_on_error do
          Logger.error("SingularityLLM startup validation failed, halting application startup")
          {:error, issues}
        else
          Logger.warning("SingularityLLM startup validation found issues but continuing startup")
          :ok
        end
    end
  end
  ...
  defp get_validation_config do
    config = Application.get_env(:singularity_llm, :startup_validation, %{})
  ...
  defp log_success_message(log_level) do
    if log_level in [:debug, :info] do
      Logger.info("SingularityLLM configuration validation: ✓ All checks passed")
    end
  end

  defp format_validation_summary(error_count, warning_count, info_count) do
    "SingularityLLM validation: #{error_count} errors, #{warning_count} warnings, #{info_count} info"
  end
  ...
  defp get_provider_module(:openai), do: SingularityLLM.Providers.OpenAI
  defp get_provider_module(:anthropic), do: SingularityLLM.Providers.Anthropic
  defp get_provider_module(:gemini), do: SingularityLLM.Providers.Gemini
  defp get_provider_module(:groq), do: SingularityLLM.Providers.Groq
  defp get_provider_module(:mistral), do: SingularityLLM.Providers.Mistral
  defp get_provider_module(:openrouter), do: SingularityLLM.Providers.OpenRouter
  defp get_provider_module(:perplexity), do: SingularityLLM.Providers.Perplexity
  defp get_provider_module(:xai), do: SingularityLLM.Providers.XAI

  defp get_provider_module(provider),
    do: Module.concat(SingularityLLM.Providers, Macro.camelize(to_string(provider)))
end
