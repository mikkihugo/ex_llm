if Code.ensure_loaded?(OpenTelemetry) and Code.ensure_loaded?(OpenTelemetry.Tracer) do
  defmodule SingularityLLM.Infrastructure.Telemetry.OpenTelemetry do
    @moduledoc """
    OpenTelemetry integration for SingularityLLM.

    This module provides OpenTelemetry instrumentation for SingularityLLM operations,
    ...
    Replace direct SingularityLLM calls with the instrumented versions:

        # Instead of:
        SingularityLLM.chat(model: "gpt-4", messages: messages)
        
        # Use:
        SingularityLLM.Infrastructure.Telemetry.OpenTelemetry.chat(model: "gpt-4", messages: messages)
    ...
    @tracer :singularity_llm

    @doc """
    Instrumented version of SingularityLLM.chat/3.
    ...
    def chat(model_or_config, messages, opts \\ []) do
      ...
      Tracer.with_span @tracer, "singularity_llm.chat", %{attributes: attributes} do
        ...
        case SingularityLLM.chat(model_or_config, messages, opts_with_ctx) do
          {:ok, response} = result ->
    ...
    @doc """
    Instrumented version of SingularityLLM.stream_chat/3.
    ...
    def stream_chat(model_or_config, messages, opts \\ []) do
      ...
      Tracer.with_span @tracer, "singularity_llm.stream", %{attributes: attributes} do
        otel_ctx = OpenTelemetry.Ctx.get_current()
        opts_with_ctx = Keyword.put(opts, :otel_ctx, otel_ctx)

        case SingularityLLM.stream_chat(model_or_config, messages, opts_with_ctx) do
          {:ok, stream} ->
    ...
    @doc """
    Instrumented version of SingularityLLM.embed/3.
    """
    def embed(model_or_config, input, opts \\ []) do
      ...
      Tracer.with_span @tracer, "singularity_llm.embed", %{attributes: attributes} do
        otel_ctx = OpenTelemetry.Ctx.get_current()
        opts_with_ctx = Keyword.put(opts, :otel_ctx, otel_ctx)

        case SingularityLLM.embed(model_or_config, input, opts_with_ctx) do
          {:ok, response} = result ->
    ...
    @doc """
    Wrap any SingularityLLM operation with OpenTelemetry tracing.

    ## Example

        with_span "custom_operation", %{custom: "attribute"} do
          # Your SingularityLLM operations here
        end
    """
    ...
    ## Example
        ...
        Task.async(fn ->
          SingularityLLM.Infrastructure.Telemetry.OpenTelemetry.with_context(ctx, fn ->
    ...
    This creates OpenTelemetry spans from SingularityLLM telemetry events.
    ...
    def attach_telemetry_handlers do
      handlers = [
        {[:singularity_llm, :chat, :start], &handle_start/4},
        {[:singularity_llm, :chat, :stop], &handle_stop/4},
        {[:singularity_llm, :chat, :exception], &handle_exception/4},
        {[:singularity_llm, :provider, :request, :start], &handle_start/4},
        {[:singularity_llm, :provider, :request, :stop], &handle_stop/4},
        {[:singularity_llm, :provider, :request, :exception], &handle_exception/4}
      ]
    ...
    defp build_attributes(operation, model_or_config, opts) do
      ...
      case SingularityLLM.Utils.build_config(model_or_config, opts) do
        {:ok, config} ->
          Map.merge(base, %{
  ...
  defmodule SingularityLLM.Infrastructure.Telemetry.OpenTelemetry do
    @moduledoc """
    OpenTelemetry integration for SingularityLLM (Stub).
    ...
    def chat(model_or_config, messages, opts \\ []) do
      SingularityLLM.chat(model_or_config, messages, opts)
    end
    ...
    def stream_chat(model_or_config, messages, opts \\ []) do
      ...
      SingularityLLM.stream(model_or_config, messages, callback, opts_list)
    end

    def embed(model_or_config, input, opts \\ []) do
      SingularityLLM.embeddings(model_or_config, input, opts)
    end
