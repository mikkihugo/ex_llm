defmodule SingularityLLM.Infrastructure.Telemetry.Instrumentation do
  @moduledoc """
  Helper module for adding telemetry instrumentation to SingularityLLM operations.
  ...
  ### Basic Instrumentation
      ...
      defmodule MyModule do
        import SingularityLLM.Infrastructure.Telemetry.Instrumentation
        
        def my_function(args) do
          instrument [:singularity_llm, :my_module, :my_function], %{args: args} do
            # Your code here
            {:ok, result}
          end
        end
  ...
  defmacro instrument(event_prefix, metadata \\ %{}, do: block) do
    quote do
      SingularityLLM.Infrastructure.Telemetry.span(unquote(event_prefix), unquote(metadata), fn ->
        unquote(block)
      end)
    end
  end
  ...
  def instrument_cache_lookup(key, lookup_fn) do
    case lookup_fn.() do
      {:ok, value} ->
        :telemetry.execute(
          [:singularity_llm, :cache, :lookup, :hit],
          %{},
          %{key: key}
        )
      ...
      :error ->
        :telemetry.execute(
          [:singularity_llm, :cache, :lookup, :miss],
          %{},
          %{key: key}
        )
  ...
  def instrument_cache_store(key, value, ttl, store_fn) do
    case store_fn.() do
      :ok ->
        :telemetry.execute(
          [:singularity_llm, :cache, :store, :success],
  ...
  def instrument_session_add_message(session_id, message, tokens) do
    :telemetry.execute(
      [:singularity_llm, :session, :message, :added],
      %{tokens: tokens},
      %{
