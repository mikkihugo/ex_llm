defmodule SingularityLLM.Infrastructure.Logger do
  @moduledoc """
  Unified logging for SingularityLLM with automatic context and security features.

  This module provides a simple, unified logging interface that combines
  the simplicity of Elixir's Logger with SingularityLLM-specific features like
  ...
  Simple logging (like Elixir's Logger):
      ...
      SingularityLLM.Infrastructure.Logger.info("Model loaded successfully")
      SingularityLLM.Infrastructure.Logger.error("Failed to connect", error: reason)
      SingularityLLM.Infrastructure.Logger.debug("Processing chunk", size: byte_size(data))

  With provider context:

      SingularityLLM.Infrastructure.Logger.with_context(provider: :openai, operation: :chat) do
        SingularityLLM.Infrastructure.Logger.info("Starting chat request")
        # ... do work ...
        SingularityLLM.Infrastructure.Logger.info("Chat completed", tokens: 150)
      end

  Structured logging for specific events:

      SingularityLLM.Infrastructure.Logger.log_request(:openai, url, body, headers)
      SingularityLLM.Infrastructure.Logger.log_response(:openai, response, duration_ms)
      SingularityLLM.Infrastructure.Logger.log_retry(:anthropic, attempt, max_attempts, reason)
  ...
  ## Examples
      ...
      SingularityLLM.Infrastructure.Logger.with_context(provider: :openai, operation: :chat) do
        SingularityLLM.Infrastructure.Logger.info("Starting request")
  ...
  # Structured logging for specific SingularityLLM events
  ...
  defp should_emit_log?(level) do
    configured_level = Application.get_env(:singularity_llm, :log_level, :info)
    level_value(level) >= level_value(configured_level)
  end

  defp should_log?(component) do
    components = Application.get_env(:singularity_llm, :log_components, %{})
  ...
  defp should_redact?(type) do
    redaction = Application.get_env(:singularity_llm, :log_redaction, %{})
