defmodule SingularityLLM.Infrastructure.Retry do
  @moduledoc """
  Request retry logic with exponential backoff for SingularityLLM.
  ...
  ## Usage
      ...
      SingularityLLM.Infrastructure.Retry.with_retry fn ->
  ...
  # replaced by the full SingularityLLM.Infrastructure.CircuitBreaker module implementation
  ...
  This function integrates circuit breaker protection with SingularityLLM's existing
  retry logic, providing comprehensive fault tolerance.

  ## Options
    * `:circuit_breaker` - Circuit breaker options (see SingularityLLM.CircuitBreaker.call/3)
  ...
  def with_circuit_breaker_retry(fun, opts \\ []) when is_function(fun, 0) do
    ...
    SingularityLLM.Infrastructure.CircuitBreaker.call(
      circuit_name,
      fn ->
  ...
  def chat_with_circuit_breaker(provider, messages, opts \\ []) do
    with_provider_circuit_breaker(
      provider,
      fn ->
        SingularityLLM.chat(provider, messages, opts)
      end,
      opts
    )
  end
  ...
  def stream_with_circuit_breaker(provider, messages, opts \\ []) do
    ...
    with_provider_circuit_breaker(
      ...
      fn ->
        ...
        SingularityLLM.stream(provider, messages, callback, opts_list)
      end,
