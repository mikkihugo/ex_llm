defmodule SingularityLLM.Infrastructure.Cache do
  @moduledoc """
  Production caching implementation for SingularityLLM using ETS-based storage.

  This module provides the runtime caching infrastructure for LLM responses,
  operating as part of SingularityLLM's dual-cache architecture. It focuses exclusively
  on production performance optimization, while test-specific caching is handled
  separately through the strategy pattern.

  For a complete understanding of SingularityLLM's caching architecture, see the
  ...
  This module serves as the production cache implementation within SingularityLLM's
  ...
  ## Usage
      ...
      {:ok, response} = SingularityLLM.chat(:anthropic, messages, cache: false)
  ...
  - `[:singularity_llm, :cache, :put]` - Item stored with size in bytes
  - `[:singularity_llm, :cache, :evict]` - Item evicted from cache
  ...
  # alias SingularityLLM.Infrastructure.Cache.Storage

  @default_ttl :timer.minutes(15)
  @cleanup_interval :timer.minutes(5)
  @default_storage {SingularityLLM.Infrastructure.Cache.Storage.ETS, []}
  ...
  def get(key) do
    ...
    case result do
      {:ok, _} ->
        ...
        SingularityLLM.Infrastructure.Telemetry.emit_cache_hit(key)

      :miss ->
        Logger.log_cache_event(:miss, key)
        SingularityLLM.Infrastructure.Telemetry.emit_cache_miss(key)
    end
  ...
  def put(key, value, opts \\ []) do
    ...
    SingularityLLM.Infrastructure.Telemetry.emit_cache_put(key, size_bytes)
  ...
  def should_cache?(options) do
    cond do
      ...
      true -> Application.get_env(:singularity_llm, :cache_enabled, false)
    end
  end
  ...
  This is the main integration point for SingularityLLM modules. It uses a configured
  strategy to handle caching.
  """
  @spec with_cache(String.t(), keyword(), fun()) :: any()
  def with_cache(cache_key, opts, fun) do
    strategy = Application.get_env(:singularity_llm, :cache_strategy, SingularityLLM.Cache.Strategies.Production)
  ...
  def init(opts) do
    # Get configuration
    config = Application.get_env(:singularity_llm, :cache, [])

    # Get disk persistence config from environment or config
    persist_disk =
      System.get_env("EX_LLM_CACHE_PERSIST") == "true" or
        Application.get_env(:singularity_llm, :cache_persist_disk, false)

    disk_path =
      System.get_env("EX_LLM_CACHE_DIR") ||
        Application.get_env(:singularity_llm, :cache_disk_path, "/tmp/ex_llm_cache")
  ...
  def handle_cast({:put, key, value, opts}, state) do
    ...
    case state.storage_mod.put(key, value, expires_at, state.storage_state) do
      ...
      _ ->
        ...
        Logger.error("SingularityLLM.Cache: Failed to store key #{key}")
  ...
  def handle_info(:cleanup, state) do
    ...
    if state.storage_mod == SingularityLLM.Infrastructure.Cache.Storage.ETS do
      # Get storage info
      case state.storage_mod.info(state.storage_state) do
        {:ok, info, _} ->
          Logger.debug("SingularityLLM.Cache: Storage size: #{info.size} entries")
  ...
  defp persist_to_disk_async(cache_key, cached_response, opts, state) do
    ...
    Task.start(fn ->
      try do
        SingularityLLM.Testing.MockResponseRecorder.store_from_cache(
          cache_key,
          cached_response,
          provider,
          endpoint,
