defmodule SingularityLLM.Infrastructure.CircuitBreaker do
  @moduledoc """
  ...
  Called during SingularityLLM application startup.
  """
  def init do
    :ets.new(@table_name, [
      :named_table,
  ...
  ## Examples
      iex> SingularityLLM.CircuitBreaker.call("api_service", fn -> 
      ...>   HTTP.Core.get("/api/data")
      ...> end)
      {:ok, response}
      
      iex> SingularityLLM.CircuitBreaker.call("failing_service", fn -> 
      ...>   raise "Service down"
      ...> end)
      {:error, :circuit_open}
  """
  ...
  def apply_profile(circuit_name, profile_name) do
    SingularityLLM.Infrastructure.CircuitBreaker.ConfigManager.apply_profile(circuit_name, profile_name)
  end
  ...
  def update_circuit_config(circuit_name, config_changes) do
    SingularityLLM.Infrastructure.CircuitBreaker.ConfigManager.update_circuit(circuit_name, config_changes)
  end
  ...
  def get_circuit_config(circuit_name) do
    SingularityLLM.Infrastructure.CircuitBreaker.ConfigManager.get_config(circuit_name)
  end
  ...
  def rollback_config(circuit_name) do
    SingularityLLM.Infrastructure.CircuitBreaker.ConfigManager.rollback(circuit_name)
  end
  ...
  def call_with_bulkhead(circuit_name, fun, opts \\ []) when is_function(fun, 0) do
    SingularityLLM.Infrastructure.CircuitBreaker.Bulkhead.execute(circuit_name, fun, opts)
  end
  ...
  defp emit_telemetry(event, circuit_name, metadata) when is_map(metadata) do
    ...
    :telemetry.execute(
      [:singularity_llm, :circuit_breaker, event],
      measurements,
