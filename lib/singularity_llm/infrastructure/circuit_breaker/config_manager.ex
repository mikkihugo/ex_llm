defmodule SingularityLLM.Infrastructure.CircuitBreaker.ConfigManager do
  @moduledoc """
  ...
  ## Usage
      ...
      SingularityLLM.CircuitBreaker.ConfigManager.update_circuit("api_service", %{
        failure_threshold: 10,
        reset_timeout: 60_000
      })
      
      # Apply configuration profile
      SingularityLLM.CircuitBreaker.ConfigManager.apply_profile("api_service", :conservative)
      
      # Batch update multiple circuits
      SingularityLLM.CircuitBreaker.ConfigManager.batch_update(%{
        "service_1" => %{failure_threshold: 5},
        "service_2" => %{reset_timeout: 30_000}
      })
      
      # Rollback to previous configuration
      SingularityLLM.CircuitBreaker.ConfigManager.rollback("api_service")
  """
  ...
  defp apply_config_to_circuit(circuit_name, config) do
    ...
    SingularityLLM.Infrastructure.CircuitBreaker.call(circuit_name, fn -> :dummy end, cb_config)

    # Then update the configuration
    SingularityLLM.Infrastructure.CircuitBreaker.update_config(circuit_name, cb_config)
  end

  defp apply_bulkhead_config(circuit_name, config) do
    if Map.has_key?(config, :bulkhead) do
      SingularityLLM.Infrastructure.CircuitBreaker.Bulkhead.configure(circuit_name, config.bulkhead)
    else
      :ok
    end
  end
  ...
  defp emit_telemetry(event, circuit_name, metadata) do
    :telemetry.execute(
      [:singularity_llm, :circuit_breaker, :config_manager, event],
      %{count: 1},
