defmodule SingularityLLM.Tesla.MiddlewareFactory do
  @moduledoc """
  ...
  across different Tesla client builders in the SingularityLLM codebase.
  """
  ...
  ## Examples
      ...
      middleware = SingularityLLM.Tesla.MiddlewareFactory.build_middleware(:openai, config)

      # Streaming-specific middleware
      middleware = SingularityLLM.Tesla.MiddlewareFactory.build_middleware(:openai, config, is_streaming: true)

      # Custom middleware stack
      middleware = SingularityLLM.Tesla.MiddlewareFactory.build_middleware(:anthropic, config,
        include_retry: false,
  ...
  defp add_circuit_breaker(middleware, provider, config, opts) do
    ...
    if Keyword.get(opts, :include_circuit_breaker, true) && !is_streaming do
      ...
      [
        {SingularityLLM.Tesla.Middleware.CircuitBreaker,
  ...
  defp add_telemetry(middleware, provider, _config, opts) do
    if Keyword.get(opts, :include_telemetry, true) do
      [
        {SingularityLLM.Tesla.Middleware.Telemetry, metadata: %{provider: provider}}
        | middleware
      ]
    else
      middleware
