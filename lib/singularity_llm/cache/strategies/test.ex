defmodule SingularityLLM.Cache.Strategies.Test do
  @moduledoc """
  ...
  - `SingularityLLM.Testing.TestCacheDetector` - Detects test context
  - `SingularityLLM.Testing.TestResponseInterceptor` - HTTP-level caching
  - `SingularityLLM.Testing.LiveApiCacheStorage` - Disk persistence
  ...
  - `SingularityLLM.Cache.Strategies.Production` - The fallback strategy
  - `SingularityLLM.Testing.TestCacheDetector` - Test context detection
  - [Caching Architecture Guide](docs/caching_architecture.md)
  """

  @behaviour SingularityLLM.Cache.Strategy

  alias SingularityLLM.Cache.Strategies.Production
  ...
  defp test_caching_enabled?() do
    ...
    if Code.ensure_loaded?(SingularityLLM.Testing.TestCacheDetector) do
      SingularityLLM.Testing.TestCacheDetector.should_cache_responses?()
    else
      false
    end
  end
