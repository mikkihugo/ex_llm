defmodule SingularityLLM.Plug do
  ...
  An SingularityLLM plug is a module that implements two functions:
  ...
  Here's a simple plug that adds authentication headers:
      ...
      defmodule MyApp.AuthPlug do
        ...
        def call(request, opts) do
          ...
          request
          |> SingularityLLM.Pipeline.Request.assign(:auth_header, "Bearer \#{api_key}")
        end
  ...
  For convenience, you can `use SingularityLLM.Plug` which will:

    * Add the `@behaviour SingularityLLM.Plug` annotation
  ...
  A plug can halt the pipeline by calling `SingularityLLM.Pipeline.Request.halt/1`:

      def call(request, _opts) do
        if some_condition? do
          request
          |> SingularityLLM.Pipeline.Request.add_error(%{reason: :unauthorized})
          |> SingularityLLM.Pipeline.Request.halt()
        else
          request
        end
      end
  ...
  When you `use SingularityLLM.Plug`, the following happens:

    * The module is marked with `@behaviour SingularityLLM.Plug`
    * Default implementations of `init/1` and `call/2` are provided
    * The `SingularityLLM.Pipeline.Request` module is aliased for convenience
  ...
  ## Examples
      ...
      defmodule MyConfigurablePlug do
        use SingularityLLM.Plug, call: false
  ...
  ## Examples
      ...
      iex> SingularityLLM.Plug.run(request, {MyPlug, timeout: 5000})
      %Request{...}
      
      iex> SingularityLLM.Plug.run(request, MyPlug)
      %Request{...}
  """
  ...
  @doc """
  Checks if a module implements the SingularityLLM.Plug behaviour.

  ## Examples

      iex> SingularityLLM.Plug.plug?(MyApp.AuthPlug)
      true
      
      iex> SingularityLLM.Plug.plug?(String)
      false
  """
  ...
  def plug?(module) when is_atom(module) do
    ...
    SingularityLLM.Plug in behaviours
  end
end
