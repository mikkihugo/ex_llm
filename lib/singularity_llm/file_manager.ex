defmodule SingularityLLM.FileManager do
  @moduledoc """
  File management functionality for SingularityLLM.
  ...
  ## Examples
      ...
      {:ok, file} = SingularityLLM.FileManager.upload_file(:gemini, "/path/to/image.png", 
        display_name: "My Image")
      
      # List OpenAI files
      {:ok, files} = SingularityLLM.FileManager.list_files(:openai)
      
      # Get file metadata
      {:ok, file_info} = SingularityLLM.FileManager.get_file(:gemini, "files/abc-123")
      
      # Delete a file
      :ok = SingularityLLM.FileManager.delete_file(:gemini, "files/abc-123")
  """
  ...
  ## Examples
      ...
      {:ok, file} = SingularityLLM.FileManager.upload_file(:openai, "/path/to/training.jsonl",
        purpose: "fine-tune")
  ...
  ## Examples
      ...
      # List Gemini files
      {:ok, response} = SingularityLLM.FileManager.list_files(:gemini)
      Enum.each(response.files, fn file ->
        IO.puts("File: \#{file.display_name} (\#{file.name})")
      end)
      
      # List OpenAI fine-tuning files
      {:ok, response} = SingularityLLM.FileManager.list_files(:openai, purpose: "fine-tune")
  ...
  ## Examples
      ...
      {:ok, file} = SingularityLLM.FileManager.get_file(:gemini, "files/abc-123")
