defmodule SingularityLLM.Application do
  @moduledoc false

  use Application

  @impl true
  def start(_type, _args) do
    # Ensure telemetry is started first - check if already started
    case Application.ensure_all_started(:telemetry) do
      {:ok, _apps} ->
        :ok
      {:error, _} ->
        # Telemetry might not be available, continue anyway
        :ok
    end

    # Run startup configuration validation
    SingularityLLM.Infrastructure.StartupValidator.run_startup_validation()

    # Initialize circuit breaker ETS table
    SingularityLLM.Infrastructure.CircuitBreaker.init()

    # Delay metrics setup to avoid telemetry warnings
    if Mix.env() in [:dev, :test] do
      spawn(fn ->
        Process.sleep(100)
        SingularityLLM.Infrastructure.CircuitBreaker.Metrics.setup()
      end)
    else
      # In production, set up immediately
      SingularityLLM.Infrastructure.CircuitBreaker.Metrics.setup()
    end

    children =
      [
        # Start StreamRecovery for all adapters
        SingularityLLM.Core.Streaming.Recovery,
        # Start StreamingEngine with stream tracking
        SingularityLLM.Providers.Shared.StreamingEngine,
        # Start Tesla Client Cache
        SingularityLLM.Tesla.ClientCache,
        # Start Cache if enabled
        cache_child_spec(),
        # Start Circuit Breaker Configuration Manager
        SingularityLLM.Infrastructure.CircuitBreaker.ConfigManager,
        # Start Circuit Breaker Metrics system if enabled
        metrics_child_spec(),
        # Start Ollama Model Registry
        SingularityLLM.Infrastructure.OllamaModelRegistry,
        # Start Models.dev syncer for model data enrichment (startup sync if empty, respects 24h TTL)
        SingularityLLM.ModelDiscovery.ModelsDevInitializer,
        # Start Codex Token Manager for OAuth2 token lifecycle management
        codex_token_manager_child_spec()
      ]
      |> Enum.filter(& &1)

    # Only start ModelLoader if Bumblebee is available and not in unit test env
    # Check if we're in test mode by looking for ExUnit
    in_test = Code.ensure_loaded?(ExUnit)
    # Allow ModelLoader in integration tests
    force_start_modelloader = System.get_env("EX_LLM_START_MODELLOADER") == "true"

    children =
      if Code.ensure_loaded?(Bumblebee) and (not in_test or force_start_modelloader) do
        children ++ [SingularityLLM.Providers.Bumblebee.ModelLoader]
      else
        children
      end

    opts = [strategy: :one_for_one, name: SingularityLLM.Supervisor]
    Supervisor.start_link(children, opts)
  end

  defp cache_child_spec do
    if Application.get_env(:singularity_llm, :cache_enabled, false) do
      SingularityLLM.Infrastructure.Cache
    else
      nil
    end
  end

  defp metrics_child_spec do
    config = Application.get_env(:singularity_llm, :circuit_breaker_metrics, [])

    if Keyword.get(config, :enabled, false) and :statsd in Keyword.get(config, :backends, []) do
      SingularityLLM.Infrastructure.CircuitBreaker.Metrics.StatsDReporter
    else
      nil
    end
  end

  defp codex_token_manager_child_spec do
    # Only start TokenManager if Codex credentials exist
    case File.read(Path.expand("~/.codex/auth.json")) do
      {:ok, _} ->
        SingularityLLM.Providers.Codex.TokenManager

      {:error, _} ->
        nil
    end
  end
end
