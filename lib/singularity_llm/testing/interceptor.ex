defmodule SingularityLLM.Testing.TestResponseInterceptor do
  @moduledoc """
  ...
  alias SingularityLLM.Testing.TestCacheDetector
  alias SingularityLLM.Testing.TestCacheStrategy
  alias SingularityLLM.Testing.TestResponseMetadata
  ...
  def intercept_request(url, body, headers, opts) do
    ...
    if Application.get_env(:singularity_llm, :debug_test_cache, false) do
      ...
      cache_key ->
        ...
        case LiveApiCacheStorage.store(cache_key, response_data, metadata) do
          :ok ->
            ...
            SingularityLLM.Infrastructure.Telemetry.emit_test_cache_save(cache_key, size_bytes, metadata)
            :ok
          ...
          {:error, reason} = error ->
            ...
            SingularityLLM.Infrastructure.Telemetry.emit_test_cache_error(cache_key, reason, metadata)
            error
        end
    end
  end
  ...
  def record_cache_hit(cache_key, metadata) do
    ...
    SingularityLLM.Infrastructure.Telemetry.emit_test_cache_hit(cache_key, metadata)
  ...
  def record_cache_miss(cache_key, metadata) do
    ...
    SingularityLLM.Infrastructure.Telemetry.emit_test_cache_miss(cache_key, metadata)
