defmodule SingularityLLM.Testing.Config do
  @moduledoc """
  Centralized test configuration for SingularityLLM.
  ...
  def provider_api_keys do
    ...
    SingularityLLM.Environment.all_api_key_vars()
  ...
  @doc """
  SingularityLLM application configuration for tests.
  """
  ...
  def env_file_path do
    System.get_env("EX_LLM_ENV_FILE") ||
      Application.get_env(:singularity_llm, :env_file) ||
  ...
  def cache_fresh? do
    try do
      SingularityLLM.Testing.Cache.fresh?(max_age: 24 * 60 * 60)
    rescue
      _ -> false
    end
  end
