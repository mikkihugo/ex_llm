defmodule SingularityLLM.Testing.MockResponseRecorder do
  @moduledoc """
  Response caching system for collecting and storing real provider responses.

  This module allows SingularityLLM to cache actual responses from providers like OpenAI,
  ...
  ### Manual Response Caching
      ...
      # Store a response
      SingularityLLM.ResponseCache.store_response("openai", request_key, response_data)
      
      # Retrieve a cached response
      cached = SingularityLLM.ResponseCache.get_response("openai", request_key)
      
      # Load all responses for a provider
      responses = SingularityLLM.ResponseCache.load_provider_responses("anthropic")
  ...
  This is called by SingularityLLM.Cache when disk persistence is enabled.
  """
  ...
  def configure_mock_provider(provider) when is_atom(provider) do
    ...
    if length(responses) > 0 do
      ...
      handler = fn messages, options ->
        ...
        case get_response(to_string(provider), endpoint, request_data) do
          %CacheEntry{response_data: cached_response} ->
            # Convert cached response to SingularityLLM format
      ...
      SingularityLLM.Providers.Mock.set_response_handler(handler)
  ...
  defp convert_cached_response(cached_response, provider) do
    # Convert provider-specific response format to SingularityLLM format
    case provider do
