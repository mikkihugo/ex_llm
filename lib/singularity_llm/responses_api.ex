defmodule SingularityLLM.ResponsesAPI do
  @moduledoc """
  ...
  ## Usage
      ...
      {:ok, response} = SingularityLLM.ResponsesAPI.chat(:openai, [
        %{role: "user", content: "Hello!"}
      ])
      
      # With MCP servers
      {:ok, response} = SingularityLLM.ResponsesAPI.chat(:openai, messages,
        mcp_servers: [
          %{
            name: "filesystem",
            command: "npx",
  ...
  @spec chat(atom(), [map()] | String.t(), keyword()) :: 
    {:ok, SingularityLLM.Types.LLMResponse.t()} | {:error, term()}
  ...
  """
  @spec get_conversation_id(SingularityLLM.Types.LLMResponse.t()) :: String.t() | nil
  ...
  """
  @spec has_tool_calls?(SingularityLLM.Types.LLMResponse.t()) :: boolean()
  ...
  """
  @spec has_mcp_interactions?(SingularityLLM.Types.LLMResponse.t()) :: boolean()
  ...
  defp get_responses_provider(provider) do
    case provider do
      :openai ->
        ...
        if Code.ensure_loaded?(SingularityLLM.Providers.OpenAI.Responses) do
          {:ok, SingularityLLM.Providers.OpenAI.Responses}
        else
      ...
      :openrouter ->
        ...
        if Code.ensure_loaded?(SingularityLLM.Providers.OpenRouter.Responses) do
          {:ok, SingularityLLM.Providers.OpenRouter.Responses}
        else
          {:error, :responses_not_implemented}
        end
      ...
      :codex ->
        ...
        if Code.ensure_loaded?(SingularityLLM.Providers.Codex.Responses) do
          {:ok, SingularityLLM.Providers.Codex.Responses}
        else
          {:error, :responses_not_implemented}
        end
      ...
      :xai ->
        ...
        if Code.ensure_loaded?(SingularityLLM.Providers.XAI.Responses) do
          {:ok, SingularityLLM.Providers.XAI.Responses}
        else
          {:error, :responses_not_implemented}
        end
