defmodule SingularityLLM.API.Transformers do
  @moduledoc """
  ...
  defp build_count_tokens_request(content) when is_binary(content) do
    ...
    text_content = %SingularityLLM.Providers.Gemini.Content.Content{
      parts: [%SingularityLLM.Providers.Gemini.Content.Part{text: content}],
      role: "user"
    }

    %SingularityLLM.Providers.Gemini.Tokens.CountTokensRequest{
  ...
  defp build_count_tokens_request(messages) when is_list(messages) do
    ...
    contents =
      Enum.map(messages, fn message ->
        ...
        %SingularityLLM.Providers.Gemini.Content.Content{
          parts: [%SingularityLLM.Providers.Gemini.Content.Part{text: content}],
          role: role
        }
      end)
  ...
  defp build_count_tokens_request(%SingularityLLM.Providers.Gemini.Tokens.CountTokensRequest{} = request) do
  ...
  defp build_count_tokens_request(content) do
    ...
    %SingularityLLM.Providers.Gemini.Tokens.CountTokensRequest{
      contents: [
        %SingularityLLM.Providers.Gemini.Content.Content{
          parts: [%SingularityLLM.Providers.Gemini.Content.Part{text: inspect(content)}],
          role: "user"
        }
      ]
    }
