defmodule SingularityLLM.API.Delegator do
  @moduledoc """
  ...
  alias SingularityLLM.API.{Capabilities, Transformers}
  ...
  ## Examples
      ...
      {:ok, files} = SingularityLLM.API.Delegator.delegate(:list_files, :openai, [opts])
      
      # With argument transformation  
      {:ok, file} = SingularityLLM.API.Delegator.delegate(:upload_file, :openai, [file_path, opts])
      
      # Unsupported operation
      {:error, "upload_file not supported for provider: anthropic"} = 
        SingularityLLM.API.Delegator.delegate(:upload_file, :anthropic, [file_path, opts])
  """
