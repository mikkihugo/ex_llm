defmodule SingularityLLM.Embeddings do
  @moduledoc """
  Embeddings functionality for SingularityLLM.
  ...
  ## Examples
      ...
      # Find similar items
      results = SingularityLLM.Embeddings.find_similar(query_embedding, items, top_k: 5)
      
      # Calculate similarity
      similarity = SingularityLLM.Embeddings.cosine_similarity(vector1, vector2)
  """
  ...
  def find_similar(query_embedding, items, opts \\ []) do
    SingularityLLM.Core.Embeddings.find_similar(query_embedding, items, opts)
  end
  ...
  ## Examples
      ...
      similarity = SingularityLLM.Embeddings.cosine_similarity([1.0, 2.0], [3.0, 4.0])
      # => 0.9838699100999074
      
      # Identical vectors have similarity of 1.0
      SingularityLLM.Embeddings.cosine_similarity([1, 2, 3], [1, 2, 3])
      # => 1.0
      
      # Orthogonal vectors have similarity of 0.0
      SingularityLLM.Embeddings.cosine_similarity([1, 0], [0, 1])
      # => 0.0
  """
  @spec cosine_similarity([float()], [float()]) :: float()
  def cosine_similarity(vector1, vector2) do
    SingularityLLM.Core.Embeddings.similarity(vector1, vector2, :cosine)
  end
  ...
  ## Examples
      ...
      # With custom metric
      similarity = SingularityLLM.Embeddings.similarity([0.1, 0.2], [0.3, 0.4], :dot_product)
  """
  @spec similarity([float()], [float()], atom()) :: float()
  def similarity(vector1, vector2, metric \\ :cosine) do
    SingularityLLM.Core.Embeddings.similarity(vector1, vector2, metric)
  end
  ...
  ## Examples
      ...
      {:ok, response} = SingularityLLM.Embeddings.batch_generate(:openai, [
        "First document text",
      ...
      # With custom model
      {:ok, response} = SingularityLLM.Embeddings.batch_generate(:openai, texts,
  ...
  def batch_generate(provider, requests, opts \\ []) do
    ...
    SingularityLLM.Core.Embeddings.batch_generate(provider, formatted_requests)
  end
  ...
  ## Examples
      ...
      {:ok, info} = SingularityLLM.Embeddings.model_info(:openai, "text-embedding-3-small")
      # => {:ok, %{
  ...
  def model_info(provider, model_id) do
    SingularityLLM.get_model_info(provider, model_id)
  end
  ...
  ## Examples
      ...
      {:ok, index} = SingularityLLM.Embeddings.create_index(:openai, documents, key: :text)
  """
