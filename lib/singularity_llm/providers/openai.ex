defmodule SingularityLLM.Providers.OpenAI do
  @moduledoc """
  OpenAI GPT API adapter for SingularityLLM.
  ...
  ### Using Environment Variables
      ...
      SingularityLLM.Providers.OpenAI.chat(messages, config_provider: SingularityLLM.Infrastructure.ConfigProvider.Env)
  ...
  ### Using Static Configuration
      ...
      SingularityLLM.Providers.OpenAI.chat(messages, config_provider: provider)
  ...
  ## Example Usage
      ...
      # Simple chat
      {:ok, response} = SingularityLLM.Providers.OpenAI.chat(messages)
      IO.puts(response.content)

      # Streaming chat
      {:ok, stream} = SingularityLLM.Providers.OpenAI.stream_chat(messages)
      for chunk <- stream do
  ...
  @behaviour SingularityLLM.Providers.Shared.StreamingBehavior
  ...
  def chat(messages, options \\ []) do
    with :ok <- MessageFormatter.validate_messages(messages),
      ...
      SingularityLLM.Infrastructure.Telemetry.span([:singularity_llm, :provider, :request], metadata, fn ->
  ...
  @impl SingularityLLM.Providers.Shared.StreamingBehavior
  ...
  def parse_response(response, model) do
    ...
    # Calculate cost info
    cost_info =
      SingularityLLM.Core.Cost.calculate("openai", model, %{
  ...
  defp get_context_window(model_id) do
    ...
    SingularityLLM.Infrastructure.Config.ModelConfig.get_context_window(:openai, model_id)
  end
  ...
  defp parse_embeddings_response(response, _inputs, _config, options) do
    case response do
      %{"data" => data, "usage" => usage, "model" => model} ->
        ...
        embedding_response =
          if Keyword.get(options, :track_cost, true) do
            cost = SingularityLLM.Core.Cost.calculate("openai", model, embedding_response.usage)
      ...
      _ ->
        {:error, SingularityLLM.Infrastructure.Error.json_parse_error("Invalid embeddings response")}
    end
  end
  ...
  defp validate_file_purpose(purpose) do
    SingularityLLM.Infrastructure.Error.validation_error(
      :purpose,
  ...
  defp validate_upload_params(params) do
    ...
    if missing != [] do
      ...
      if params[:bytes] > max_bytes do
        SingularityLLM.Infrastructure.Error.validation_error(:bytes, "Upload size cannot exceed 8 GB")
      else
        # Validate purpose
  ...
  defp validate_part_size(data) do
    ...
    if size > max_part_size do
      SingularityLLM.Infrastructure.Error.validation_error(:data, "Part size cannot exceed 64 MB")
    else
      {:ok, data}
    end
  end
  ...
  defp handle_openai_response({:ok, env}, opts) do
    if SingularityLLM.HTTP.successful?(env) do
      body = SingularityLLM.HTTP.get_body(env)
      headers = SingularityLLM.HTTP.get_headers(env)
      expect_binary = Keyword.get(opts, :expect_binary, false)
      parsed_body = parse_openai_response_body(body, headers, expect_binary)
      {:ok, parsed_body}
    else
      {status, body} = SingularityLLM.HTTP.get_error_info(env)
      {:error, {:api_error, %{status: status, body: body}}}
    end
  end
