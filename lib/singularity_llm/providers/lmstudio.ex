defmodule SingularityLLM.Providers.LMStudio do
  @moduledoc """
  ...
  on `http://localhost:1234` with API key `"lm-studio"`.
      ...
      # Basic usage
      {:ok, response} = SingularityLLM.chat(:lmstudio, messages)
  ...
  the high-level `SingularityLLM.stream/4` API may not work as expected. Instead, use the
  provider's direct streaming method:

      # Recommended approach for streaming with LM Studio
      {:ok, stream} = SingularityLLM.Providers.LMStudio.stream_chat(messages, model: "gpt-4")
  ...
  alias SingularityLLM.Providers.Shared.ModelUtils
  ...
  def chat(messages, options) do
    SingularityLLM.Core.Chat.chat(:lmstudio, messages, options)
  end

  @impl SingularityLLM.Provider
  def stream_chat(messages, options) do
    SingularityLLM.Core.Chat.stream_chat(:lmstudio, messages, options)
  end
