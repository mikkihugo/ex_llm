defmodule SingularityLLM.Providers.Mistral do
  @moduledoc """
  Mistral AI API adapter for SingularityLLM.
  ...
  ### Using Environment Variables
      ...
      SingularityLLM.Providers.Mistral.chat(messages, config_provider: SingularityLLM.Infrastructure.ConfigProvider.Env)
  ...
  ### Using Static Configuration
      ...
      SingularityLLM.Providers.Mistral.chat(messages, config_provider: provider)
  ...
  ## Example Usage
      ...
      # Simple chat
      {:ok, response} = SingularityLLM.Providers.Mistral.chat(messages)
      IO.puts(response.content)

      # Streaming chat
      {:ok, stream} = SingularityLLM.Providers.Mistral.stream_chat(messages)
      for chunk <- stream do
      ...
      {:ok, response} = SingularityLLM.Providers.Mistral.chat(messages, tools: functions)
  """

  import SingularityLLM.Providers.Shared.ModelUtils, only: [format_model_name: 1]
  ...
  def chat(messages, options) do
    SingularityLLM.Core.Chat.chat(:mistral, messages, options)
  end

  @impl SingularityLLM.Provider
  def stream_chat(messages, options) do
    SingularityLLM.Core.Chat.stream_chat(:mistral, messages, options)
  end

  alias SingularityLLM.Infrastructure.Config.ModelConfig
  alias SingularityLLM.Types
  alias SingularityLLM.Providers.Shared.{ConfigHelper, ResponseBuilder, Validation}
  import SingularityLLM.Providers.Shared.ModelUtils, only: [generate_description: 2]
  alias SingularityLLM.Infrastructure.Error, as: Error
  ...
  @impl SingularityLLM.Providers.OpenAICompatible
  ...
  def parse_model(model) do
    ...
    %Types.Model{
      id: model_id,
      name: SingularityLLM.Providers.Shared.ModelUtils.format_model_name(model_id),
    ...
    if map_size(models_map) > 0 do
      models =
        ...
        |> Enum.map(fn {model_id, config} ->
          ...
          %Types.Model{
            id: model_id_str,
            name: SingularityLLM.Providers.Shared.ModelUtils.format_model_name(model_id_str),
            description:
