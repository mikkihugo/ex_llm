defmodule SingularityLLM.Providers.OpenRouter.Responses do
  @moduledoc """
  ...
  ## Usage
      ...
      # Basic usage
      {:ok, response} = SingularityLLM.Providers.OpenRouter.Responses.chat([
        %{role: "user", content: "Hello!"}
      ])
      
      # With MCP servers
      {:ok, response} = SingularityLLM.Providers.OpenRouter.Responses.chat(messages,
        mcp_servers: [
          %{
            name: "filesystem",
            command: "npx",
  ...
  import SingularityLLM.Providers.Shared.ConfigHelper
  ...
  def configured?(opts \\ []) do
    ...
    case SingularityLLM.Providers.OpenRouter.configured?(opts) do
      true -> true
      false -> false
    end
  end
  ...
  def chat(messages, opts \\ []) do
    ...
    with {:ok, config} <- get_config(:openrouter, config_provider),
      ...
      {:error, :responses_not_available} ->
        ...
        SingularityLLM.Providers.OpenRouter.chat(messages, opts)
      {:error, reason} -> 
        {:error, reason}
    end
  end
  ...
  def stream_chat(messages, opts \\ []) do
    ...
    SingularityLLM.Providers.OpenRouter.stream_chat(messages, opts)
  end

  @impl true
  def list_models(opts \\ []) do
    # Use the same models as regular OpenRouter
    SingularityLLM.Providers.OpenRouter.list_models(opts)
  end
  ...
  def execute_responses_request(request, config) do
    ...
    case SingularityLLM.Providers.Shared.HTTP.Core.post(url, request, headers) do
