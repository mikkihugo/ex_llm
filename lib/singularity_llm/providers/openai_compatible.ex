defmodule SingularityLLM.Providers.OpenAICompatible do
  ...
  Create a new provider by using this module:
      ...
      defmodule SingularityLLM.Providers.MyProvider do
  ...
  defmacro __using__(opts) do
    ...
    quote do
      ...
      @behaviour SingularityLLM.Providers.OpenAICompatible
      @behaviour SingularityLLM.Providers.Shared.StreamingBehavior

      alias SingularityLLM.Infrastructure.{Error, Logger}
      alias Types
      ...
      defp parse_chat_response(response, model, options) do
        case response do
          ...
          _ ->
            {:error, SingularityLLM.Infrastructure.Error.json_parse_error("Invalid response format")}
        end
      end
      ...
      defp add_cost_tracking(response, options) do
        if Keyword.get(options, :track_cost, true) && response.usage do
          cost =
            SingularityLLM.Core.Cost.calculate(Atom.to_string(@provider), response.model, response.usage)
      ...
      defp handle_error(%{network_error: reason}) do
        {:error, SingularityLLM.Infrastructure.Error.connection_error(reason)}
      end

      defp handle_error(error) do
        {:error, SingularityLLM.Infrastructure.Error.unknown_error(error)}
      end
      ...
      def parse_error(%{status: 401}) do
        {:error, SingularityLLM.Infrastructure.Error.authentication_error("Invalid API key")}
      end

      def parse_error(%{status: 429, body: body}) do
        {:error, SingularityLLM.Infrastructure.Error.rate_limit_error(inspect(body))}
      end

      def parse_error(%{status: status, body: body}) when status >= 500 do
        {:error, SingularityLLM.Infrastructure.Error.api_error(status, body)}
      end
