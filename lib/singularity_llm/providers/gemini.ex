defmodule SingularityLLM.Providers.Gemini do
  @moduledoc """
  Google Gemini API adapter for SingularityLLM.
  ...
  ### Using Environment Variables
      ...
      SingularityLLM.Providers.Gemini.chat(messages, config_provider: SingularityLLM.Infrastructure.ConfigProvider.Env)
  ...
  ### Using Static Configuration
      ...
      SingularityLLM.Providers.Gemini.chat(messages, config_provider: provider)
  ...
  ## Example Usage
      ...
      # Simple chat
      {:ok, response} = SingularityLLM.Providers.Gemini.chat(messages)
      IO.puts(response.content)

      # Streaming chat
      {:ok, stream} = SingularityLLM.Providers.Gemini.stream_chat(messages)
      for chunk <- stream do
  ...
  You can customize safety settings:
      ...
      {:ok, response} = SingularityLLM.Providers.Gemini.chat(messages, options)
  ...
  alias SingularityLLM.Providers.Shared.{ConfigHelper, ModelUtils}

  alias SingularityLLM.Providers.Gemini.Content.{
    Content,
  ...
  def chat(messages, options \\ []) do
    config_provider =
      Keyword.get(
        ...
        Application.get_env(
          :singularity_llm,
          :config_provider,
          SingularityLLM.Infrastructure.ConfigProvider.Default
        )
      )
    ...
    if !api_key || api_key == "" do
      ...
      case SingularityLLM.Providers.Gemini.Content.generate_content(model, request,
  ...
  def stream_chat(messages, options \\ []) do
    ...
    if !api_key || api_key == "" do
      ...
      case SingularityLLM.Providers.Gemini.Content.stream_generate_content(model, request,
             config_provider: config_provider
           ) do
        {:ok, stream} ->
          # Convert the Content stream to SingularityLLM stream format
          converted_stream =
  ...
  def list_models(options \\ []) do
    ...
    case SingularityLLM.Providers.Gemini.Models.list_models(options) do
      {:ok, response} ->
        # Convert from Models API format to SingularityLLM Types.Model format
        models =
          response[:models]
      ...
      {:error, _reason} ->
        ...
        SingularityLLM.Infrastructure.Config.ModelLoader.load_models(
          :gemini,
  ...
  defp is_gemini_chat_model_struct?(%SingularityLLM.Providers.Gemini.Models.Model{} = model) do
    # Filter for Gemini chat models from API struct
    String.starts_with?(model.name, "models/gemini")
  end

  defp convert_api_model_to_types(%SingularityLLM.Providers.Gemini.Models.Model{} = api_model) do
    # Convert from Gemini Models API format to SingularityLLM Types.Model format
    ...
    if !api_key || api_key == "" do
      ...
      case inputs do
        [single_text] when is_binary(single_text) ->
          ...
          case SingularityLLM.Providers.Gemini.Embeddings.embed_text(model, single_text, options) do
            {:ok, embedding} ->
              # Convert from Gemini format to SingularityLLM format - embeddings should be list(list(float()))
              {:ok,
        ...
        multiple_texts when is_list(multiple_texts) ->
          ...
          case SingularityLLM.Providers.Gemini.Embeddings.embed_texts(model, multiple_texts, options) do
            {:ok, embeddings} ->
              # Convert from Gemini format to SingularityLLM format - embeddings should be list(list(float()))
  ...
  def get_model(model_name, options \\ []) do
    case SingularityLLM.Providers.Gemini.Models.get_model(model_name, options) do
      {:ok, api_model} ->
        # Convert from Gemini Models API format to SingularityLLM Types.Model format
        model_id = String.replace_prefix(api_model.name, "models/", "")
  ...
  def count_tokens_with_request(request, options \\ []) do
    ...
    count_request = %SingularityLLM.Providers.Gemini.Tokens.CountTokensRequest{
      generate_content_request: generate_content_request
    }

    case SingularityLLM.Providers.Gemini.Tokens.count_tokens(model, count_request,
           config_provider: config_provider
         ) do
      {:ok, result} ->
  ...
  def count_tokens(messages, model, options \\ []) do
    ...
    request = %SingularityLLM.Providers.Gemini.Tokens.CountTokensRequest{
      contents: contents
    }

    case SingularityLLM.Providers.Gemini.Tokens.count_tokens(model, request, options) do
      {:ok, response} ->
  ...
  def list_files(options \\ []) do
    case SingularityLLM.Providers.Gemini.Files.list_files(options) do
      {:ok, response} ->
        # Convert to simple map format expected by tests
        {:ok,
         %{
  ...
  def create_file(file_content, display_name, options \\ []) do
    ...
    try do
      ...
      case SingularityLLM.Providers.Gemini.Files.upload_file(temp_path, upload_options) do
        {:ok, file} ->
          {:ok, file_to_map(file)}
  ...
  def get_file(file_name, options \\ []) do
    case SingularityLLM.Providers.Gemini.Files.get_file(file_name, options) do
      {:ok, file} ->
        {:ok, file_to_map(file)}
  ...
  def delete_file(file_name, options \\ []) do
    case SingularityLLM.Providers.Gemini.Files.delete_file(file_name, options) do
      :ok ->
        {:ok, %{}}
  ...
  alias SingularityLLM.Providers.Gemini.Caching
      ...
      [candidate | _] ->
        ...
        usage =
          if response.usage_metadata do
            ...
            %{
              input_tokens: 100,
              output_tokens: SingularityLLM.Core.Cost.estimate_tokens(content),
              total_tokens: 100 + SingularityLLM.Core.Cost.estimate_tokens(content)
            }
          end
  ...
  defp file_to_map(%SingularityLLM.Providers.Gemini.Files.File{} = file) do
    %{
      "name" => file.name,
