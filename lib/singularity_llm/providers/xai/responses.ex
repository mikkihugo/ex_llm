defmodule SingularityLLM.Providers.XAI.Responses do
  @moduledoc """
  ...
  ## Usage
      ...
      # Simple request
      {:ok, response} = SingularityLLM.Providers.XAI.Responses.chat([
  ...
  def chat(messages, opts \\ []) do
    with {:ok, config} <- get_config(opts),
      ...
      {:error, :responses_not_available} ->
        ...
        SingularityLLM.Providers.XAI.chat(messages, opts)
      {:error, reason} ->
        {:error, reason}
    end
  end
  ...
  defp get_config(opts) do
    api_key = Keyword.get(opts, :api_key) || 
              Application.get_env(:singularity_llm, :xai_api_key) ||
