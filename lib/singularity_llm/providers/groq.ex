defmodule SingularityLLM.Providers.Groq do
  @moduledoc """
  Groq adapter for SingularityLLM.
  ...
  ## Example Usage
      ...
      {:ok, response} = SingularityLLM.Providers.Groq.chat(messages, model: "llama3-70b-8192")
  """

  alias SingularityLLM.Infrastructure.Config.ModelConfig
  alias SingularityLLM.Providers.Shared.{ConfigHelper, ErrorHandler, ModelUtils}
  ...
  def chat(messages, options) do
    SingularityLLM.Core.Chat.chat(:groq, messages, options)
  end

  @impl SingularityLLM.Provider
  def stream_chat(messages, options) do
    SingularityLLM.Core.Chat.stream_chat(:groq, messages, options)
  end
    ...
    if Keyword.get(options, :source) == :api do
      ...
      SingularityLLM.Infrastructure.Config.ModelLoader.load_models(
        ...
        Keyword.merge(options,
          ...
          config_transformer: &SingularityLLM.Providers.OpenAICompatible.default_model_transformer/2
        )
      )
    else
