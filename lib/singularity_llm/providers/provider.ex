defmodule SingularityLLM.Provider do
  @moduledoc """
  ...
  ## Example Implementation
      ...
      defmodule MyProvider do
        ...
        def chat(messages, options \\ []) do
          # Implementation here
          {:ok, %SingularityLLM.Types.LLMResponse{content: "Hello"}}
        end
  ...
  Providers support configuration injection through the `:config_provider` option:
      ...
      # Use static config
      {:ok, provider} = SingularityLLM.Infrastructure.ConfigProvider.Static.start_link(%{
        openai: %{api_key: "sk-..."}
      })
  ...
  ## Returns
  `{:ok, %SingularityLLM.Types.LLMResponse{}}` on success, `{:error, reason}` on failure.
  """
  ...
  ## Returns
  `{:ok, [%SingularityLLM.Types.Model{}]}` on success, `{:error, reason}` on failure.
  """
  ...
  ## Returns
  `{:ok, %SingularityLLM.Types.EmbeddingResponse{}}` on success, `{:error, reason}` on failure.
  """
  ...
  ## Returns  
  `{:ok, [%SingularityLLM.Types.EmbeddingModel{}]}` on success, `{:error, reason}` on failure.
  """
