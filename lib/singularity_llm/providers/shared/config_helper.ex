defmodule SingularityLLM.Providers.Shared.ConfigHelper do
  @moduledoc false

  alias SingularityLLM.{Infrastructure.Config.ModelConfig, Infrastructure.ConfigProvider}
  ...
  ## Examples
      ...
      iex> ConfigHelper.get_config(:anthropic, SingularityLLM.Infrastructure.ConfigProvider.Env)
    ...
    case config_provider do
      ...
      ConfigProvider.Static ->
        ...
        You must create an instance first:
          ...
          SingularityLLM.chat(messages, config_provider: provider)
        """

      provider when is_pid(provider) ->
        # Static provider instance - get all config and extract adapter-specific part
        all_config = SingularityLLM.Infrastructure.ConfigProvider.Static.get_all(provider) || %{}
  ...
  def get_config_provider(options) do
    Keyword.get(
      options,
      :config_provider,
      Application.get_env(:singularity_llm, :config_provider, ConfigProvider.Default)
    )
  end
