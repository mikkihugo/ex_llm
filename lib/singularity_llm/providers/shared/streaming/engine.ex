defmodule SingularityLLM.Providers.Shared.Streaming.Engine do
  @moduledoc """
  ...
  defp build_middleware_stack(opts) do
    ...
    headers = [{"user-agent", "SingularityLLM/1.0.0"}] ++ build_auth_headers(provider, api_key)
    ...
    base_middleware = [
      ...
      SingularityLLM.Providers.Shared.Streaming.Middleware.StreamCollector
    ]
  ...
  defp maybe_add_metrics_middleware(middleware, opts) do
    if Keyword.get(opts, :enable_metrics, false) do
      ...
      middleware ++ [{SingularityLLM.Providers.Shared.Streaming.Middleware.MetricsPlug, metrics_opts}]
    else
      middleware
    end
  end
