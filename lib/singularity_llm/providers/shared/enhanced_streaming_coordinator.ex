defmodule SingularityLLM.Providers.Shared.EnhancedStreamingCoordinator do
  @moduledoc """
  ...
  alias SingularityLLM.Infrastructure.Streaming.FlowController
    ...
    result =
      case Core.stream(client, path, request, stream_callback,
        ...
        {:ok, _response} ->
          ...
          enhanced_callback.(%SingularityLLM.Types.StreamChunk{
            content: "",
            finish_reason: "stop"
          })
  ...
  defp handle_event_data(data, parse_chunk_fn, recovery_id, stats, options) do
    ...
    case parse_chunk_fn.(data) do
      ...
      %SingularityLLM.Types.StreamChunk{} = chunk ->
        validate_and_save_chunk(chunk, recovery_id, stats.chunk_count, options)
  ...
  defp save_chunk_if_recovery_enabled(recovery_id, chunk, chunk_count, options) do
    if Keyword.get(options, :stream_recovery, false) do
      ...
      if stream_recovery_enabled?() do
        ...
        SingularityLLM.Core.Streaming.Recovery.record_chunk(recovery_id, chunk)
  ...
  defp handle_enhanced_stream_error(reason, callback, stream_context, options) do
    ...
    error_chunk = %SingularityLLM.Types.StreamChunk{
  ...
  defp save_stream_state(recovery_id, _url, request, _headers, options) do
    ...
    if stream_recovery_enabled?() do
      ...
      {:ok, _} = SingularityLLM.Core.Streaming.Recovery.init_recovery(provider, messages, options)
  ...
  defp mark_stream_recoverable(recovery_id, reason) do
    if stream_recovery_enabled?() do
      case SingularityLLM.Core.Streaming.Recovery.record_error(recovery_id, reason) do
        {:ok, recoverable} ->
          if recoverable do
            Logger.info(
  ...
  defp cleanup_stream_state(recovery_id) do
    if stream_recovery_enabled?() do
      ...
      SingularityLLM.Core.Streaming.Recovery.complete_stream(recovery_id)
      Logger.debug("Enhanced stream recovery state cleaned up: #{recovery_id}")
    end
  end

  defp stream_recovery_enabled? do
    Process.whereis(SingularityLLM.Core.Streaming.Recovery) != nil
  end
