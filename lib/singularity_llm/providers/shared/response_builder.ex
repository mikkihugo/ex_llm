defmodule SingularityLLM.Providers.Shared.ResponseBuilder do
  @moduledoc """
  ...
  alias SingularityLLM.{Core.Cost, Types}
  ...
  def build_error_response(status, data, opts \\ []) do
    ...
    case normalize_error_data(data) do
      ...
      _ ->
        {:error, SingularityLLM.Infrastructure.Error.api_error(status, data)}
    end
  end
  ...
  Maps provider-specific field names to standardized SingularityLLM format:
  - API field names: prompt_tokens, completion_tokens (OpenAI, OpenRouter, etc.)
  - SingularityLLM field names: input_tokens, output_tokens
  """
  ...
  defp categorize_structured_error("authentication_error", message) do
    {:error, SingularityLLM.Infrastructure.Error.authentication_error(message)}
  end

  defp categorize_structured_error("rate_limit_error", message) do
    {:error, SingularityLLM.Infrastructure.Error.rate_limit_error(message)}
  end

  defp categorize_structured_error("invalid_request_error", message) do
    {:error, SingularityLLM.Infrastructure.Error.validation_error(:request, message)}
  end
  ...
  defp categorize_structured_error(_, message) do
    {:error, SingularityLLM.Infrastructure.Error.api_error(500, message)}
  end
  ...
  defp categorize_error_by_status(503, message) do
    {:error, SingularityLLM.Infrastructure.Error.service_unavailable(message)}
  end

  defp categorize_error_by_status(status, message) do
    {:error, SingularityLLM.Infrastructure.Error.api_error(status, message)}
  end
