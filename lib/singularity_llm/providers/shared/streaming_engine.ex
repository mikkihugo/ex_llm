defmodule SingularityLLM.Providers.Shared.StreamingEngine do
  @moduledoc """
  Unified streaming facade that replaces the three parallel streaming implementations.

  This module serves as the single entry point for all streaming operations in SingularityLLM.
  ...
  alias SingularityLLM.Providers.Shared.{EnhancedStreamingCoordinator, StreamingCoordinator}
  require Logger
  ...
  defdelegate tesla_client(opts), to: SingularityLLM.Providers.Shared.Streaming.Engine, as: :client

  defdelegate tesla_stream(client, path, body, opts),
    to: SingularityLLM.Providers.Shared.Streaming.Engine,
    as: :stream
  ...
  def advanced_features_available?() do
    Code.ensure_loaded?(SingularityLLM.Infrastructure.Streaming.FlowController) and
      Code.ensure_loaded?(SingularityLLM.Providers.Shared.EnhancedStreamingCoordinator)
  end
