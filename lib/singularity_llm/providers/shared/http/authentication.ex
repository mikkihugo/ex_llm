defmodule SingularityLLM.Providers.Shared.HTTP.Authentication do
  @moduledoc """
  ...
  defp add_version_headers(env, :gemini, opts) do
    ...
    env = Tesla.put_header(env, "x-goog-api-client", "singularity_llm/1.0.0")
  ...
  defp add_version_headers(env, :openrouter, opts) do
    env = Tesla.put_header(env, "x-title", Keyword.get(opts, :app_name, "SingularityLLM"))
  ...
  defp get_config_api_key(provider, opts) do
    case Keyword.get(opts, :config_provider) do
      ...
      config_provider ->
        case SingularityLLM.Infrastructure.ConfigProvider.get_config(config_provider, provider) do
