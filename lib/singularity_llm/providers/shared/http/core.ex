defmodule SingularityLLM.Providers.Shared.HTTP.Core do
  @moduledoc """
  ...
  alias SingularityLLM.Providers.Shared.HTTP
  ...
  def client(opts \\ []) do
    ...
    SingularityLLM.Tesla.ClientCache.get_or_create(provider, cache_config, fn ->
  ...
  def stream(client, path, body, callback, opts \\ []) do
    ...
    if Application.get_env(:singularity_llm, :use_tesla_mock, false) do
  ...
  defp handle_streaming_response(env, callback, parse_chunk_fn, timeout) do
    cond do
      SingularityLLM.HTTP.successful?(env) ->
        status = SingularityLLM.HTTP.get_status(env)
      ...
      SingularityLLM.HTTP.timeout_error?(env) ->
        {:error, :timeout}
  ...
  defp build_middleware_stack(provider, opts) do
    ...
    logging_middleware =
      if Keyword.get(opts, :logging_enabled, true) do
        debug_level =
          Keyword.get(opts, :debug, Application.get_env(:singularity_llm, :log_level, :info) == :debug)
  ...
  defp build_adapter(opts) do
    ...
    default_adapter =
      if Application.get_env(:singularity_llm, :use_tesla_mock, false) do
        ...
        SingularityLLM.Providers.Shared.HTTP.SafeHackneyAdapter
      end
  ...
  defp default_parse_chunk(data) do
    case Jason.decode(data) do
      {:ok, parsed} ->
        ...
        chunk = %SingularityLLM.Types.StreamChunk{
          content: content,
