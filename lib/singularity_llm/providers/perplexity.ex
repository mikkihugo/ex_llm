defmodule SingularityLLM.Providers.Perplexity do
  @moduledoc """
  Perplexity AI API adapter for SingularityLLM.
  ...
  ### Using Environment Variables
      ...
      SingularityLLM.Providers.Perplexity.chat(messages, config_provider: SingularityLLM.Infrastructure.ConfigProvider.Env)
  ...
  ### Using Static Configuration
      ...
      SingularityLLM.Providers.Perplexity.chat(messages, config_provider: provider)
  ...
  ## Example Usage
      ...
      {:ok, response} = SingularityLLM.Providers.Perplexity.chat(messages, model: "perplexity/sonar-pro")
      IO.puts(response.content)

      # Academic search mode
      {:ok, response} = SingularityLLM.Providers.Perplexity.chat(messages,
        model: "perplexity/sonar-pro",
      ...
      {:ok, stream} = SingularityLLM.Providers.Perplexity.stream_chat(messages,
  ...
  import SingularityLLM.Providers.Shared.ModelUtils, only: [format_model_name: 1, generate_description: 2]
  import SingularityLLM.Providers.OpenAICompatible, only: [default_model_transformer: 2]
  ...
  def chat(messages, options) do
    with :ok <- validate_perplexity_parameters(options) do
      SingularityLLM.Core.Chat.chat(:perplexity, messages, options)
    end
  end
  ...
  def stream_chat(messages, options) do
    with :ok <- validate_perplexity_parameters(options) do
      SingularityLLM.Core.Chat.stream_chat(:perplexity, messages, options)
    end
  end
