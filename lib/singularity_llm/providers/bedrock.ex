defmodule SingularityLLM.Providers.Bedrock do
  @moduledoc """
  AWS Bedrock adapter for SingularityLLM.
  ...
  ## Example Usage
      ...
      # Simple chat
      {:ok, response} = SingularityLLM.Providers.Bedrock.chat(messages)
      IO.puts(response.content)

      # Streaming chat
      {:ok, stream} = SingularityLLM.Providers.Bedrock.stream_chat(messages)
      for chunk <- stream do
  ...
  alias SingularityLLM.{Infrastructure.Config.ModelConfig, Infrastructure.Logger, Types}
  ...
  defp get_config(options) do
    ...
    app_config = Application.get_env(:singularity_llm, :bedrock, [])
