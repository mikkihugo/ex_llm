defmodule SingularityLLM.Providers.Bumblebee do
  @moduledoc """
  ...
  The Bumblebee adapter doesn't require API keys but may need backend configuration:
      ...
      {:ok, response} = SingularityLLM.chat(:bumblebee, messages)
      
      # With specific model
      {:ok, response} = SingularityLLM.chat(:bumblebee, messages, model: "microsoft/phi-2")
  ...
  alias SingularityLLM.Providers.Bumblebee.ModelLoader
  ...
  defp do_stream_chat(messages, opts) do
    ...
    case do_chat(messages, opts) do
      {:ok, chunks} when is_list(chunks) ->
        # Convert to stream format expected by SingularityLLM
        stream =
  ...
  defp do_list_models(opts) do
    ...
    # Convert to SingularityLLM Model format
    models =
      all_models
      |> Enum.uniq()
  ...
  defp model_loader_running? do
    case Process.whereis(SingularityLLM.Providers.Bumblebee.ModelLoader) do
      nil -> false
      _pid -> true
    end
  end
