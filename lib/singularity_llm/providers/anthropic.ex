defmodule SingularityLLM.Providers.Anthropic do
  @moduledoc """
  Anthropic Claude API adapter for SingularityLLM.
  ...
  ### Using Environment Variables
      ...
      SingularityLLM.Providers.Anthropic.chat(messages, config_provider: SingularityLLM.Infrastructure.ConfigProvider.Env)
  ...
  ### Using Static Configuration
      ...
      SingularityLLM.Providers.Anthropic.chat(messages, config_provider: provider)
  ...
  ## Example Usage
      ...
      # Simple chat
      {:ok, response} = SingularityLLM.Providers.Anthropic.chat(messages)
      IO.puts(response.content)

      # Streaming chat
      {:ok, stream} = SingularityLLM.Providers.Anthropic.stream_chat(messages)
      for chunk <- stream do
  ...
  import SingularityLLM.Providers.OpenAICompatible,
  ...
  defp parse_response(response) do
    ...
    cost =
      if usage && response["model"] do
        SingularityLLM.Core.Cost.calculate("anthropic", response["model"], usage)
      else
        nil
      end
  ...
  defp handle_anthropic_response({:ok, env}, method) do
    case SingularityLLM.HTTP.handle_response(env) do
      {:ok, body} ->
