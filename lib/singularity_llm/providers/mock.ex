defmodule SingularityLLM.Providers.Mock do
  @moduledoc """
  Mock adapter for testing SingularityLLM integrations.

  This adapter provides predictable responses for testing without making actual API calls.
  It supports all SingularityLLM features including streaming, function calling, and error simulation.
  ...
  The mock adapter can be configured with predefined responses or response generators:
      ...
      SingularityLLM.Providers.Mock.set_response_handler(fn messages, options ->
        %{content: "You said: \#{List.last(messages).content}"}
      end)
      
      # Simulate errors
      SingularityLLM.Providers.Mock.set_error({:api_error, %{status: 500, body: "Server error"}})
  ...
  ## Examples
      ...
      SingularityLLM.Providers.Mock.use_cached_responses(:openai)
      
      # Use cached Anthropic responses  
      SingularityLLM.Providers.Mock.use_cached_responses("anthropic")
  ...
  def use_cached_responses(provider) when is_atom(provider) do
    ...
    case SingularityLLM.Testing.MockResponseRecorder.configure_mock_provider(provider) do
      :ok ->
        Agent.update(__MODULE__, fn state ->
  ...
  def list_cached_providers do
    SingularityLLM.Testing.MockResponseRecorder.list_cached_providers()
  end
  ...
  def chat(messages, options \\ []) do
    ...
    if mock_error = Keyword.get(options, :mock_error) do
      ...
      case Application.get_env(:singularity_llm, :mock_responses, %{})[:chat] do
        nil ->
  ...
  def stream_chat(messages, options \\ []) do
    ...
    case Application.get_env(:singularity_llm, :mock_responses, %{})[:stream] do
      nil ->
  ...
  def embeddings(inputs, options \\ []) do
    ...
    # Check for mock response in Application env first
    case Application.get_env(:singularity_llm, :mock_responses, %{})[:embeddings] do
      nil ->
  ...
  defp build_embedding_response(embeddings, inputs, options) do
    base_response = %SingularityLLM.Types.EmbeddingResponse{
  ...
  def list_embedding_models(_options \\ []) do
    # Check for mock response in Application env first
    case Application.get_env(:singularity_llm, :mock_responses, %{})[:list_embedding_models] do
      nil ->
        # Default models
        models = [
          %SingularityLLM.Types.EmbeddingModel{
  ...
  defp normalize_response(%SingularityLLM.Types.LLMResponse{} = response), do: response
  ...
  defp default_stream_chunks do
    [
      %SingularityLLM.Types.StreamChunk{content: "This ", finish_reason: nil},
      %SingularityLLM.Types.StreamChunk{content: "is ", finish_reason: nil},
      %SingularityLLM.Types.StreamChunk{content: "a ", finish_reason: nil},
      %SingularityLLM.Types.StreamChunk{content: "mock ", finish_reason: nil},
      %SingularityLLM.Types.StreamChunk{content: "stream ", finish_reason: nil},
      %SingularityLLM.Types.StreamChunk{content: "response", finish_reason: nil},
      %SingularityLLM.Types.StreamChunk{content: "", finish_reason: "stop"}
    ]
  end
  ...
  defp handle_embeddings_response(embeddings, inputs, options) when is_list(embeddings) do
    response = %SingularityLLM.Types.EmbeddingResponse{
      embeddings: embeddings,
      model: Keyword.get(options, :model, "mock-embedding-model"),
      usage: %{
        input_tokens: estimate_tokens(inputs),
