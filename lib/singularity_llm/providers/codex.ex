defmodule SingularityLLM.Providers.Codex do
  @moduledoc """
  OpenAI Codex CLI Provider for SingularityLLM.

  Integrates with OpenAI's Codex CLI tool, which stores OAuth2 credentials in `~/.codex/auth.json`.
  This provider acts as a bridge to use Codex-authenticated LLM access from SingularityLLM.
  ...
  For real-time chat completions with streaming responses.
      ...
      iex> {:ok, response} = SingularityLLM.Providers.Codex.chat([
  ...
  For code generation tasks using the WHAM protocol.
      ...
      iex> {:ok, task_id} = SingularityLLM.Providers.Codex.create_task(
      ...>   environment_id: "owner/repo",
      ...>   branch: "main",
      ...>   prompt: "Add dark mode support"
      ...> )
      iex> {:ok, response} = SingularityLLM.Providers.Codex.poll_task(task_id)
  ...
  3. SingularityLLM loads and uses these same credentials
  ...
  def create_task(opts) when is_list(opts) do
    alias SingularityLLM.Providers.Codex.TaskClient
  ...
  def extract_response(response) when is_map(response) do
    alias SingularityLLM.Providers.Codex.ResponseExtractor
