defmodule SingularityLLM.Providers.Ollama do
  @moduledoc """
  Ollama API adapter for SingularityLLM - provides local model inference via Ollama server.
  ...
  ### Using Environment Variables
      ...
      SingularityLLM.Providers.Ollama.chat(messages, config_provider: SingularityLLM.Infrastructure.ConfigProvider.Env)
  ...
  ### Using Static Configuration
      ...
      SingularityLLM.Providers.Ollama.chat(messages, config_provider: provider)
  ...
  ## Example Usage
      ...
      # Simple chat
      {:ok, response} = SingularityLLM.Providers.Ollama.chat(messages)
      IO.puts(response.content)

      # Streaming chat
      {:ok, stream} = SingularityLLM.Providers.Ollama.stream_chat(messages)
      for chunk <- stream do
  ...
  To see available models, ensure Ollama is running and use:
      ...
      {:ok, models} = SingularityLLM.Providers.Ollama.list_models()
  ...
  The adapter provides functions to generate and update model configurations:
      ...
      {:ok, yaml} = SingularityLLM.Providers.Ollama.generate_config()
      
      # Save the configuration
      {:ok, path} = SingularityLLM.Providers.Ollama.generate_config(save: true)
      
      # Update a specific model's configuration
      {:ok, yaml} = SingularityLLM.Providers.Ollama.update_model_config("llama3.1")
  ...
  alias SingularityLLM.Providers.Shared.{ConfigHelper, EnhancedStreamingCoordinator}
  alias SingularityLLM.Providers.Shared.HTTP.Core
  alias SingularityLLM.{Infrastructure.Logger, Types}
  ...
  def chat(messages, options \\ []) do
    ...
    case Tesla.post(client, "/api/chat", body, opts: [timeout: timeout]) do
      ...
      {:ok, %Tesla.Env{status: status, body: error_body}} ->
        SingularityLLM.Infrastructure.Error.api_error(status, error_body)

      {:error, reason} ->
        SingularityLLM.Infrastructure.Error.connection_error(reason)
    end
  end
  ...
  def embeddings(inputs, options \\ []) do
    ...
    case Req.post(url, json: body, headers: headers, receive_timeout: timeout) do
      ...
      {:ok, %{status: status, body: body}} ->
        SingularityLLM.Infrastructure.Error.api_error(status, body)
  ...
  ## Examples
      ...
      # Simple completion
      {:ok, response} = SingularityLLM.Providers.Ollama.generate("Complete this: The sky is", 
        model: "llama3.1")
      
      # With options
      {:ok, response} = SingularityLLM.Providers.Ollama.generate("Write a haiku about coding",
        model: "llama3.1",
        options: %{temperature: 0.7, seed: 42})
      
      # Maintain conversation context
      {:ok, response1} = SingularityLLM.Providers.Ollama.generate("Hi, I'm learning Elixir",
        model: "llama3.1")
      {:ok, response2} = SingularityLLM.Providers.Ollama.generate("What should I learn first?",
        model: "llama3.1",
        ...
        {:ok, response} ->
          if response.status == 200 do
            ...
            send(
              parent,
              {:stream_error,
               SingularityLLM.Infrastructure.Error.api_error(response.status, response.body)}
            )
          end

        {:error, reason} ->
          Logger.error("Ollama connection error: #{inspect(reason)}")
          send(parent, {:stream_error, SingularityLLM.Infrastructure.Error.connection_error(reason)})
      end
    end)
  ...
  ## Examples
      ...
      {:ok, info} = SingularityLLM.Providers.Ollama.show_model("llama3.1")
  """
  ...
  ## Examples
      ...
      {:ok, _} = SingularityLLM.Providers.Ollama.copy_model("llama3.1", "my-llama")
  """
  ...
  ## Examples
      ...
      {:ok, _} = SingularityLLM.Providers.Ollama.delete_model("old-model")
  """
  ...
  ## Examples
      ...
      {:ok, stream} = SingularityLLM.Providers.Ollama.pull_model("llama3.1:latest")
  ...
  ## Examples
      ...
      {:ok, stream} = SingularityLLM.Providers.Ollama.push_model("my-model:latest")
  """
  ...
  ## Examples
      ...
      {:ok, loaded} = SingularityLLM.Providers.Ollama.list_running_models()
  """
  ...
  ## Examples
      ...
      {:ok, version} = SingularityLLM.Providers.Ollama.version()
  """
  ...
  ## Examples
      ...
      {:ok, path} = SingularityLLM.Providers.Ollama.generate_config(
        save: true, 
        path: "my_ollama_config.yml"
      )
      
      # Replace existing configuration instead of merging
      {:ok, yaml} = SingularityLLM.Providers.Ollama.generate_config(merge: false)
  """
  ...
  ## Examples
      ...
      {:ok, yaml} = SingularityLLM.Providers.Ollama.update_model_config("llama3.1", save: false)
  """
