defmodule SingularityLLM.Providers.Gemini.Content do
  @moduledoc """
  ...
  alias SingularityLLM.Providers.Gemini.Base
  ...
  defmodule Part do
    ...
    defimpl Jason.Encoder, for: __MODULE__ do
      def encode(part, opts) do
        part
        |> SingularityLLM.Providers.Gemini.Content.Part.to_json()
  ...
  defmodule Content do
    ...
    defimpl Jason.Encoder, for: __MODULE__ do
      def encode(content, opts) do
        content
        |> SingularityLLM.Providers.Gemini.Content.content_to_json()
        |> Jason.Encoder.encode(opts)
      end
    end
  end
  ...
  defmodule GenerateContentRequest do
    ...
    defimpl Jason.Encoder, for: __MODULE__ do
      def encode(request, opts) do
        request
        |> SingularityLLM.Providers.Gemini.Content.GenerateContentRequest.to_json()
        |> Jason.Encoder.encode(opts)
      end
    end
    ...
    def to_json(%__MODULE__{} = request) do
      json = %{
        "contents" =>
          Enum.map(request.contents, &SingularityLLM.Providers.Gemini.Content.content_to_json/1)
      }
        ...
        if request.system_instruction,
          do:
            Map.put(
              json,
              "systemInstruction",
              SingularityLLM.Providers.Gemini.Content.content_to_json(request.system_instruction)
            ),
          else: json
  ...
  ## Examples
      ...
      {:ok, response} = SingularityLLM.Providers.Gemini.Content.generate_content("gemini-2.0-flash", request)
  """
