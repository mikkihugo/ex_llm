defmodule SingularityLLM.Providers.Gemini.Auth do
  @moduledoc """
  ...
  ## Usage
      ...
      {:ok, token} = SingularityLLM.Providers.Gemini.Auth.get_service_account_token()
      
      # Option 2: User OAuth2 flow
      {:ok, auth_url} = SingularityLLM.Providers.Gemini.Auth.get_authorization_url()
      # ... user authorizes ...
      {:ok, tokens} = SingularityLLM.Providers.Gemini.Auth.exchange_code(auth_code)
      
      # Use with Permissions API
      SingularityLLM.Providers.Gemini.Permissions.list_permissions("tunedModels/my-model",
        oauth_token: token
      )
  """
  ...
  * `:state` - Optional state parameter for CSRF protection
  ...
  ## Examples
      ...
      {:ok, url} = SingularityLLM.Providers.Gemini.Auth.get_authorization_url(
  ...
  * `opts` - Options including client credentials
  ...
  ## Examples
      ...
      {:ok, new_token} = SingularityLLM.Providers.Gemini.Auth.refresh_token(stored_refresh_token)
  """
  ...
  * `opts` - Options (currently unused)
  ...
  ## Examples
      ...
      :ok = SingularityLLM.Providers.Gemini.Auth.revoke_token(access_token)
  """
  ...
  * `:scopes` - List of OAuth2 scopes
  ...
  ## Examples
      ...
      {:ok, tokens} = SingularityLLM.Providers.Gemini.Auth.cli_flow(
        client_id: "your-client-id",
        client_secret: "your-client-secret"
      )
  """
