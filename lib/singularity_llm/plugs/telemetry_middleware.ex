defmodule SingularityLLM.Plugs.TelemetryMiddleware do
  @moduledoc """
  ...
  - `:event_name` (optional) - A list of atoms for the telemetry event prefix.
    Defaults to `[:singularity_llm, :pipeline, :execution]`.
  ...
  This plug is used to wrap other plugs in a pipeline definition.
      ...
      pipeline = [
        {SingularityLLM.Plugs.TelemetryMiddleware, %{
          event_name: [:singularity_llm, :chat],
          pipeline: [
            {SingularityLLM.Plugs.ValidateProvider, []},
            {SingularityLLM.Plugs.PrepareRequest, []},
            {SingularityLLM.Plugs.ExecuteRequest, []}
          ]
        }}
      ]

      SingularityLLM.Pipeline.run(initial_request, pipeline)

  This setup will emit telemetry events like `[:singularity_llm, :chat, :start]`,
  `[:singularity_llm, :chat, :stop]`, and `[:singularity_llm, :chat, :exception]`.
  """

  use SingularityLLM.Plug
  alias SingularityLLM.Infrastructure.Telemetry
  ...
  def init(opts) when is_list(opts) do
    ...
    event_name = Keyword.get(opts, :event_name, [:singularity_llm, :pipeline, :execution])
  ...
  def init(opts) when is_map(opts) do
    ...
    event_name = Map.get(opts, :event_name, [:singularity_llm, :pipeline, :execution])
  ...
  # be centralized in a dedicated `SingularityLLM.Pipeline` module. It is included
