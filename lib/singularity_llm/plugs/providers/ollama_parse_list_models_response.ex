defmodule SingularityLLM.Plugs.Providers.OllamaParseListModelsResponse do
  @moduledoc """
  ...
  @impl true
  def call(%SingularityLLM.Pipeline.Request{response: %Tesla.Env{} = response} = request, _opts) do
  ...
  defp parse_not_found_response(request, _response) do
    ...
    SingularityLLM.Pipeline.Request.halt_with_error(request, error_data)
  end
  ...
  defp get_model_config(model_name, base_name) do
    ...
    case SingularityLLM.Infrastructure.OllamaModelRegistry.get_model_details(model_name) do
      {:ok, details} ->
        details
