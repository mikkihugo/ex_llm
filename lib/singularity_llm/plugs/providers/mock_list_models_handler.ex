defmodule SingularityLLM.Plugs.Providers.MockListModelsHandler do
  @moduledoc """
  ...
  def call(%SingularityLLM.Pipeline.Request{} = request, opts) do
    ...
    if error = get_mock_error(request.config, opts) do
      ...
      |> SingularityLLM.Pipeline.Request.assign(:mock_list_models_handler_called, true)
    end
  end

  defp get_mock_error(config, opts) do
    config[:mock_error] || opts[:error] ||
      Application.get_env(:singularity_llm, :mock_list_models_error)
  end
  ...
  defp get_mock_models(config, opts) do
    ...
    custom_models =
      config[:models] ||
        opts[:models] ||
        Application.get_env(:singularity_llm, :mock_models)
