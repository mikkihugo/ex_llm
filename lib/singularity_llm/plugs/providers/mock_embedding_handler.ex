defmodule SingularityLLM.Plugs.Providers.MockEmbeddingHandler do
  @moduledoc """
  ...
  alias SingularityLLM.Types.EmbeddingResponse
    ...
    if error = get_mock_error(request.config, opts) do
      ...
      case Application.get_env(:singularity_llm, :mock_responses, %{})[:embeddings] do
        %EmbeddingResponse{} = response ->
          ...
          |> SingularityLLM.Pipeline.Request.assign(:mock_embedding_handler_called, true)
  ...
  defp get_input_from_request(request) do
    cond do
      ...
      app_input = Application.get_env(:singularity_llm, :mock_embedding_input) ->
        app_input
  ...
  defp get_mock_error(config, opts) do
    ...
    error =
      config[:mock_error] || opts[:error] ||
        Application.get_env(:singularity_llm, :mock_embedding_error)
