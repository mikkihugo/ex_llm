defmodule SingularityLLM.Plugs.Providers.MockHandler do
  @moduledoc """
  ...
  defp handle_streaming_request(request, messages, config, opts) do
    ...
    if error_to_simulate do
      ...
      # The callback will be triggered when SingularityLLM.stream consumes the stream
  ...
  defp get_mock_agent_response(messages, config) do
    try do
      ...
      case Process.whereis(SingularityLLM.Providers.Mock) do
        nil ->
          {:error, :agent_not_running}

        _pid ->
          # Use the Mock agent's chat function to get the response
          case SingularityLLM.Providers.Mock.chat(messages, Enum.to_list(config)) do
            {:ok, response} ->
    ...
    try do
      case Process.whereis(SingularityLLM.Providers.Mock) do
        ...
        _pid ->
          ...
          case SingularityLLM.Providers.Mock.stream_chat(messages, Enum.to_list(config)) do
            {:ok, stream} -> {:ok, stream}
            {:error, _} = error -> error
          end
      end
    ...
    try do
      case Process.whereis(SingularityLLM.Providers.Mock) do
        ...
        _pid ->
          ...
          Agent.get(SingularityLLM.Providers.Mock, fn state ->
  ...
  defp normalize_app_response(app_response, _messages, config) do
    case app_response do
      %SingularityLLM.Types.LLMResponse{} = response ->
