defmodule SingularityLLM.Plugs.ExecuteRequest do
  @moduledoc """
  ...
  ## Examples
      ...
      plug SingularityLLM.Plugs.ExecuteRequest, endpoint: "/v1/completions"
  """

  use SingularityLLM.Plug
  alias SingularityLLM.Infrastructure.Logger
  alias SingularityLLM.Pipeline.Request
  alias SingularityLLM.Testing.TestResponseInterceptor, as: TestResponseInterceptor
  ...
  defp handle_tesla_response(%Request{} = request, %Tesla.Env{} = response) do
    if SingularityLLM.HTTP.successful?(response) do
      # Handle successful Tesla responses
      status = SingularityLLM.HTTP.get_status(response)
      headers = SingularityLLM.HTTP.get_headers(response)
      body = SingularityLLM.HTTP.get_body(response)
