defmodule SingularityLLM.Session do
  @moduledoc """
  Session management functionality for SingularityLLM.
  ...
  ## Examples
      ...
      session = SingularityLLM.Session.add_message(session, "user", "Hello!")
      session = SingularityLLM.Session.add_message(session, "assistant", "Hi there!")
      
      # Chat with session (automatically manages context)
      {:ok, response, updated_session} = SingularityLLM.Session.chat_session(session, "How are you?")
      
      # Save and load sessions
      SingularityLLM.Session.save_session(session, "conversation.json")
      loaded_session = SingularityLLM.Session.load_session("conversation.json")
  """

  alias SingularityLLM.Core.Session, as: CoreSession
  alias SingularityLLM.Types.Session
  ...
  ## Examples
      ...
      session = SingularityLLM.Session.add_message(session, "user", "What is the weather like?")
      session = SingularityLLM.Session.add_message(session, "assistant", "I don't have access to current weather data.")
  ...
  ## Examples
      ...
      messages = SingularityLLM.Session.get_messages(session)
      # => [
  ...
  ## Examples
      ...
      # Get all messages
      all_messages = SingularityLLM.Session.get_session_messages(session)
      
      # Get last 5 messages
      recent_messages = SingularityLLM.Session.get_session_messages(session, 5)
  ...
  ## Examples
      ...
      session = SingularityLLM.Session.add_session_message(session, "user", "Hello!")
  ...
  ## Examples
      ...
      total_tokens = SingularityLLM.Session.session_token_usage(session)
      # => 1250
  ...
  ## Examples
      ...
      cleared_session = SingularityLLM.Session.clear_session(session)
  ...
  ## Examples
      ...
      :ok = SingularityLLM.Session.save_session(session, "my_conversation.json")
  ...
  ## Examples
      ...
      json_string = SingularityLLM.Session.save_session(session)
  ...
  ## Examples
      ...
      # Load from file
      session = SingularityLLM.Session.load_session("my_conversation.json")
      
      # Load from JSON string
      json = ~s({"provider": "openai", "messages": []})
      session = SingularityLLM.Session.load_session(json)
  ...
  ## Parameters
    ...
    * `opts` - Chat options (same as `SingularityLLM.chat/3`)

  ## Examples

      {:ok, response, updated_session} = SingularityLLM.Session.chat_session(session, 
  ...
  def chat_session(session, user_message, opts \\ []) do
    ...
    case SingularityLLM.chat(session.llm_backend, messages, merged_opts) do
      {:ok, response} ->
        ...
        final_session =
          if response.usage do
            SingularityLLM.Core.Session.update_token_usage(session_with_message, response.usage)
          else
  ...
  ## Examples
      ...
      {:ok, {response, updated_session}} = SingularityLLM.Session.chat_with_session(session, 
        "Tell me a joke")
