defmodule SingularityLLM.ChatBuilder do
  @moduledoc """
  Enhanced builder-style API for SingularityLLM chat operations.
  ...
  ## Basic Usage
      ...
      {:ok, response} = 
        ...
        |> SingularityLLM.ChatBuilder.with_model("gpt-4-turbo")
        |> SingularityLLM.ChatBuilder.with_temperature(0.7)
  ...
  ## Advanced Pipeline Customization
      ...
      {:ok, response} = 
        SingularityLLM.chat(:openai, messages)
        |> SingularityLLM.ChatBuilder.with_cache(ttl: 3600)
        |> SingularityLLM.ChatBuilder.with_custom_plug(MyApp.Plugs.Logger)
        |> SingularityLLM.ChatBuilder.without_cost_tracking()
        |> SingularityLLM.ChatBuilder.execute()
  ...
  ## Streaming
      ...
      |> SingularityLLM.ChatBuilder.stream(fn chunk ->
  ...
  This is typically called by SingularityLLM.chat/2, but can be used directly:

      builder = SingularityLLM.ChatBuilder.new(:openai, messages)
  """
  ...
  def without_cost_tracking(%__MODULE__{} = builder) do
    add_pipeline_mod(builder, {:remove, SingularityLLM.Plugs.TrackCost})
  end
  ...
  ## Examples
      ...
      builder |> insert_before(SingularityLLM.Plugs.ExecuteRequest, MyApp.Plugs.RequestModifier)
  """
  ...
  ## Examples
      ...
      builder |> insert_after(SingularityLLM.Plugs.FetchConfig, MyApp.Plugs.ConfigValidator)
  """
  ...
  ## Examples
      ...
      builder |> replace_plug(SingularityLLM.Plugs.Cache, MyApp.Plugs.CustomCache, ttl: 7200)
  """
  ...
  def with_context_strategy(%__MODULE__{} = builder, strategy, opts \\ []) do
    ...
    add_pipeline_mod(builder, {:replace, SingularityLLM.Plugs.ManageContext, context_opts})
  end
