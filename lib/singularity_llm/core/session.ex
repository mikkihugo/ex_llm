defmodule SingularityLLM.Core.Session do
  @moduledoc """
  Session management for SingularityLLM - handles conversation sessions with LLM providers.

  SingularityLLM.Session provides pure functional operations for managing conversation sessions,
  ...
  ## Quick Start
      ...
      session = SingularityLLM.Session.new("anthropic")
      ...
      # Update token usage
      session = SingularityLLM.Session.update_token_usage(session, %{input_tokens: 10, output_tokens: 15})
  ...
  - **LLM Integration**: Seamlessly works with SingularityLLM providers

  ## Session Structure

  Sessions are represented by `SingularityLLM.Types.Session` structs containing:
  ...
  ## Examples
      ...
      session = SingularityLLM.Session.new("anthropic")
      session = SingularityLLM.Session.new("openai", name: "My Chat")
  """
  ...
  ## Examples
      ...
      session = SingularityLLM.Session.add_message(session, "user", "Hello!")
      session = SingularityLLM.Session.add_message(session, "assistant", "Hi there!", timestamp: DateTime.utc_now())
  """
  ...
  ## Examples
      ...
      all_messages = SingularityLLM.Session.get_messages(session)
      last_5 = SingularityLLM.Session.get_messages(session, 5)
  """
  ...
  ## Examples
      ...
      session = SingularityLLM.Session.update_token_usage(session, %{input_tokens: 100, output_tokens: 150})
  """
  ...
  ## Examples
      ...
      session = SingularityLLM.Session.set_context(session, %{temperature: 0.7, max_tokens: 1000})
  """
  ...
  ## Examples
      ...
      session = SingularityLLM.Session.clear_messages(session)
  """
  ...
  ## Examples
      ...
      session = SingularityLLM.Session.set_name(session, "My Important Chat")
  """
  ...
  ## Examples
      ...
      total = SingularityLLM.Session.total_tokens(session)
  """
  ...
  ## Examples
      ...
      {:ok, json} = SingularityLLM.Session.to_json(session)
  """
  ...
  ## Examples
      ...
      {:ok, session} = SingularityLLM.Session.from_json(json_string)
  """
  ...
  ## Examples
      ...
      SingularityLLM.Session.save_to_file(session, "my_session.json")
  """
  ...
  ## Examples
      ...
      {:ok, session} = SingularityLLM.Session.load_from_file("my_session.json")
  """
