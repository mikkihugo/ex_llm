defmodule SingularityLLM.Core.Capabilities do
  @moduledoc """
  ...
  ## Examples
      ...
      SingularityLLM.Core.Capabilities.supports?(:openai, :function_calling)
      # => true
      
      # Works with provider-specific names too
      SingularityLLM.Core.Capabilities.supports?(:anthropic, :tool_use)  # normalized to :function_calling
      # => true
      
      # Check if a specific model supports a capability
      SingularityLLM.Core.Capabilities.model_supports?(:openai, "gpt-4o", :vision)
      # => true
      
      # Find all providers that support a capability
      SingularityLLM.Core.Capabilities.find_providers(:image_generation)
      # => [:openai]
  """

  alias SingularityLLM.Infrastructure.Config.{ModelCapabilities, ProviderCapabilities}
