defmodule SingularityLLM.Core.Chat do
  @moduledoc """
  Core chat functionality for SingularityLLM.

  This module contains the primary chat operations that power SingularityLLM's unified
  interface. It handles regular and streaming chat requests, with automatic
  context management, retry logic, and cost tracking.

  ## Usage

  While this module can be used directly, most users should use the main SingularityLLM
  module which delegates to these functions.

      # Direct usage
      {:ok, response} = SingularityLLM.Core.Chat.chat(:anthropic, messages, options)
  ...
  @type provider :: SingularityLLM.Types.provider()
  ...
  ## Options
  See `SingularityLLM.chat/3` for complete options documentation.

  ## Returns
  `{:ok, %SingularityLLM.Types.LLMResponse{}}` on success, or `{:ok, struct}` when using
  ...
  def chat(provider_or_model, messages, options \\ []) do
    ...
    SingularityLLM.Infrastructure.Telemetry.span([:singularity_llm, :chat], metadata, fn ->
  ...
  ## Returns
  `{:ok, stream}` on success where stream yields `%SingularityLLM.Types.StreamChunk{}` structs,
  ...
  def stream_chat(provider_or_model, messages, options \\ []) do
    ...
    SingularityLLM.Infrastructure.Telemetry.emit_stream_start(provider, metadata.model)
  ...
  defp handle_structured_output(provider, messages, options) do
    ...
    if Code.ensure_loaded?(SingularityLLM.Core.StructuredOutputs) and
         SingularityLLM.Core.StructuredOutputs.available?() do
      SingularityLLM.Core.StructuredOutputs.chat(provider, messages, options)
    else
  ...
  defp execute_with_pipeline_retry(pipeline, provider, prepared_messages, options) do
    ...
    if Keyword.get(options, :retry, true) do
      SingularityLLM.Infrastructure.Retry.with_provider_retry(
        provider,
        fn ->
  ...
  defp build_pipeline_for_provider(provider) do
    ...
    provider_plugs = %{
      openai: [
        build_request: {SingularityLLM.Plugs.Providers.OpenAIPrepareRequest, []},
        parse_response: {SingularityLLM.Providers.OpenAI.ParseResponse, []},
        stream_parse_response: {SingularityLLM.Plugs.Providers.OpenAIParseStreamResponse, []}
      ],
      anthropic: [
        build_request: {SingularityLLM.Plugs.Providers.AnthropicPrepareRequest, []},
        parse_response: {SingularityLLM.Providers.Anthropic.ParseResponse, []},
        stream_parse_response: {SingularityLLM.Plugs.Providers.AnthropicParseStreamResponse, []}
      ],
      groq: [
        build_request: {SingularityLLM.Providers.Groq.BuildRequest, []},
        parse_response: {SingularityLLM.Providers.Groq.ParseResponse, []},
        stream_parse_response: {SingularityLLM.Plugs.Providers.GroqParseStreamResponse, []}
      ],
      lmstudio: [
        build_request: {SingularityLLM.Providers.LMStudio.BuildRequest, []},
        parse_response: {SingularityLLM.Providers.LMStudio.ParseResponse, []},
        stream_parse_response: {SingularityLLM.Plugs.Providers.LMStudioParseStreamResponse, []}
      ],
      xai: [
        build_request: {SingularityLLM.Providers.XAI.BuildRequest, []},
        parse_response: {SingularityLLM.Providers.XAI.ParseResponse, []},
        stream_parse_response: {SingularityLLM.Plugs.Providers.XAIParseStreamResponse, []}
      ],
      mistral: [
        build_request: {SingularityLLM.Providers.Mistral.BuildRequest, []},
        parse_response: {SingularityLLM.Providers.Mistral.ParseResponse, []},
        stream_parse_response: {SingularityLLM.Plugs.Providers.MistralParseStreamResponse, []}
      ],
      ollama: [
        build_request: {SingularityLLM.Providers.Ollama.BuildRequest, []},
        parse_response: {SingularityLLM.Providers.Ollama.ParseResponse, []},
        stream_parse_response: {SingularityLLM.Plugs.Providers.OllamaParseStreamResponse, []}
      ],
      openrouter: [
        build_request: {SingularityLLM.Providers.OpenRouter.BuildRequest, []},
        parse_response: {SingularityLLM.Providers.OpenRouter.ParseResponse, []},
        stream_parse_response: {SingularityLLM.Plugs.Providers.OpenRouterParseStreamResponse, []}
      ],
      perplexity: [
        build_request: {SingularityLLM.Providers.Perplexity.BuildRequest, []},
        parse_response: {SingularityLLM.Providers.Perplexity.ParseResponse, []},
        stream_parse_response: {SingularityLLM.Plugs.Providers.PerplexityParseStreamResponse, []}
      ],
      gemini: [
        build_request: {SingularityLLM.Providers.Gemini.BuildRequest, []},
        parse_response: {SingularityLLM.Providers.Gemini.ParseResponse, []},
        stream_parse_response: {SingularityLLM.Plugs.Providers.GeminiParseStreamResponse, []}
      ],
      bumblebee: [
        build_request: {SingularityLLM.Providers.Bumblebee.BuildRequest, []},
        execute_request: {SingularityLLM.Plugs.ExecuteLocal, []},
        parse_response: {SingularityLLM.Providers.Bumblebee.ParseResponse, []},
        stream_parse_response: {SingularityLLM.Plugs.Providers.BumblebeeParseStreamResponse, []}
      ],
      bedrock: [
        build_request: {SingularityLLM.Providers.Bedrock.BuildRequest, []},
        auth_request: {SingularityLLM.Plugs.AWSAuth, []},
        parse_response: {SingularityLLM.Providers.Bedrock.ParseResponse, []},
        stream_parse_response: {SingularityLLM.Plugs.Providers.BedrockParseStreamResponse, []}
      ],
      mock: [
        build_request: {SingularityLLM.Plugs.Providers.MockHandler, []},
        execute_request: {SingularityLLM.Plugs.Providers.MockHandler, []},
        parse_response: {SingularityLLM.Plugs.Providers.MockHandler, []},
        stream_parse_response: {SingularityLLM.Plugs.Providers.MockHandler, []}
      ]
    }
  ...
  defp prepare_messages_for_provider(provider, messages, options) do
    ...
    model = Keyword.get(options, :model) || SingularityLLM.default_model(provider)
  ...
  defp emit_cost_telemetry(provider, model, total_cost) do
    SingularityLLM.Infrastructure.Telemetry.emit_cost_calculated(
      provider,
      model,
      # Convert to cents
  ...
  defp execute_stream_with_pipeline_recovery(pipeline, provider, messages, options) do
    ...
    case SingularityLLM.Infrastructure.Streaming.StreamRecovery.start_stream(
           stream_fn,
           callback,
           recovery_opts
         ) do
      {:ok, recovery_pid} ->
        ...
        stream =
          Stream.resource(
            ...
            fn pid -> SingularityLLM.Infrastructure.Streaming.StreamRecovery.stop_stream(pid) end
          )
  ...
  defp wrap_stream_with_telemetry(stream, provider, model, start_time) do
    ...
    Stream.transform(stream, nil, fn chunk, _acc ->
      ...
      SingularityLLM.Infrastructure.Telemetry.emit_stream_chunk(provider, model, chunk_size)
      ...
      if chunk.finish_reason == "stop" do
        ...
        SingularityLLM.Infrastructure.Telemetry.emit_stream_complete(
          provider,
          model,
          total_chunks,
          duration
  ...
  defp detect_provider(provider_or_model, options) when is_binary(provider_or_model) do
    ...
    providers = %{
      anthropic: SingularityLLM.Providers.Anthropic,
      bumblebee: SingularityLLM.Providers.Bumblebee,
      groq: SingularityLLM.Providers.Groq,
      lmstudio: SingularityLLM.Providers.LMStudio,
      mistral: SingularityLLM.Providers.Mistral,
      openai: SingularityLLM.Providers.OpenAI,
      openrouter: SingularityLLM.Providers.OpenRouter,
      ollama: SingularityLLM.Providers.Ollama,
      perplexity: SingularityLLM.Providers.Perplexity,
      bedrock: SingularityLLM.Providers.Bedrock,
      gemini: SingularityLLM.Providers.Gemini,
      xai: SingularityLLM.Providers.XAI,
      mock: SingularityLLM.Providers.Mock
    }
