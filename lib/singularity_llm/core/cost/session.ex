defmodule SingularityLLM.Core.Cost.Session do
  @moduledoc """
  ...
  ## Usage
      ...
      # Start a new session
      session = SingularityLLM.Core.Cost.Session.new("chat_session_1")

      # Add responses to track costs
      session = session
        |> SingularityLLM.Core.Cost.Session.add_response(response1)
        |> SingularityLLM.Core.Cost.Session.add_response(response2)

      # Get session summary
      summary = SingularityLLM.Core.Cost.Session.get_summary(session)

      # Format for display
      formatted = SingularityLLM.Core.Cost.Session.format_summary(session, format: :detailed)
  ...
  ## Examples
      ...
      iex> session = SingularityLLM.Core.Cost.Session.new("chat_session_1")
  ...
  The response should contain cost information (typically from `SingularityLLM.Core.Cost.calculate/3`)
  ...
  ## Examples
      ...
      session = session |> SingularityLLM.Core.Cost.Session.add_response(response)
  """
  ...
  ## Examples
      ...
      # Detailed format
      SingularityLLM.Core.Cost.Session.format_summary(session)

      # Compact format
      SingularityLLM.Core.Cost.Session.format_summary(session, format: :compact)

      # Table format
      SingularityLLM.Core.Cost.Session.format_summary(session, format: :table)
  """
  ...
  ## Examples
      ...
      breakdown = SingularityLLM.Core.Cost.Session.provider_breakdown(session)
  ...
  ## Examples
      ...
      breakdown = SingularityLLM.Core.Cost.Session.model_breakdown(session)
  ...
  defp format_detailed_summary(summary) do
    ...
    Total Cost: #{SingularityLLM.Core.Cost.format(summary.total_cost)}
    ...
    Avg Cost/Message: #{SingularityLLM.Core.Cost.format(summary.average_cost_per_message)}
    Cost/1K Tokens: #{SingularityLLM.Core.Cost.format(summary.cost_per_1k_tokens)}
  ...
  defp format_compact_summary(summary) do
    "💰 #{SingularityLLM.Core.Cost.format(summary.total_cost, style: :compact)} " <>
  ...
  defp format_model_breakdown(model_breakdown) do
    breakdown_text =
      ...
      |> Enum.take(5)
      |> Enum.map(fn {model, stats} ->
        "  ├─ #{model}: #{SingularityLLM.Core.Cost.format(stats.total_cost)} " <>
