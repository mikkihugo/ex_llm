defmodule SingularityLLM.Core.Cost.Display do
  @moduledoc """
  ...
  CLI displays. It's designed to work with SingularityLLM.Cost and SingularityLLM.Core.Cost.Session
  data structures.
  ...
  ## Usage
      ...
      table = SingularityLLM.Core.Cost.Display.cost_breakdown_table(cost_data, format: :ascii)

      # CLI-friendly session summary
      summary = SingularityLLM.Core.Cost.Display.cli_summary(session_summary)

      # Real-time streaming cost display
      display = SingularityLLM.Core.Cost.Display.streaming_cost_display(0.05, 0.12)

      # Cost alerts
      alert = SingularityLLM.Core.Cost.Display.cost_alert(:budget_exceeded, %{current: 1.25, budget: 1.00})
  """
  ...
  ## Examples
      ...
      # ASCII table
      SingularityLLM.Core.Cost.Display.cost_breakdown_table(cost_data)

      # Markdown table
      SingularityLLM.Core.Cost.Display.cost_breakdown_table(cost_data, format: :markdown)
  ...
  ## Examples
      ...
      summary_text = SingularityLLM.Core.Cost.Display.cli_summary(session_summary)
  ...
  def cli_summary(session_summary) do
    ...
    Total Cost: #{SingularityLLM.Core.Cost.format(session_summary.total_cost)}
    ...
    Avg Cost/Message: #{SingularityLLM.Core.Cost.format(session_summary.average_cost_per_message)}
    Cost/1K Tokens: #{SingularityLLM.Core.Cost.format(session_summary.cost_per_1k_tokens)}
  ...
  ## Examples
      ...
      # With custom format
      display = SingularityLLM.Core.Cost.Display.streaming_cost_display(0.05, 0.12, style: :compact)
      # => "💰 $0.050 (42%)"
  """
  ...
  def streaming_cost_display(current_cost, estimated_final_cost, opts \\ []) do
    ...
    case style do
      :compact ->
        "💰 #{SingularityLLM.Core.Cost.format(current_cost, style: :compact)} (#{format_percentage(progress, 0)}%)"

      :detailed ->
        "💰 #{SingularityLLM.Core.Cost.format(current_cost)} (#{format_percentage(progress, 1)}% of estimated #{SingularityLLM.Core.Cost.format(estimated_final_cost)})"

      _ ->
        "💰 #{SingularityLLM.Core.Cost.format(current_cost)} (#{format_percentage(progress, 1)}%)"
    end
  end
  ...
  ## Examples
      ...
      alert = SingularityLLM.Core.Cost.Display.cost_alert(:budget_exceeded, %{
        current: 1.25, 
        budget: 1.00,
        session_id: "chat_123"
      })
      ...
      # High cost warning
      alert = SingularityLLM.Core.Cost.Display.cost_alert(:high_cost_warning, %{
        cost: 0.75,
        model: "gpt-4",
        threshold: 0.50
      })
  ...
  def cost_alert(alert_type, data) do
    case alert_type do
      :budget_exceeded ->
        ...
        "🚨 Budget exceeded!#{session_info} Current: #{SingularityLLM.Core.Cost.format(data.current)}, Budget: #{SingularityLLM.Core.Cost.format(data.budget)}"

      :high_cost_warning ->
        model_info = if data[:model], do: " using #{data.model}", else: ""
        "⚠️  High cost detected: #{SingularityLLM.Core.Cost.format(data.cost)}#{model_info}"
      ...
      :provider_recommended ->
        ...
        "💡 Consider #{data.recommended_provider} for #{SingularityLLM.Core.Cost.format(savings)} potential savings"

      :session_complete ->
        "✅ Session complete. Total cost: #{SingularityLLM.Core.Cost.format(data.total_cost)}"
  ...
  ## Examples
      ...
      comparison = SingularityLLM.Core.Cost.Display.comparison_table([
    ...
    ranked_comparisons =
      ...
      |> Enum.map(fn {comparison, rank} ->
        ...
        |> Map.put(
          :savings,
          if(savings > 0,
            do: "+#{SingularityLLM.Core.Cost.format(savings)} (+#{format_percentage(savings_pct, 0)}%)",
            else: "Cheapest"
          )
        )
      end)
  ...
  defp format_cell_value(value) when is_float(value) do
    if value < 0.01 do
      SingularityLLM.Core.Cost.format(value)
    else
      SingularityLLM.Core.Cost.format(value, style: :compact)
    end
  end
  ...
  defp format_provider_breakdown(provider_breakdown) do
    breakdown_text =
      ...
      |> Enum.map(fn {provider, stats} ->
        "  ├─ #{String.capitalize(provider)}: #{SingularityLLM.Core.Cost.format(stats.total_cost)} " <>
