defmodule SingularityLLM.Core.StructuredOutputs do
  @moduledoc """
  Structured output support for SingularityLLM using instructor_ex.

  This module provides integration with the `instructor` library to enable
  structured outputs with validation when using SingularityLLM. It allows you to
  ...
  ### Basic Example
      ...
      {:ok, result} = SingularityLLM.Core.StructuredOutputs.chat(:anthropic, [
  ...
  ### With Retries
      ...
      {:ok, result} = SingularityLLM.Core.StructuredOutputs.chat(:anthropic, messages,
  ...
  ## Integration with SingularityLLM

  The structured output functionality is also available through the main
  SingularityLLM module when instructor is installed:
  ...
  ## Examples
      ...
      if SingularityLLM.Core.StructuredOutputs.available?() do
  ...
  - All standard SingularityLLM.chat/3 options
  ...
  ## Examples
      ...
      # With Ecto schema
      {:ok, classification} = SingularityLLM.Core.StructuredOutputs.chat(:anthropic, messages,
        response_model: EmailClassification,
        max_retries: 3
      )

      # With simple type spec
      {:ok, data} = SingularityLLM.Core.StructuredOutputs.chat(:openai, messages,
        response_model: %{name: :string, age: :integer}
      )
  """
  @spec chat(SingularityLLM.Types.provider(), SingularityLLM.Types.message(), keyword()) ::
  ...
  @doc """
  Transform a regular SingularityLLM response into a structured output.

  This function is useful when you already have a response from SingularityLLM
  ...
  ## Examples
      ...
      {:ok, response} = SingularityLLM.chat(:anthropic, messages)
      {:ok, structured} = SingularityLLM.Core.StructuredOutputs.parse_response(response, UserProfile)
  """
  ...
  defp get_model_for_instructor(provider, merged_opts, options) do
    merged_opts[:model] || Keyword.get(options, :model) || SingularityLLM.default_model(provider)
  end
  ...
  defp do_parse_response(%SingularityLLM.Types.LLMResponse{content: content}, response_model) do
  ...
  defp get_provider_config(provider, options) do
    config_provider =
      Keyword.get(options, :config_provider, SingularityLLM.Infrastructure.ConfigProvider.Env)
  ...
  defp get_model_config(provider, config_provider) do
    config_provider.get(provider, :model) ||
      case SingularityLLM.Infrastructure.Config.ModelConfig.get_default_model(provider) do
        {:ok, model} -> model
        {:error, _} -> nil
      end
  end
  ...
  ## Examples
      ...
      schema = SingularityLLM.Core.StructuredOutputs.simple_schema(%{
        name: :string,
        age: :integer,
        email: :string
      }, [
  ...
  defp handle_mock_structured_output(provider, messages, options, response_model) do
    ...
    case SingularityLLM.chat(provider, messages, chat_opts) do
      {:ok, response} ->
