defmodule SingularityLLM.ContextCache do
  @moduledoc """
  Context Caching functionality for SingularityLLM.
  ...
  ## Examples
      ...
      {:ok, cached_content} = SingularityLLM.ContextCache.create_cached_context(:gemini, content_to_cache)
      ...
      {:ok, response} = SingularityLLM.ContextCache.list_cached_contexts(:gemini)

      # 5. Get a specific cached context
      {:ok, details} = SingularityLLM.ContextCache.get_cached_context(:gemini, cached_content.name)

      # 6. Delete the cached context when no longer needed
      :ok = SingularityLLM.ContextCache.delete_cached_context(:gemini, cached_content.name)
  """
  ...
  ## Examples
      ...
      {:ok, cached} = SingularityLLM.ContextCache.create_cached_context(:gemini, request)
  ...
  ## Examples
      ...
      {:ok, cached} = SingularityLLM.ContextCache.get_cached_context(:gemini, "cachedContents/abc-123")
  ...
  ## Examples
      ...
      {:ok, updated} = SingularityLLM.ContextCache.update_cached_context(:gemini, "cachedContents/abc-123", updates)
  ...
  ## Examples
      ...
      :ok = SingularityLLM.ContextCache.delete_cached_context(:gemini, "cachedContents/abc-123")
  ...
  ## Examples
      ...
      {:ok, %{cached_contents: contents, next_page_token: token}} = SingularityLLM.ContextCache.list_cached_contexts(:gemini)
      
      # With pagination
      {:ok, %{cached_contents: more_contents}} = SingularityLLM.ContextCache.list_cached_contexts(:gemini, 
