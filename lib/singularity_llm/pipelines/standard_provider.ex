defmodule SingularityLLM.Pipelines.StandardProvider do
  @moduledoc """
  ...
  `SingularityLLM.Plugs.TelemetryMiddleware` for instrumentation:

  1. `SingularityLLM.Plugs.ValidateProvider`: Ensures the provider is supported.
  2. `SingularityLLM.Plugs.ValidateMessages`: Validates the format of input messages.
  3. `SingularityLLM.Plugs.FetchConfiguration`: Fetches provider configuration and API keys.
  4. **Provider-Specific `build_request` plug**: Transforms the request into the provider's native format.
  5. **Authentication plug** (optional): Handles provider-specific authentication (e.g., `SingularityLLM.Plugs.AWSAuth` for AWS SigV4).
  6. **Execution plug**: Either `SingularityLLM.Plugs.ExecuteRequest` for HTTP APIs or a custom plug (e.g., `SingularityLLM.Plugs.ExecuteLocal` for local models).
  7. **Provider-Specific `stream_parse_response` plug** (optional): Handles streaming responses if provided.
  8. **Provider-Specific `parse_response` plug**: Parses the response into a standardized `SingularityLLM.Response`.
  ...
  module, passing in their specific plugs.
      ...
      defmodule SingularityLLM.Providers.SomeProvider do
        def chat(messages, opts \\ []) do
          provider_plugs = [
            build_request: {SingularityLLM.Providers.SomeProvider.BuildRequest, []},
            parse_response: {SingularityLLM.Providers.SomeProvider.ParseResponse, []}
          ]

          SingularityLLM.Pipelines.StandardProvider.run(:some_provider, messages, opts, provider_plugs)
        end
      end
  """
  ...
  The final `SingularityLLM.Pipeline.Request` struct after the pipeline has run. The
  ...
  A `t:SingularityLLM.Pipeline.pipeline/0` definition that can be executed by `SingularityLLM.Pipeline.run/2`.
  """
  ...
  def build(provider_plugs) do
    ...
    [
      {Plugs.TelemetryMiddleware,
       %{
         event_name: [:singularity_llm, :provider, :execution],
