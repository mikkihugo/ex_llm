defmodule SingularityLLM.PipelineOptimizer do
  @moduledoc """
  Pipeline optimization utilities for SingularityLLM.
  ...
  ## Usage
      ...
      # Optimize a pipeline
      optimized = SingularityLLM.PipelineOptimizer.optimize(pipeline, [:cache_pipelines, :reorder_plugs])
      
      # Enable global optimizations
      SingularityLLM.PipelineOptimizer.configure(enabled: true, strategies: [:all])
      
      # Benchmark optimization impact
      SingularityLLM.PipelineOptimizer.benchmark_optimization(provider, pipeline, messages)
  """
  ...
  ## Examples
      ...
      SingularityLLM.PipelineOptimizer.configure(enabled: true)
      
      # Enable all safe optimizations
      SingularityLLM.PipelineOptimizer.configure(
        enabled: true,
  ...
  def configure(opts \\ []) do
    ...
    Application.put_env(:singularity_llm, :pipeline_optimizer, config)
  ...
  ## Examples
      ...
      # Basic optimization
      optimized = SingularityLLM.PipelineOptimizer.optimize(pipeline)
      ...
      result = SingularityLLM.PipelineOptimizer.optimize_with_report(pipeline, [:all])
  """
  @spec optimize([SingularityLLM.Pipeline.plug()], [optimization_strategy()] | nil) :: [
          SingularityLLM.Pipeline.plug()
        ]
    ...
    Enum.flat_map(segments, fn segment ->
      case segment do
        {:parallel, plugs} when length(plugs) > 1 ->
          ...
          [{SingularityLLM.Plugs.ParallelExecutor, plugs: plugs}]
  ...
  defp is_validation_plug?(plug_spec) do
    ...
    plug in [
      ...
      SingularityLLM.Plugs.ValidateRequest,
      SingularityLLM.Plugs.ValidateConfiguration
    ]
  end
  ...
  defp is_config_plug?(plug_spec) do
    ...
    plug in [
      ...
      SingularityLLM.Plugs.SetDefaults
    ]
  end
  ...
  defp is_execution_plug?(plug_spec) do
    ...
    plug in [
      ...
      SingularityLLM.Plugs.StreamRequest
  ...
  defp is_cache_plug?(plug_spec) do
    plug = get_plug_module(plug_spec)
    plug == SingularityLLM.Plugs.Cache
  end
  ...
  defp optimization_enabled?() do
    config = Application.get_env(:singularity_llm, :pipeline_optimizer, %{})
  ...
  defp get_plug_reads(plug) do
    ...
    case plug do
      SingularityLLM.Plugs.ValidateProvider -> [:provider]
      SingularityLLM.Plugs.ValidateRequest -> [:messages, :options]
      SingularityLLM.Plugs.ValidateConfiguration -> [:provider, :config]
      SingularityLLM.Plugs.FetchConfig -> [:provider]
      SingularityLLM.Plugs.BuildTeslaClient -> [:config]
      SingularityLLM.Plugs.SetDefaults -> [:provider, :options]
      SingularityLLM.Plugs.ExecuteRequest -> [:tesla_client, :request_body, :url]
      SingularityLLM.Plugs.ParseResponse -> [:response]
      SingularityLLM.Plugs.TrackCost -> [:response, :provider, :model]
      SingularityLLM.Plugs.Cache -> [:provider, :messages, :options]
  ...
  defp get_plug_writes(plug) do
    ...
    case plug do
      ...
      SingularityLLM.Plugs.ValidateRequest -> []
      SingularityLLM.Plugs.ValidateConfiguration -> []
      SingularityLLM.Plugs.FetchConfig -> [:config]
      SingularityLLM.Plugs.BuildTeslaClient -> [:tesla_client]
      SingularityLLM.Plugs.SetDefaults -> [:options, :model]
      SingularityLLM.Plugs.ExecuteRequest -> [:response]
      SingularityLLM.Plugs.ParseResponse -> [:parsed_response]
      SingularityLLM.Plugs.TrackCost -> [:cost]
      SingularityLLM.Plugs.Cache -> [:cached_response]
