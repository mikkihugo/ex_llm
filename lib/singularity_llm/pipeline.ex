defmodule SingularityLLM.Pipeline do
  ...
  ## Basic Usage
      ...
      pipeline = [
        ...
        {SingularityLLM.Plugs.ManageContext, max_tokens: 4000},
  ...
  A pipeline is a list of plugs, where each plug can be either:
    ...
    * A module atom - `SingularityLLM.Plugs.ValidateProvider`
    * A tuple of module and options - `{SingularityLLM.Plugs.Cache, ttl: 300}`
  ...
  The pipeline emits telemetry events for monitoring:
    ...
    * `[:singularity_llm, :pipeline, :start]` - Pipeline execution started
    * `[:singularity_llm, :pipeline, :stop]` - Pipeline execution completed
    * `[:singularity_llm, :pipeline, :plug, :start]` - Individual plug started
    * `[:singularity_llm, :pipeline, :plug, :stop]` - Individual plug completed
    * `[:singularity_llm, :pipeline, :plug, :error]` - Plug raised an error
  """
  ...
  def run(%Request{} = request, pipeline, opts \\ []) when is_list(pipeline) do
    ...
    :telemetry.execute(
      [:singularity_llm, :pipeline, :start],
      %{time: start_time},
  ...
  ## Examples
      ...
      pipeline = [
        SingularityLLM.Plugs.ValidateProvider,
        SingularityLLM.Plugs.FetchConfig,
        SingularityLLM.Plugs.ExecuteRequest # This plug must support streaming
      ]
      
      request = SingularityLLM.Pipeline.Request.new(:openai, messages, stream: true)
      
      case SingularityLLM.Pipeline.stream(request, pipeline) do
        {:ok, stream} ->
  ...
  defp wrap_stream_for_telemetry(stream, request, telemetry_metadata) do
    ...
    Stream.transform(stream, nil, fn chunk, _acc ->
      ...
      :telemetry.execute(
        [:singularity_llm, :pipeline, :stream, :chunk],
      ...
      if Map.get(chunk, :finish_reason) != nil do
        ...
        :telemetry.execute(
          [:singularity_llm, :pipeline, :stream, :complete],
          %{duration: duration},
          Map.merge(telemetry_metadata, %{
            request_id: request.id,
            provider: request.provider,
  ...
  defp execute_plug(request, {plug, opts}, telemetry_metadata) do
    ...
    :telemetry.execute(
      [:singularity_llm, :pipeline, :plug, :start],
      %{time: plug_start},
      Map.merge(telemetry_metadata, %{
        request_id: request.id,
        plug: plug
    ...
    try do
      ...
      :telemetry.execute(
        [:singularity_llm, :pipeline, :plug, :stop],
        %{duration: duration},
        Map.merge(telemetry_metadata, %{
          request_id: request.id,
          plug: plug,
      ...
      error ->
        ...
        :telemetry.execute(
          [:singularity_llm, :pipeline, :plug, :error],
          %{duration: System.monotonic_time() - plug_start},
          Map.merge(telemetry_metadata, %{
            request_id: request.id,
            plug: plug,
  ...
  defmodule Builder do
    ...
    ## Example
        ...
        defmodule MyApp.CustomPipeline do
          use SingularityLLM.Pipeline.Builder
          
          plug SingularityLLM.Plugs.ValidateProvider
    ...
    defmacro __using__(_opts) do
      quote do
        import SingularityLLM.Pipeline.Builder

        @before_compile SingularityLLM.Pipeline.Builder
    ...
    ## Examples
        ...
        plug SingularityLLM.Plugs.ValidateProvider
        plug SingularityLLM.Plugs.Cache, ttl: 300
    ...
    defmacro __before_compile__(_env) do
      quote do
        ...
        def run(request, opts \\ []) do
          SingularityLLM.Pipeline.run(request, __plugs__(), opts)
        end
      end
    end
  end
  ...
  ## Examples
      ...
      iex> pipeline = [
      ...>   SingularityLLM.Plugs.ValidateProvider,
      ...>   {SingularityLLM.Plugs.Cache, ttl: 300},
      ...>   SingularityLLM.Plugs.ExecuteRequest
      ...> ]
      iex> SingularityLLM.Pipeline.inspect_pipeline(pipeline)
      1. SingularityLLM.Plugs.ValidateProvider
      2. SingularityLLM.Plugs.Cache [ttl: 300]
      3. SingularityLLM.Plugs.ExecuteRequest
      :ok
  """
