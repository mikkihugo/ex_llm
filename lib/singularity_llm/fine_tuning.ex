defmodule SingularityLLM.FineTuning do
  @moduledoc """
  Fine-tuning functionality for SingularityLLM.
  ...
  ## Examples
      ...
      {:ok, tuned_model} = SingularityLLM.FineTuning.create_fine_tune(:gemini, dataset, 
        base_model: "models/gemini-1.5-flash-001",
        display_name: "My Custom Model"
      )

      # OpenAI fine-tuning
      {:ok, job} = SingularityLLM.FineTuning.create_fine_tune(:openai, "file-abc123",
        model: "gpt-3.5-turbo",
        suffix: "my-model"
      )

      # List fine-tuning jobs
      {:ok, jobs} = SingularityLLM.FineTuning.list_fine_tunes(:openai)

      # Monitor a specific job
      {:ok, job_details} = SingularityLLM.FineTuning.get_fine_tune(:openai, "ftjob-abc123")

      # Cancel if needed
      {:ok, cancelled_job} = SingularityLLM.FineTuning.cancel_fine_tune(:openai, "ftjob-abc123")
  """
  ...
  ## Examples
      ...
      {:ok, %{tuned_models: models}} = SingularityLLM.FineTuning.list_fine_tunes(:gemini)

      # List OpenAI fine-tuning jobs
      {:ok, %{data: jobs}} = SingularityLLM.FineTuning.list_fine_tunes(:openai, limit: 20)
  ...
  ## Examples
      ...
      {:ok, model} = SingularityLLM.FineTuning.get_fine_tune(:gemini, "tunedModels/my-model-abc123")

      # Get OpenAI fine-tuning job
      {:ok, job} = SingularityLLM.FineTuning.get_fine_tune(:openai, "ftjob-abc123")
  ...
  ## Examples
      ...
      :ok = SingularityLLM.FineTuning.cancel_fine_tune(:gemini, "tunedModels/my-model-abc123")

      # Cancel OpenAI fine-tuning job
      {:ok, job} = SingularityLLM.FineTuning.cancel_fine_tune(:openai, "ftjob-abc123")
