defmodule SingularityLLM.PlugTest do
  ...
  @moduledoc """
  Testing utilities for SingularityLLM plugs and pipelines.

  This module provides helpers and utilities for testing SingularityLLM plugs in isolation
  and testing complete pipelines. It follows the patterns established by Phoenix's
  ConnTest but adapted for SingularityLLM's pipeline architecture.
  ...
  ## Examples
      ...
      request = build_request()
      result = run_plug(request, SingularityLLM.Plugs.ValidateProvider)
      
      # With options
      result = run_plug(request, SingularityLLM.Plugs.Cache, ttl: 3600)
  """
  ...
  def run_pipeline(%Request{} = request, pipeline) when is_list(pipeline) do
    SingularityLLM.Pipeline.run(request, pipeline)
  end
  ...
  ## Examples
      ...
      pipeline = test_pipeline([
        ...
        {SingularityLLM.TestPlugs.MockSuccess, result: "test result"},
        SingularityLLM.TestPlugs.MockError
      ])
  """
  ...
  ## Examples
      ...
      {result, logs} = capture_log(fn ->
        run_plug(request, SingularityLLM.Plugs.FetchConfig)
      end)
