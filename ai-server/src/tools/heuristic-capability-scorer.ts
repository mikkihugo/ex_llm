/**
 * @file Heuristic-Based Capability Scorer
 * @description This module provides a fast, heuristic-based method for scoring AI models
 * on various capabilities. It uses naming patterns and known characteristics of model
 * families to provide a baseline score without the need for expensive LLM analysis.
 */

/**
 * @interface ModelCapabilityScore
 * @description Defines the structure for the capability scores of a model.
 */
interface ModelCapabilityScore {
  code: number;
  reasoning: number;
...
/**
 * Scores a model's capabilities based on a set of heuristics.
 * @param {any} model The model object, which should contain an `id`, `displayName`, `description`, and `cost`.
 * @returns {ModelCapabilityScore} The calculated capability scores for the model.
 */
export function scoreModelByHeuristics(model: any): ModelCapabilityScore {
  ...
  if (id.includes('claude')) {
    ...
    if (id.includes('pro') || id.includes('2.5-pro')) {
      ...
      code = 7; reasoning = 7; creativity = 6; speed = 10; confidence = 'high';
    ...
    if (id.includes('gpt-5') && !id.includes('mini')) {
      ...
      code = 7; reasoning = 7; creativity = 6; speed = 7; confidence = 'medium';
      reasoningText = 'GPT-4.x: Solid performance';
    } else if (id.includes('mini') || id.includes('nano')) {
      code = 6; reasoning = 6; creativity = 5; speed = 10; confidence = 'medium';
    ...
    if (id.includes('fast') || id.includes('code-fast')) {
      ...
      code = 7; reasoning = 8; creativity = 7; speed = 6; confidence = 'medium';
      reasoningText = 'Grok: Alternative reasoning perspective';
    }
  } else if (id.includes('deepseek')) {
    code = 8; reasoning = 9; creativity = 7; speed = 7; confidence = 'medium';
    ...
    if (id.includes('405b') || id.includes('70b')) {
      code = 7; reasoning = 8; creativity = 7; speed = 5; confidence = 'medium';
      reasoningText = 'Large Llama model: Strong reasoning, slower';
    } else {
      code = 6; reasoning = 6; creativity = 6; speed = 8; confidence = 'medium';
      reasoningText = 'Llama model: Open source';
    }
  } else if (id.includes('phi')) {
    code = 7; reasoning = 7; creativity = 6; speed = 9; confidence = 'medium';
    reasoningText = 'Microsoft Phi: Fast, efficient small model';
  } else if (id.includes('cohere')) {
    code = 6; reasoning = 7; creativity = 6; speed = 7; confidence = 'low';
    reasoningText = 'Cohere model: General purpose';
  } else if (id.includes('jamba')) {
    code = 6; reasoning = 7; creativity = 6; speed = 8; confidence = 'low';
    reasoningText = 'AI21 Jamba: Hybrid architecture';
  } else if (id.includes('embedding') || id.includes('embed')) {
    code = 0; reasoning = 0; creativity = 0; speed = 10; costScore = 10; confidence = 'high';
    reasoningText = 'Embedding model: Not for text generation';
  }

  return { code, reasoning, creativity, speed, cost: costScore, confidence, reasoning_text: reasoningText };
}