defmodule SingularityLLM.Providers.AnthropicPublicAPITest do
  @moduledoc """
  Anthropic-specific integration tests using the public SingularityLLM API.
  Common tests are handled by the shared module.
  """

  use SingularityLLM.Shared.ProviderIntegrationTest, provider: :anthropic
  ...
  describe "anthropic-specific features via public API" do
    ...
    test "handles Claude vision capabilities" do
      ...
      case SingularityLLM.chat(:anthropic, messages, model: "claude-3-5-sonnet-20241022", max_tokens: 50) do
        {:ok, response} ->
    ...
    test "handles multiple system messages gracefully" do
      ...
      case SingularityLLM.chat(:anthropic, messages, max_tokens: 50) do
        {:ok, response} ->
    ...
    test "streaming includes proper finish reasons" do
      ...
      case SingularityLLM.stream(:anthropic, messages, collector, max_tokens: 10, timeout: 10_000) do
        :ok ->
          ...
          stream_chunks = Enum.filter(chunks, &match?(%SingularityLLM.Types.StreamChunk{}, &1))
