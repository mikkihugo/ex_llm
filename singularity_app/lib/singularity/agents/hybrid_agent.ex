defmodule Singularity.Agents.HybridAgent do
  @moduledoc """
  Cost-optimized hybrid agent: Rules-first, LLM fallback.
  ...
  alias Singularity.{Autonomy, LLM}
  alias Autonomy.{RuleEngineV2, Correlation}
  ...
  defp try_rules_first(task, state, correlation_id) do
    ...
    rule_result = RuleEngineV2.execute_category(rule_category, task, correlation_id)
  ...
  defp try_llm_with_cache(task, rule_result, state, correlation_id) do
    ...
    case check_semantic_cache(task) do
      {:ok, cached_result} ->
        ...
        new_state = %{state |
          rule_calls_count: state.rule_calls_count + 1,  # Cache is like free rule
          status: :idle
        }
  ...
  defp select_optimal_model(complexity) do
    case complexity do
      :simple -> {:gemini, "gemini-1.5-flash"}  # Cheapest: $0.075/M tokens
      :medium -> {:claude, "claude-3-5-haiku-20241022"}  # Mid: $1/M tokens
      :complex -> {:claude, "claude-3-5-sonnet-20241022"}  # Best: $3/M tokens
    end
  end
  ...
  defp extract_code_from_response(response) do
    ...
    case Regex.run(~r/```(?:\w+)?\n(.*?)```/s, response) do
      ...
      nil -> response  # No markdown, assume entire response is code
    end
  end
  ...
  defp via_tuple(id) do
    {:via, Registry, {Singularity.AgentRegistry, {:agent, id}}}
  end
end
