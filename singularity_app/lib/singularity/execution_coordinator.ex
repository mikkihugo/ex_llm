defmodule Singularity.ExecutionCoordinator do
  @moduledoc """
  Orchestrates the two DAGs:
  - Template Performance DAG (top) - Selects best templates
  - SPARC HTDAG (bottom) - Executes tasks
  ...
  def init(_opts) do
    state = %__MODULE__{
      template_dag: nil,  # Will connect to TemplateOptimizer
      sparc_dag: nil,     # Will be created per execution
  ...
  defp evaluate_quality(result) do
    ...
    # Has error handling?
    score = score + if String.contains?(result, ["try", "catch", "rescue", "with"]), do: 0.1, else: 0

    # Has structure?
    score = score + if String.contains?(result, ["def", "defmodule", "function", "class"]), do: 0.2, else: 0
