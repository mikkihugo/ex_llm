defmodule Singularity.TemplateOptimizer do
  @moduledoc """
  HTDAG for tracking which templates work best for different tasks.
  ...
  def handle_cast({:record, template_id, task, metrics}, state) do
    ...
    new_rankings = if state.learning_enabled do
    ...
    {:noreply, %{state |
      performance_data: updated_data,
      template_rankings: new_rankings
    }}
  end
  ...
  def handle_call({:get_best, task_type, language}, _from, state) do
    ...
    best_template = case template_scores do
  ...
  defp update_performance_data(data, template_id, task, metrics) do
    ...
    updated = %{current |
      success_rate: (1 - alpha) * current.success_rate + alpha * (if metrics.success, do: 1.0, else: 0.0),
      ...
      feedback: [metrics.feedback | current.feedback] |> Enum.take(100)  # Keep last 100
    }
  ...
  defp calculate_ranking_score(perf) do
    ...
    days_ago = if perf.last_used do
      DateTime.diff(DateTime.utc_now(), perf.last_used, :day)
    else
      365
    end
    recency_score = :math.exp(-days_ago / 30)  # Exponential decay
  ...
  defp load_historical_data(state) do
    # Load past performance data from database
    case CodeStore.query_facts(%{type: "template_performance", limit: 1000}) do
      {:ok, facts} ->
        Enum.each(facts, fn fact ->
          GenServer.cast(self(), {:record, fact.template_id, fact.task, fact.metrics})
        end)

      _ ->
        Logger.info("No historical template performance data found")
    end
  end
