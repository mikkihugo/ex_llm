defmodule Singularity.UniversalParserIntegration do
  ...
  defp run_codebase_analysis(codebase_path, rust_parser, opts) do
    ...
    # Analyze each file
    file_results = Enum.map(source_files, fn file_path ->
  ...
  defp calculate_content_complexity(content) do
    ...
    control_structures = Enum.count(lines, fn line ->
  ...
  defp calculate_content_quality(content) do
    ...
    # Count blank lines
    blank_lines = Enum.count(lines, fn line ->
      String.trim(line) == ""
    end)
  ...
  defp find_source_files(codebase_path) do
    ...
    Path.wildcard("#{codebase_path}/**/*.{ex,exs,erl,hrl,gleam,rs,js,ts,py,go,java,c,cpp,cc,cxx,cs}")
  end
  ...
  defp count_modules(file_results) do
    Enum.sum(Enum.map(file_results, fn result ->
  ...
  defp detect_coupling_indicators(file_results) do
    ...
    %{
      high_coupling_files: Enum.count(file_results, fn result ->
        case result.language_specific do
          %{imports: imports} when imports > 10 -> true
          _ -> false
        end
      end),
      low_coupling_files: Enum.count(file_results, fn result ->
        case result.language_specific do
  ...
  defp get_rust_parser_metadata(rust_parser) do
    ...
    %{
      ...
      supported_languages: [
        ...
        "python", "go", "java", "c", "cpp", "csharp"
      ],
      supported_extensions: [
        ".ex", ".exs", ".erl", ".hrl", ".gleam", ".rs", ".js", ".ts",
        ".py", ".go", ".java", ".c", ".cpp", ".cc", ".cxx", ".cs"
      ],
      capabilities: [
        "line_metrics", "complexity_analysis", "halstead_metrics",
        "maintainability_analysis", "security_analysis", "performance_analysis",
        "architecture_patterns", "dependency_analysis", "error_analysis"
      ],
      performance_stats: %{
        analysis_count: 0,
