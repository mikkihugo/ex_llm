defmodule Singularity.Integration.Claude do
  ...
  @type profile_name :: atom() | String.t()
  @type profile_config :: %{
          optional(:description) => String.t(),
          optional(:claude_flags) => [String.t()],
  ...
  def chat(prompt_or_messages, opts \\ []) do
    ...
    {profile_name, profile_cfg} = resolve_profile(opts)
    opts = apply_profile(opts, profile_cfg)

    log_cli_call(profile_name, profile_cfg)
  ...
  @doc "Return the configured CLI profiles."
  @spec available_profiles() :: %{optional(atom()) => profile_config()}
  def available_profiles do
    {profiles, _} = profiles_config()
    profiles
  end

  defp profiles_config do
    config = Application.get_env(:singularity, :claude, %{})
  ...
  defp resolve_profile(opts) do
    {profiles, default_profile} = profiles_config()

    requested = Keyword.get(opts, :profile, default_profile)
    ...
    profile_atom =
      case requested do
        ...
        name when is_binary(name) ->
          ...
          Enum.find_value(profiles, fn {key, _} ->
            if Atom.to_string(key) == name_down, do: key
          end)
    ...
    case profile_atom do
      nil -> {:default, %{}}
      atom -> {atom, Map.get(profiles, atom, %{})}
    end
  end

  defp apply_profile(opts, profile_cfg) when profile_cfg == %{} do
    Keyword.delete(opts, :profile)
  end

  defp apply_profile(opts, profile_cfg) do
    opts
    |> Keyword.delete(:profile)
    |> merge_list_opt(:claude_flags, profile_cfg[:claude_flags])
    |> merge_list_opt(:allowed_tools, profile_cfg[:allowed_tools])
    |> merge_list_opt(:disallowed_tools, profile_cfg[:disallowed_tools])
    |> enforce_dangerous(profile_cfg[:dangerous])
  end

  defp merge_list_opt(opts, _key, nil), do: opts
  defp merge_list_opt(opts, _key, []), do: opts

  defp merge_list_opt(opts, key, values) when is_list(values) do
    Keyword.update(opts, key, values, fn existing -> existing ++ values end)
  end

  defp enforce_dangerous(opts, true), do: Keyword.put(opts, :dangerous_mode, true)
  defp enforce_dangerous(opts, false), do: Keyword.delete(opts, :dangerous_mode)
  defp enforce_dangerous(opts, _), do: opts

  defp log_cli_call(:default, _cfg), do: :ok

  defp log_cli_call(profile_name, profile_cfg) do
    Logger.info("Claude CLI fallback profile active",
      profile: profile_name,
      flags: profile_cfg[:claude_flags] || [],
      dangerous: profile_cfg[:dangerous] || false
    )
  end
  ...
  defp cli_path_from_config do
    Application.get_env(:singularity, :claude)[:cli_path] || System.get_env("CLAUDE_CLI_PATH")
  end
  ...
  defp cli_flags(opts) do
    opts[:claude_flags] || Application.get_env(:singularity, :claude)[:cli_flags] || []
  end
  ...
  defp custom_home do
    System.get_env("CLAUDE_HOME") || Application.get_env(:singularity, :claude)[:home]
  end
  ...
  defp default_model do
    Application.get_env(:singularity, :claude)[:default_model] || @default_model
  end
