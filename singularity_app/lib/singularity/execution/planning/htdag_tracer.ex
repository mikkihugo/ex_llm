defmodule Singularity.Execution.Planning.HTDAGTracer do
  ...
  - `Singularity.Execution.Planning.HTDAGLearner` - Learning integration (HTDAGLearner.learn_with_tracing/1)
  ...
  def find_broken_functions do
    ...
    broken = Enum.reduce(all_functions, [], fn {mod, fun, arity}, acc ->
  ...
  def full_analysis(opts \\ []) do
    ...
    {:ok, trace_results} = trace_runtime(duration_ms: Keyword.get(opts, :trace_duration_ms, 15_000))
  ...
  def get_recommendations(analysis) do
    ...
    recommendations = if length(analysis.dead_code) > 0 do
      dead_recs = Enum.map(analysis.dead_code, fn {mod, fun, arity} ->
        %{
          type: :dead_code,
          severity: :low,
          module: mod,
    ...
    recommendations = if length(analysis.broken_functions) > 0 do
      broken_recs = Enum.map(analysis.broken_functions, fn {mod, fun, arity, reason} ->
        %{
    ...
    recommendations = if length(disconnected) > 0 do
      disconn_recs = Enum.map(disconnected, fn mod ->
        %{
    ...
    recommendations = if length(slow_functions) > 0 do
      perf_recs = Enum.map(slow_functions, fn {mod, fun, arity, avg_time} ->
        %{
          type: :slow_function,
          severity: :medium,
          module: mod,
  ...
  defp build_call_graph(trace_results) do
    ...
    edges = Enum.flat_map(trace_results, fn {{mod, fun, _arity}, _data} ->
  ...
  defp analyze_connectivity(call_graph) do
    ...
    connected_modules = call_graph.edges
    |> Enum.flat_map(fn {a, b} -> [a, b] end)
    |> Enum.uniq()
    
    disconnected = MapSet.difference(
  ...
  defp calculate_avg_time(trace_results) do
    if map_size(trace_results) == 0 do
      ...
      total = Enum.reduce(trace_results, 0, fn {_, data}, acc ->
  ...
  defp print_summary(result) do
    ...
    Logger.info("  Connectivity Ratio: #{Float.round(result.coverage_data.connectivity_ratio * 100, 1)}%")
    Logger.info("")
    Logger.info("Performance:")
    Logger.info("  Avg Response Time: #{Float.round(result.performance_data.avg_response_time, 2)}ms")
    Logger.info("  P95 Response Time: #{Float.round(result.performance_data.p95_response_time, 2)}ms")
