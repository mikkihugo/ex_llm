...
impl LintingEngine {
    ...
    pub fn new() -> Self {
        let mut engine = Self {
            config: LintingEngineConfig::default(),
    ...
    fn initialize_ai_pattern_rules(&mut self) {
        self.ai_pattern_rules = vec![
            ...
            QualityRule {
                ...
                description:
                    "Detect placeholder comments that indicate incomplete AI-generated code"
                        .to_string(),
            ...
            QualityRule {
                ...
                message: "Unused parameter detected - implement or remove if not needed"
                    .to_string(),
                category: RuleCategory::AIGenerated,
            },
            QualityRule {
            ...
            QualityRule {
                ...
                message: "Hardcoded path detected - use configuration or environment variables"
                    .to_string(),
                category: RuleCategory::AIGenerated,
            },
        ];
    ...
    fn initialize_enterprise_rules(&mut self) {
        self.enterprise_rules = vec![
            ...
            QualityRule {
                ...
                pattern:
                    r"(?i)(password|secret|key|token|credential)\s*[:=]\s*['\x22][^'\x22]+['\x22]"
                        .to_string(),
            ...
            QualityRule {
                ...
                message: "Potential N+1 query pattern detected - consider batch operations"
                    .to_string(),
    ...
    pub async fn run_all_gates(&self, project_path: &str) -> Result<QualityGateResult> {
        ...
        let info: Vec<_> = all_issues
            .iter()
            .filter(|issue| issue.severity == RuleSeverity::Info)
            .cloned()
            .collect();

        let ai_pattern_issues: Vec<_> = all_issues
            .iter()
    ...
    fn parse_eslint_output(line: &str) -> Option<QualityIssue> {
        ...
        if let Ok(json_value) = serde_json::from_str::<serde_json::Value>(line) {
            ...
            if let Some(file_path) = json_value.get("filePath").and_then(|v| v.as_str()) {
                let line_number = json_value
                    .get("line")
                    .and_then(serde_json::Value::as_u64)
                    .and_then(|n| usize::try_from(n).ok());
                let column = json_value
                    .get("column")
                    .and_then(serde_json::Value::as_u64)
                    .and_then(|n| usize::try_from(n).ok());
                let message = json_value
                    .get("message")
                    .and_then(|v| v.as_str())
                    .unwrap_or("ESLint issue");
                let rule = json_value
                    .get("ruleId")
                    .and_then(|v| v.as_str())
                    .unwrap_or("eslint_rule");
    ...
    fn parse_clippy_output(&self, line: &str) -> Option<QualityIssue> {
        ...
        if let Some(caps) = re.captures(line) {
            ...
            let rule = caps
                ...
                .unwrap_or_else(|| "clippy_rule".to_string());
        ...
        if let Ok(json_value) = serde_json::from_str::<serde_json::Value>(line) {
            if let Some(file_path) = json_value.get("file").and_then(|v| v.as_str()) {
                ...
                let message = json_value
                    .get("text")
                    .and_then(|v| v.as_str())
                    .unwrap_or("golangci-lint issue");
                let rule = json_value
                    .get("rule")
                    .and_then(|v| v.as_str())
                    .unwrap_or("golangci_rule");
