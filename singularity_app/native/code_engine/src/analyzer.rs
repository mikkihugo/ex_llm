...
impl CodeNamer {
  ...
  // Stub method - real implementation in sparc-engine CodeNamer
  pub fn suggest_name(
    &self,
    base_name: &str,
...
impl CodebaseAnalyzer {
  /// Create a new codebase analyzer (async, requires project_id for storage path)
  pub async fn new(project_id: &str) -> Result<Self> {
    use crate::paths::SPARCPaths;

    // Get proper storage path using SPARCPaths
    let storage_path = SPARCPaths::project_storage(project_id)?;

    // Initialize Store with proper path
    let storage = Some(CodebaseDatabase::new(storage_path).await?);

    Ok(Self {
      storage,
      // global_cache: GlobalCacheCoordinator::default(), // Removed - belongs in engine
      // session_manager: SessionCoordinator::new(), // Removed - belongs in engine
      // intelligence: IntelligenceEngine::default(), // Removed - belongs in engine
