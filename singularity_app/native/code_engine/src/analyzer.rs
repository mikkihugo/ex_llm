...
// Naming moved to architecture_engine
...
// map_element_type function removed - naming moved to architecture_engine

use anyhow::Result;
use prompt_engine::prompt_tracking::ProjectTechStackFact;
use tracing::info;

// Storage removed - Elixir handles storage

/// Framework detector stub
/// Real implementation: sparc-engine/src/framework_detector/detector.rs
#[derive(Debug, Clone)]
...
impl CodebaseAnalyzer {
    /// Create a new codebase analyzer (pure analysis, no I/O)
    pub fn new() -> Result<Self> {
        Ok(Self {
            performance_tracker: PerformanceTracker::new(),
            metrics_collector: crate::analysis::metrics::MetricsCollector::new(),
            graph_analyzer: CodeGraph::new(
                crate::analysis::graph::code_graph::GraphType::CallGraph,
            ),
            ...
            framework_detector: FrameworkDetector::new()
                .unwrap_or_else(|_| FrameworkDetector::default()),
        })
    }
    ...
    pub async fn detect_frameworks(
        &self,
        _project_path: &Path,
    ) -> Result<Vec<ProjectTechStackFact>, String> {
    ...
    // Old analyze_code method removed - replaced with pure analysis version

    // Naming suggestions moved to architecture_engine
    ...
    async fn parse_and_store_files(&self, project_path: &Path) -> Result<Vec<ParsedFile>, String> {
            ...
            .filter(|e| e.file_type().is_file())
        {
            ...
            match self
                .analyze_with_language_parser(&content, language, file_path)
                .await
            {
                Ok(analysis_result) => {
                ...
                Err(e) => {
                    ...
                    let parsed_file = ParsedFile {
                        ...
                        language,
                        content: content.clone(),
                        analysis_result: self.create_fallback_analysis_result(
        ...
        info!(
            "Successfully parsed {} files from project",
            parsed_files.len()
        );
        Ok(parsed_files)
    }
        ...
        match language {
            ProgrammingLanguage::Rust => {
                ...
                match rust_parser::RustParser::new() {
                    Ok(parser) => parser
                        .analyze_content(content, file_path.to_str().unwrap_or("unknown"))
                        .await
                        .map_err(|e| format!("Rust parser error: {}", e)),
            ...
            ProgrammingLanguage::Python => {
                ...
                match python_parser::PythonParser::new() {
                    Ok(parser) => parser
                        .analyze_content(content, file_path.to_str().unwrap_or("unknown"))
                        .await
                        .map_err(|e| format!("Python parser error: {}", e)),
            ...
            ProgrammingLanguage::JavaScript => {
                ...
                match javascript_parser::JavaScriptParser::new() {
                    Ok(parser) => parser
                        .analyze_content(content, file_path.to_str().unwrap_or("unknown"))
                        .await
                        .map_err(|e| format!("JavaScript parser error: {}", e)),
            ...
            ProgrammingLanguage::TypeScript => {
                ...
                match typescript_parser::TypeScriptParser::new() {
                    Ok(parser) => parser
                        .analyze_content(content, file_path.to_str().unwrap_or("unknown"))
                        .await
                        .map_err(|e| format!("TypeScript parser error: {}", e)),
            ...
            ProgrammingLanguage::Go => {
                ...
                match go_parser::GoParser::new() {
                    Ok(parser) => parser
                        .analyze_content(content, file_path.to_str().unwrap_or("unknown"))
                        .await
                        .map_err(|e| format!("Go parser error: {}", e)),
            ...
            ProgrammingLanguage::Java => {
                ...
                match java_parser::JavaParser::new() {
                    Ok(parser) => parser
                        .analyze_content(content, file_path.to_str().unwrap_or("unknown"))
                        .await
                        .map_err(|e| format!("Java parser error: {}", e)),
            ...
            ProgrammingLanguage::CSharp => {
                ...
                match csharp_parser::CSharpParser::new() {
                    Ok(parser) => parser
                        .analyze_content(content, file_path.to_str().unwrap_or("unknown"))
                        .await
                        .map_err(|e| format!("C# parser error: {}", e)),
            ...
            ProgrammingLanguage::C => {
                ...
                match c_cpp_parser::CCppParser::new() {
                    Ok(parser) => parser
                        .analyze_content(content, file_path.to_str().unwrap_or("unknown"))
                        .await
                        .map_err(|e| format!("C/C++ parser error: {}", e)),
            ...
            ProgrammingLanguage::Erlang => {
                ...
                match erlang_parser::ErlangParser::new() {
                    Ok(parser) => parser
                        .analyze_content(content, file_path.to_str().unwrap_or("unknown"))
                        .await
                        .map_err(|e| format!("Erlang parser error: {}", e)),
            ...
            ProgrammingLanguage::Elixir => {
                ...
                match elixir_parser::ElixirParser::new() {
                    Ok(parser) => parser
                        .analyze_content(content, file_path.to_str().unwrap_or("unknown"))
                        .await
                        .map_err(|e| format!("Elixir parser error: {}", e)),
    ...
    fn extract_metrics_from_analysis(
        &self,
    ...
    fn process_rust_specific_data(
        &self,
        metrics: &mut FileMetrics,
        language_data: &std::collections::HashMap<String, serde_json::Value>,
    ) {
        if let Some(rust_data) = language_data.get("rust") {
            if let Ok(rust_analysis) =
                serde_json::from_value::<rust_parser::RustSpecificAnalysis>(rust_data.clone())
            {
                // Adjust complexity based on Rust-specific patterns
                if rust_analysis.unsafe_usage.len() > 0 {
                    metrics.cyclomatic_complexity += rust_analysis.unsafe_usage.len() as f64 * 2.0;
                    // Unsafe code increases complexity
                }
    ...
    /// Process Rustler integration patterns
    fn process_rustler_integration(
        &self,
        rustler: &rust_parser::RustlerIntegration,
        metrics: &mut FileMetrics,
    ) {
        if rustler.has_rustler {
            ...
            if rustler
                .error_handling
                .contains(&"Result type usage".to_string())
            {
            ...
            if rustler
                .error_handling
                .contains(&"Option type usage".to_string())
            {
            ...
            if rustler
                ...
                .contains(&"Inline functions".to_string())
            {
            ...
            if rustler
                .performance_optimizations
                .contains(&"Const functions".to_string())
            {
            ...
            if rustler
                .performance_optimizations
                .contains(&"No-std optimization".to_string())
            {
            ...
            if rustler
                .memory_management
                .contains(&"Box allocation".to_string())
            {
            ...
            if rustler
                .memory_management
                .contains(&"Arc atomic reference counting".to_string())
            {
            ...
            if rustler
                .memory_management
                .contains(&"Vec dynamic allocation".to_string())
            {
            ...
            if !rustler.concurrency_patterns.is_empty() {
                ...
                metrics.cyclomatic_complexity += concurrency_count as f64 * 1.5;
                // Concurrency adds branches
            }
    ...
    fn process_erlang_specific_data(
        &self,
        metrics: &mut FileMetrics,
        language_data: &std::collections::HashMap<String, serde_json::Value>,
    ) {
        if let Some(erlang_data) = language_data.get("erlang") {
            ...
            if let Ok(erlang_analysis) =
                serde_json::from_value::<erlang_parser::ErlangAnalysisResult>(erlang_data.clone())
            {
                    ...
                    .is_empty()
                {
                    ...
                    metrics.cyclomatic_complexity += message_count as f64 * 1.2;
                    // Message passing adds complexity
                }
    ...
    fn process_elixir_specific_data(
        &self,
        metrics: &mut FileMetrics,
        language_data: &std::collections::HashMap<String, serde_json::Value>,
    ) {
        if let Some(elixir_data) = language_data.get("elixir") {
            ...
            if let Ok(elixir_analysis) =
                serde_json::from_value::<elixir_parser::ElixirAnalysisResult>(elixir_data.clone())
            {
                // Process OTP behaviors
                ...
                {
                    let component_count =
                        elixir_analysis.phoenix_analysis.detected_components.len();
                ...
                if elixir_analysis.functional_analysis.pattern_match_complexity > 5.0 {
                    metrics.cognitive_complexity +=
                        elixir_analysis.functional_analysis.pattern_match_complexity * 0.5;
                }
            } else {
                // Fallback to simple processing if parsing fails
    ...
    fn process_gleam_specific_data(
        &self,
        metrics: &mut FileMetrics,
        language_data: &std::collections::HashMap<String, serde_json::Value>,
    ) {
        if let Some(gleam_data) = language_data.get("gleam") {
            ...
            if let Ok(gleam_analysis) =
                serde_json::from_value::<gleam_parser::GleamAnalysisResult>(gleam_data.clone())
            {
                // Process type system
                ...
                if !gleam_analysis.actor_analysis.message_types.is_empty() {
                    ...
                    metrics.cyclomatic_complexity += message_count as f64 * 1.5;
                    // Message types add complexity
                }
                ...
                if gleam_analysis.functional_analysis.pattern_match_complexity > 5.0 {
                    metrics.cognitive_complexity +=
                        gleam_analysis.functional_analysis.pattern_match_complexity * 0.8;
                }
    ...
    fn process_generic_language_data(
        &self,
        metrics: &mut FileMetrics,
        language_data: &std::collections::HashMap<String, serde_json::Value>,
    ) {
    ...
    async fn generate_naming_suggestions(
        &self,
        files: &[ParsedFile],
    ) -> Result<NamingAnalysis, String> {
        ...
        analysis.overall_score = if analysis.suggestions.is_empty() {
            1.0
        } else {
            analysis
                .suggestions
                ...
                .map(|s| s.confidence)
                .sum::<f64>()
                / analysis.suggestions.len() as f64
        };
...
fn map_element_type(element_type: CodeElementType) -> ArchitectureElementType {
    use CodeElementType::*;

    match element_type {
        Function => ArchitectureElementType::Function,
        DataStructure | Class | Trait | Enum => ArchitectureElementType::Class,
        Interface => ArchitectureElementType::Interface,
        Module | Configuration | Documentation | ErrorType | Test => {
            ArchitectureElementType::Module
        }
        Field | Variable | Constant => ArchitectureElementType::Variable,
    }
}
