...
impl SparcOptimizer {
  ...
  pub fn for_phase(phase: SPARCPhase) -> Self {
    let strategy = match phase {
      SPARCPhase::Specification | SPARCPhase::Architecture | SPARCPhase::Refinement => {
        OptimizationStrategy::DeepReasoning { emphasize_logic: true, include_sparc_context: true }
      }
      SPARCPhase::Pseudocode | SPARCPhase::Completion => {
        OptimizationStrategy::CodeGeneration { target_language: "TypeScript".to_string(), use_primelearned_code_patterns: true }
      }
    };

    Self { current_phase: phase, strategy, metrics: SparcMetrics::default() }
  }

  /// Optimize prompt for current SPARC phase
  pub async fn optimize_prompt(&mut self, base_prompt: &str) -> Result<String> {
    match self.strategy.clone() {
      OptimizationStrategy::DeepReasoning { emphasize_logic, include_sparc_context } => {
        self.optimize_for_reasoning(base_prompt, emphasize_logic, include_sparc_context).await
      }
      OptimizationStrategy::CodeGeneration { target_language, use_primelearned_code_patterns } => {
        self.optimize_for_code_generation(base_prompt, &target_language, use_primelearned_code_patterns).await
      }
    }
  }

  /// Optimize prompt for deep reasoning tasks (Opus)
  async fn optimize_for_reasoning(&mut self, prompt: &str, emphasize_logic: bool, include_sparc_context: bool) -> Result<String> {
    let mut optimized = String::new();

    // Add SPARC methodology context
    if include_sparc_context {
      optimized.push_str(&format!("🎯 SPARC {} Phase Analysis\n\n", format!("{:?}", self.current_phase).to_uppercase()));
  ...
  async fn optimize_for_code_generation(&mut self, prompt: &str, target_language: &str, use_primelearned_code_patterns: bool) -> Result<String> {
    let mut optimized = String::new();

    // Add code generation context
    optimized.push_str(&format!("🔧 SPARC {} Phase - {} Development\n\n", format!("{:?}", self.current_phase).to_uppercase(), target_language));
    ...
    optimized.push_str(&format!("Generate high-quality {} code that:\n", target_language));
  ...
  fn measure_sparc_adherence(&self, result: &str) -> f64 {
    ...
    if result.contains("requirements") || result.contains("analysis") || result.contains("architecture") {
      score += 0.3;
    }

    // Check for quality considerations
    if result.contains("quality") || result.contains("standards") || result.contains("best practices") {
      score += 0.4;
    }
  ...
  fn measure_model_efficiency(&self) -> f64 {
    match (&self.current_phase, &self.strategy) {
      ...
      (SPARCPhase::Specification | SPARCPhase::Architecture | SPARCPhase::Refinement, OptimizationStrategy::DeepReasoning { .. }) => 1.0,

      // Sonnet phases with code strategy = efficient
      (SPARCPhase::Pseudocode | SPARCPhase::Completion, OptimizationStrategy::CodeGeneration { .. }) => 1.0,
...
mod tests {
  ...
  fn test_optimizer_creation() {
    ...
    assert!(matches!(optimizer.strategy, OptimizationStrategy::DeepReasoning { .. }));
  }
