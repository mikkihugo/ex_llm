...
impl SPARCCoordinator {
  ...
  pub async fn execute_request(&mut self, request: SPARCRequest) -> Result<SparcResponse> {
    ...
    if self.call_count >= self.max_calls_per_phase {
      return Err(anyhow::anyhow!("Maximum calls ({}) exceeded for {:?} phase", self.max_calls_per_phase, self.current_phase));
    }
    ...
    Ok(SparcResponse { content: response_content, model_used: model, session_id, metrics, next_steps })
  }
  ...
  async fn build_coordinated_prompt(&mut self, request: &SPARCRequest) -> Result<String> {
    ...
    if let Some(session) = self.active_sessions.values().find(|s| s.session_type == request.session_type) {
    ...
    coordinated_prompt.push_str(&format!("- This is part of SPARC {} phase\n", format!("{:?}", self.current_phase).to_lowercase()));
    coordinated_prompt.push_str("- Consider how this work fits with other tasks in this phase\n");
  ...
  fn select_model(&self, request: &SPARCRequest) -> String {
    ...
    match (&self.current_phase, &request.session_type) {
      ...
      (SPARCPhase::Specification | SPARCPhase::Architecture | SPARCPhase::Refinement, SessionType::Analysis | SessionType::Design | SessionType::Review) => {
        "opus".to_string()
      }
  ...
  fn get_or_create_session(&mut self, session_type: SessionType) -> Uuid {
    ...
    let session =
      PhaseSession { session_id, session_type, conversation: ConversationHistory::new(), optimized_prompt: None, session_metrics: SparcMetrics::default() };
  ...
  fn update_phase_metrics(&mut self, session_metrics: &SparcMetrics) {
    ...
    self.phase_metrics.methodology_adherence = self.phase_metrics.methodology_adherence * prev_weight + session_metrics.methodology_adherence * new_weight;

    self.phase_metrics.output_quality = self.phase_metrics.output_quality * prev_weight + session_metrics.output_quality * new_weight;

    self.phase_metrics.primecode_compliance = self.phase_metrics.primecode_compliance * prev_weight + session_metrics.primecode_compliance * new_weight;

    self.phase_metrics.model_efficiency = self.phase_metrics.model_efficiency * prev_weight + session_metrics.model_efficiency * new_weight;
  }
  ...
  pub fn is_phase_ready_for_transition(&self) -> bool {
    ...
    self.call_count > 0 && self.phase_metrics.methodology_adherence > 0.7 && self.phase_metrics.output_quality > 0.7
  }
...
mod tests {
  ...
  async fn test_coordinator_creation() {
    ...
    assert!(matches!(coordinator.current_phase, SPARCPhase::Specification));
  ...
  fn test_model_selection() {
    ...
    let request =
      SPARCRequest { session_type: SessionType::Analysis, task: "Analyze requirements".to_string(), phase_context: HashMap::new(), model_preference: None };
