...
impl RedbPromptStorage {
  ...
  pub fn store(&self, bit: &StoredPromptBit) -> Result<()> {
...

    {
      ...
      trigger_index.insert(trigger_key.as_str(), serde_json::to_string(&trigger_ids)?.as_str())?;
      ...
      category_index.insert(category_key.as_str(), serde_json::to_string(&category_ids)?.as_str())?;
    }
  ...
  pub fn update_usage_stats(&self, id: &str, success: bool) -> Result<()> {
...

    {
      ...
      if let Some(json) = json_data {
        ...
        if success {
          bit.success_rate = (bit.success_rate * (bit.usage_count - 1) as f64 + 1.0) / bit.usage_count as f64;
        } else {
          bit.success_rate = (bit.success_rate * (bit.usage_count - 1) as f64) / bit.usage_count as f64;
        }
  ...
  fn get_index_ids<T: ReadableTable<&'static str, &'static str>>(&self, key: &str, table: &T) -> Result<Vec<String>> {
    match table.get(key)? {
      Some(json) => {
...
mod tests {
  ...
  fn test_store_and_retrieve() {
    ...
    let bit = StoredPromptBit {
      ...
      metadata: PromptBitMetadata { confidence: 0.9, last_updated: chrono::Utc::now(), versions: vec!["1.0".to_string()], related_bits: vec![] },
  ...
  fn test_find_by_trigger() {
    ...
    let results = storage.find_by_trigger(&PromptBitTrigger::Language("Rust".to_string())).unwrap();
