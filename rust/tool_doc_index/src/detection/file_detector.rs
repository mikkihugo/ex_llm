...
impl FileDetector {
  ...
  async fn detect_python_patterns(
    ...
    if confidence > 0.5 {
      ...
      let dev_command = match framework_name {
        "django" => Some("python manage.py runserver".to_string()),
        "flask" => Some("flask run".to_string()),
        "fastapi" => Some("uvicorn main:app --reload".to_string()),
        _ => Some("python main.py".to_string()),
      };
