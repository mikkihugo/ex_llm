...
fn templates_command(tag: Option<String>, detailed: bool) -> Result<()> {
  ...
  let registry = fact_tools::template::RegistryTemplate::new();

  let templates = tag.map_or_else(
    || {
      registry
        .list()
        .into_iter()
    ...
    |tag| registry.search_by_tags(&[tag]),
  );
...
async fn benchmark_command(
  ...
  let throughput = 1000.0 / avg_duration.as_millis() as f64;
  ...
  if start {
    ...
    match orchestrator.start().await {
      Ok(()) => {
        ...
        println!("🔄 Update interval: {interval} hours");
