...
impl AutoFactOrchestrator {
  /// Create a new `AutoOrchestrator` instance
  ///
  /// # Errors
  ///
  /// Returns an error if:
  ...
  /// - Initial setup fails
  pub fn new(config: AutoConfig) -> Result<Self> {
    let fact = Fact::new();
  ...
  fn extract_dependencies_from_repo(&mut self, repo_path: &std::path::Path) {
    ...
    for package_file in package_files {
      match Self::parse_package_file(&package_file) {
        Ok(dependencies) => {
  ...
  fn parse_package_file(
    ...
    match file_name {
      ...
      "go.mod" => Ok(Self::parse_go_dependencies_local(&content)),
  ...
  async fn create_project_from_file(
    ...
    let (language, dependencies) = match file_name {
      ...
      "setup.py" => {
        let content = fs::read_to_string(file_path).await?;
        (ProjectLanguage::Python, Self::parse_python_setup(&content))
      }
      "pyproject.toml" => {
        let content = fs::read_to_string(file_path).await?;
        (
      ...
      "go.mod" => {
        let content = fs::read_to_string(file_path).await?;
        (ProjectLanguage::Go, self.parse_go_dependencies(&content))
      }
      // Java/JVM
      "pom.xml" => {
        let content = fs::read_to_string(file_path).await?;
      ...
      "composer.json" => {
        let content = fs::read_to_string(file_path).await?;
        (ProjectLanguage::PHP, self.parse_php_dependencies(&content))
      }
      // .NET
      name
      ...
      "DESCRIPTION" => {
        let content = fs::read_to_string(file_path).await?;
        (ProjectLanguage::R, self.parse_r_dependencies(&content))
      }
      // Julia
      "Project.toml" => {
        let content = fs::read_to_string(file_path).await?;
...
mod tests {
  ...
  async fn test_auto_orchestrator_creation() {
    ...
    let orchestrator = AutoFactOrchestrator::new(config).unwrap();
  ...
  async fn test_dependency_parsing() {
    ...
    let deps = AutoFactOrchestrator::parse_elixir_dependencies(mix_content);
