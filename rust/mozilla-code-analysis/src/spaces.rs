...
impl FuncSpace {
  fn new<T: Getter>(node: &Node, code: &[u8], kind: SpaceKind) -> Self {
    ...
    Self {
      name: T::get_func_space_name(node, code).map(|name| name.split_whitespace().collect::<Vec<_>>().join(" ")),
      spaces: Vec::new(),
...
fn compute_halstead_mi_and_wmc<T: ParserTrait>(state: &mut State) {
  state.halstead_maps.finalize(&mut state.space.metrics.halstead);
  T::Mi::compute(&state.space.metrics.loc, &state.space.metrics.cyclomatic, &state.space.metrics.halstead, &mut state.space.metrics.mi);
  T::Wmc::compute(state.space.kind, &state.space.metrics.cyclomatic, &mut state.space.metrics.wmc);
}
...
fn compute_averages(state: &mut State) {
  ...
  // Nargs average
  state.space.metrics.nargs.finalize(nom_functions, nom_closures);
}
...
pub fn metrics<'a, T: ParserTrait>(parser: &'a T, path: &'a Path) -> Option<FuncSpace> {
  ...
  while let Some((node, level)) = stack.pop() {
    ...
    let new_level = if func_space {
      let state = State { space: FuncSpace::new::<T::Getter>(&node, code, kind), halstead_maps: HalsteadMaps::new() };
