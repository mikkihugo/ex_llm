...
fn java_inspect_container(container_node: &Node, conditions: &mut f64) {
  ...
  let mut has_boolean_content = match node.parent().unwrap().kind_id().into() {
    ...
    TernaryExpression => node.previous_sibling().map_or(true, |prev_node| !matches!(prev_node.kind_id().into(), QMARK | COLON)),
    _ => false,
  };
  ...
  loop {
    ...
    let is_not_operator = matches!(node_kind, UnaryExpression) && matches!(node.child(0).unwrap().kind_id().into(), BANG);
...
fn java_count_unary_conditions(list_node: &Node, conditions: &mut f64) {
  ...
  if cursor.goto_first_child() {
    loop {
      ...
      if matches!(node_kind, MethodInvocation | Identifier | True | False) && matches!(list_kind, BinaryExpression) && !matches!(list_kind, ArgumentList) {
        *conditions += 1.;
      } else {
...
impl Abc for JavaCode {
  fn compute(node: &Node, stats: &mut Stats) {
    ...
    match node.kind_id().into() {
      STAREQ | SLASHEQ | PERCENTEQ | DASHEQ | PLUSEQ | LTLTEQ | GTGTEQ | AMPEQ | PIPEEQ | CARETEQ | GTGTGTEQ | PLUSPLUS | DASHDASH => {
        stats.assignments += 1.;
      }
      ...
      VariableDeclarator | AssignmentExpression => {
        ...
        if let Some(right_operand) = node.child(2) {
          if matches!(right_operand.kind_id().into(), ParenthesizedExpression | UnaryExpression) {
      ...
      ReturnStatement => {
        ...
        if let Some(value) = node.child(1) {
          if matches!(value.kind_id().into(), ParenthesizedExpression | UnaryExpression) {
      ...
      TernaryExpression => {
        ...
        if let Some(expression) = node.child(2) {
          if matches!(expression.kind_id().into(), ParenthesizedExpression | UnaryExpression) {
