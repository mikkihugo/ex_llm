...
use mozilla_code_analysis::{LANG, *};

const REPO: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/", "repositories");
const SNAPSHOT_PATH: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/", "repositories/rca-output/snapshots");
...
pub fn compare_rca_output_with_files(repo_name: &str, include: &[&str], exclude: &[&str]) {
  ...
  let files_data = FilesData { include: gsbi.build().unwrap(), exclude: gsbe.build().unwrap(), paths: vec![Path::new(REPO).join(repo_name)] };
