...
impl RepoAnalyzer {
  ...
  pub async fn analyze(&self) -> Result<RepositoryAnalysis> {
    ...
    let package_discovery = PackageDiscovery::new(self.root_path.clone(), workspace_type.clone(), build_system.clone());
    ...
    let package_infos: Vec<PackageInfo> = packages.iter().map(|p| self.analyze_package(p)).collect::<Result<Vec<_>>>()?;

    // Phase 3: Tech Stack Analysis
    let project_tech_stack_analyzer = TechStackAnalyzer::new(self.root_path.clone());
    let project_tech_stacks = project_tech_stack_analyzer.analyze_project_tech_stacks(&packages).await?;
    ...
    let architecture_patterns = architecture_analyzer.detect_architecture_patterns(&packages, &infra_for_arch).await?;
    let domains = architecture_analyzer.infer_domains(&packages).await?;
    let dependency_graph = architecture_analyzer.build_dependency_graph(&packages).await?;
    let api_protocols = architecture_analyzer.detect_api_protocols(&packages, &project_tech_stacks).await?;
    let communication_patterns = architecture_analyzer.detect_communication_patterns(&infra_for_arch, &api_protocols).await?;
    let event_systems = architecture_analyzer.detect_event_systems(&packages).await?;
  ...
  fn build_directory_tree(&self, structure: &DirectoryStructure) -> DirectoryTree {
    DirectoryTree {
      ...
      apps_dir: if structure.has_apps_dir { Some(self.root_path.join("apps")) } else { None },
      packages_dir: if structure.has_packages_dir { Some(self.root_path.join("packages")) } else { None },
      services_dir: if structure.has_services_dir { Some(self.root_path.join("services")) } else { None },
      libs_dir: if structure.has_libs_dir { Some(self.root_path.join("libs")) } else { None },
      tools_dir: if structure.has_tools_dir { Some(self.root_path.join("tools")) } else { None },
      custom_dirs: Vec::new(),
    }
  }
    ...
    if language == &Language::Rust {
      if let Ok(cargo_toml) = std::fs::read_to_string(path.join("Cargo.toml")) {
        ...
        if cargo_toml.contains("[[bin]]") || path.join("src/main.rs").exists() {
          ...
          if cargo_toml.contains("tree-sitter") || cargo_toml.contains("syn") || cargo_toml.contains("parser") {
...
mod tests {
  ...
  async fn test_analyze_single_repo() {
    ...
    assert_eq!(result.workspace_type, crate::repository::workspace::WorkspaceType::SingleRepo);
  ...
  async fn test_analyze_monorepo() {
    ...
    assert_eq!(result.workspace_type, crate::repository::workspace::WorkspaceType::Monorepo);
    assert_eq!(result.build_system, crate::repository::workspace::BuildSystem::Moon);
  }
}
