...
use crate::dspy_data::Example;

static GLOBAL_PREFIX_REGISTRY: Lazy<RwLock<HashMap<std::thread::ThreadId, String>>> = Lazy::new(|| RwLock::new(HashMap::new()));
...
impl Default for DspySignature {
  fn default() -> Self {
    Self::new()
  }
}
...
impl DspySignature {
  pub fn new() -> Self {
    Self { name: String::new(), inputs: Vec::new(), outputs: Vec::new() }
  }
  ...
  pub fn from_string(signature_str: &str) -> Result<Self> {
    ...
    let mut builder = DspySignatureBuilder::new();
...
impl Default for DspySignatureBuilder {
  fn default() -> Self {
    Self::new()
  }
}
...
impl DspySignatureBuilder {
  ...
  pub fn input(mut self, name: &str, description: &str) -> Self {
    self.inputs.push(DspyField { name: name.to_string(), description: description.to_string() });
    self
  }

  pub fn output(mut self, name: &str, description: &str) -> Self {
    self.outputs.push(DspyField { name: name.to_string(), description: description.to_string() });
    self
  }

  pub fn build(self) -> DspySignature {
    DspySignature { name: if self.name.is_empty() { "DefaultSignature".to_string() } else { self.name }, inputs: self.inputs, outputs: self.outputs }
  }
}

impl Default for DspyInput {
  fn default() -> Self {
    Self::new()
  }
}
...
pub trait MetaSignature: Send + Sync {
  ...
  fn validate_inputs(&self, inputs: &serde_json::Value) -> Result<()> {
    ...
    if let Some(fields_obj) = input_fields.as_object() {
      for field_name in fields_obj.keys() {
        if inputs.get(field_name).is_none() {
          return Err(anyhow::anyhow!("Validation error: {} is missing", field_name));
        }
      }
    }
    Ok(())
  ...
  fn validate_outputs(&self, outputs: &serde_json::Value) -> Result<()> {
    ...
    if let Some(fields_obj) = output_fields.as_object() {
      for field_name in fields_obj.keys() {
        if outputs.get(field_name).is_none() {
          return Err(anyhow::anyhow!("Validation error: {} is missing", field_name));
        }
      }
    }
  ...
  fn update_prefix(&mut self, prefix: String) -> Result<()> {
    ...
    GLOBAL_PREFIX_REGISTRY.write().expect("prefix registry poisoned").insert(thread::current().id(), normalized_prefix.clone());
