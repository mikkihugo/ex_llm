...
impl PromptFeedbackCollector {
    ...
    pub async fn query_successful_prompts(
        ...
        for entry in std::fs::read_dir(feedback_dir)? {
            ...
            if let Ok(contents) = std::fs::read_to_string(entry.path()) {
                if let Ok(feedback) = serde_json::from_str::<PromptFeedback>(&contents) {
                    // Match similar tasks
                    if &feedback.prompt.task_type == task_type
                        && feedback.quality.to_score() >= 0.75
                    {
        ...
        results.sort_by(|a, b| {
            b.quality
                .to_score()
                .partial_cmp(&a.quality.to_score())
                .unwrap()
        });
    ...
    pub async fn get_statistics(&self) -> Result<PromptStatistics> {
        ...
        let mut total = 0;
        let mut successes = 0;
        let mut by_task_type: std::collections::HashMap<String, (usize, usize)> =
    ...
    pub async fn analyze_failures(&self) -> Result<Vec<FailureCodePattern>> {
        ...
        let mut patterns: std::collections::HashMap<String, FailureCodePattern> =
            std::collections::HashMap::new();
...
impl AgentFeedbackBuilder {
    ...
    fn build_metadata(
        &self,
        success: bool,
        relevant_categories: &[PromptCategory],
    ) -> FeedbackMetadata {
        FeedbackMetadata {
            ...
            successful_categories: if success {
                relevant_categories.to_vec()
            } else {
                Vec::new()
            },
            failed_categories: if !success {
                relevant_categories.to_vec()
            } else {
                Vec::new()
            },
    ...
    fn infer_failed_categories(&self, stage: &FailureStage) -> Vec<PromptCategory> {
        match stage {
            ...
            FailureStage::Compilation => {
                vec![PromptCategory::Dependencies, PromptCategory::Examples]
            }
