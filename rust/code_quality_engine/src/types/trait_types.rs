...
pub trait CodeAnalysisAgent {
  ...
  async fn analyze_code(&self, code: &str, context: &CodeContext) -> Result<CodeAnalysisResult, String>;

  /// Check if a field name would create duplicates
  async fn check_field_name(&self, name: &str, context: &CodeContext) -> Result<NameCheckResult, String>;

  /// Check if a function name would create duplicates
  async fn check_function_name(&self, name: &str, context: &CodeContext) -> Result<NameCheckResult, String>;

  /// Get suggestions for better naming
  async fn suggest_names(&self, base_name: &str, context: &CodeContext) -> Result<Vec<String>, String>;
