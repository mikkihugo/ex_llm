...
impl MLTrainer {
  ...
  pub fn new(fact_store: FactStorage, model_save_path: PathBuf) -> Self {
    Self { fact_store, model_save_path }
  }

  /// Train confidence predictor from FACT storage
  #[cfg(feature = "ml-analysis")]
  pub async fn train_confidence_predictor(&self, config: Option<TrainingConfig>) -> Result<TrainingMetrics> {
    ...
    if training_data.len() < 10 {
      return Err(anyhow::anyhow!("Insufficient training data: {} samples (minimum 10 required)", training_data.len()));
    }

    tracing::info!("Collected {} training samples from FACT storage", training_data.len());
    ...
    tracing::info!("Training complete: {} epochs, final validation loss: {:.4}", metrics.total_epochs, metrics.final_val_loss);
  ...
  async fn collect_training_data(&self) -> Result<Vec<(PromptFeatures, f64)>> {
    ...
    for fact in all_executions {
      if let PromptFactType::PromptExecution(exec) = fact {
        prompt_executions.entry(exec.prompt_id.clone()).or_insert_with(Vec::new).push(exec);
      }
    }
    ...
    for (prompt_id, executions) in prompt_executions {
      ...
      let avg_confidence = executions.iter().map(|e| e.confidence_score).sum::<f64>() / executions.len() as f64;
  ...
  pub async fn train_model_with_real_data(&self) -> Result<TrainingMetrics> {
    ...
    tracing::info!("Model training complete: best_val_loss={:.3}, final_train_loss={:.3}", metrics.best_val_loss, metrics.final_train_loss);

    Ok(metrics)
  }

  /// Train model with training data
  async fn train_model(&self, _training_data: &[(PromptFeatures, f64)]) -> Result<TrainingMetrics> {
  ...
  fn calculate_complexity(&self, executions: &[crate::prompt_tracking::PromptExecutionFact]) -> f64 {
    ...
    let variance = times.iter().map(|&t| (t as f64 - avg_time).powi(2)).sum::<f64>() / times.len() as f64;
  ...
  pub async fn suggest_training_config(&self) -> Result<TrainingConfig> {
    let all_executions = self.fact_store.query(FactQuery::RecentFeedback(std::time::Duration::from_secs(365 * 24 * 3600))).await?;

    let data_size = all_executions.len();

    let config = match data_size {
      0..=50 => {
        TrainingConfig { max_epochs: 50, batch_size: 8, learning_rate: 0.01, early_stopping_patience: 5, save_path: Some(self.model_save_path.clone()) }
      }
      51..=200 => {
        TrainingConfig { max_epochs: 100, batch_size: 16, learning_rate: 0.005, early_stopping_patience: 10, save_path: Some(self.model_save_path.clone()) }
      }
      _ => TrainingConfig { max_epochs: 200, batch_size: 32, learning_rate: 0.001, early_stopping_patience: 15, save_path: Some(self.model_save_path.clone()) },
    };

    tracing::info!("Suggested config for {} samples: {} epochs, batch_size={}, lr={}", data_size, config.max_epochs, config.batch_size, config.learning_rate);
  ...
  pub async fn should_retrain(&self) -> Result<bool> {
    ...
    if recent_executions.len() > 100 {
      tracing::info!("Found {} new training samples in the past week, retraining recommended", recent_executions.len());
      return Ok(true);
    }
...
mod tests {
  ...
  async fn test_ml_trainer_creation() {
    ...
    assert!(trainer.model_save_path.to_string_lossy().contains("test_model"));
  }
}
