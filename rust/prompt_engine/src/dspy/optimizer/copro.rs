...
impl InstructionGenerator {
  pub fn new() -> Self {
    Self { current_instruction: String::new(), optimized_instruction: String::new() }
  }
...
impl InstructionRefiner {
  fn new() -> Self {
    Self { instruction_history: Vec::new(), refined_instruction: String::new() }
  }
}
...
static INSTRUCTION_GENERATOR: Lazy<Predict> = Lazy::new(|| Predict::new(InstructionGenerator::new()));
...
impl COPRO {
  ...
  pub fn new(breadth: usize, depth: usize, init_temperature: f32, track_stats: bool) -> Self {
    Self { breadth, depth, init_temperature, track_stats, prompt_model: None }
  }
  ...
  async fn generate_candidates_batch(&self, basic_instruction: &str, count: usize) -> Result<Vec<(String, String)>> {
    ...
    for _ in 0..count {
      ...
      let future = async move {
        ...
        prediction.map(|p| {
          let instruction = p.data.get("optimized_instruction").and_then(|v| v.as_str()).unwrap_or(&inst).to_string();
          let prefix = p.data.get("output_prefix").and_then(|v| v.as_str()).unwrap_or("").to_string();
          (instruction, prefix)
        })
      };
  ...
  pub fn process_candidates(&self, candidates: Vec<Candidate>) -> Vec<Candidate> {
    ...
    // Log processing info
    tracing::debug!("Processed {} candidates after deduplication", deduplicated.len());

    deduplicated
  }

  /// Generate multiple candidate variations
  pub async fn generate_candidate_variations(&self, instruction: &str, count: usize) -> Result<Vec<Candidate>> {
...
impl Optimizer for COPRO {
  ...
  async fn compile<M: Module + Optimizable + Evaluator>(&self, module: &mut M, trainset: Vec<Example>) -> Result<()> {
    ...
    let mut stats = ProgramStats { results_best: HashMap::new(), results_latest: HashMap::new(), total_calls: 0 };
        ...
        let candidate_futures: Vec<_> = (0..self.breadth - 1)
          .map(|_| {
            ...
            async move {
              ...
              prediction.map(|p| {
                let instruction = p.data.get("optimized_instruction").and_then(|v| v.as_str()).unwrap_or(&inst).to_string();
                let prefix = p.data.get("output_prefix").and_then(|v| v.as_str()).unwrap_or(&basic_prefix).to_string();
                (instruction, prefix)
              })
            }
          })
    ...
    for d in 0..self.depth {
      ...
      for (p_i, (predictor_name, _, _)) in predictor_info.iter().enumerate() {
        ...
        let candidates_to_eval = if predictor_info.len() > 1 {
          ...
          all_candidates.get(predictor_name).cloned().ok_or_else(|| anyhow::anyhow!("Predictor {} not found in all_candidates", predictor_name))?
        } else {
          // Just evaluate latest candidates
          latest_candidates.get(predictor_name).cloned().ok_or_else(|| anyhow::anyhow!("Predictor {} not found in latest_candidates", predictor_name))?
        };
        ...
        for (c_i, (instruction, prefix)) in candidates_to_eval.iter().enumerate() {
          ...
          let score = if let Some(existing) = evaluated_candidates.get(predictor_name).and_then(|m| m.get(&key)) {
            ...
            if let Some(predictor_candidates) = evaluated_candidates.get_mut(predictor_name) {
              predictor_candidates.insert(key, Candidate { score, instruction: instruction.clone(), prefix: prefix.clone(), depth: d });
            } else {
          ...
          .max_by(|a, b| a.score.total_cmp(&b.score))
        {
          ...
          log::info!("Updating Predictor {} to best candidate with score {:.3}", predictor_name, best.score);
        }
      ...
      for (predictor_name, _, _) in &predictor_info {
        ...
        if let Ok(prediction) = prediction {
          ...
          let instructions = Self::normalize_lm_response(&prediction.data, "refined_instruction");
    ...
    for (predictor_name, candidates_map) in &evaluated_candidates {
      if let Some(best) = candidates_map.values().filter(|c| c.score.is_finite()).max_by(|a, b| a.score.total_cmp(&b.score)) {
    ...
    if let Some((best_predictor, best_candidate)) = best_overall.clone() {
      ...
      let individual_avg = evaluated_candidates
        .values()
        .filter_map(|candidates| candidates.values().filter(|c| c.score.is_finite()).max_by(|a, b| a.score.total_cmp(&b.score)).map(|c| c.score))
        .fold((0.0, 0), |(sum, count), score| (sum + score, count + 1));

      let individual_avg_score = if individual_avg.1 > 0 { individual_avg.0 / individual_avg.1 as f32 } else { 0.0 };
    ...
    if !applied_global {
      ...
      for (predictor_name, predictor) in module_predictors {
        if let Some(best) =
          evaluated_candidates.get(&predictor_name).and_then(|m| m.values().filter(|c| c.score.is_finite()).max_by(|a, b| a.score.total_cmp(&b.score)))
        {
          predictor.update_signature_instruction(best.instruction.clone())?;
...
mod macro_examples_tests {
  ...
  fn test_teleprompter_analysis() {
    ...
    let fields_obj = output_fields.as_object().expect("Output fields should be an object");
