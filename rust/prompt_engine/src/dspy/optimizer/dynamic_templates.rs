...
impl DynamicTemplateGenerator {
    ...
    pub async fn generate_template(
        ...
        let ai_context = self
            .generate_ai_context(phase, session_type, context, memory_context)
            .await?;
        ...
        let enhanced_content = self
            .enhance_with_ai_insights(&base_content, &ai_context)
            .await?;
    ...
    async fn generate_ai_context(
        ...
        let analysis_prompt =
            self.build_context_analysis_prompt(phase, session_type, context, memory_context);

        // For now, return a placeholder response
        // The actual LLM integration will be handled by the main sparc-engine
        let ai_response = format!(
            "AI Analysis for phase: {:?} with context: {}",
            phase, analysis_prompt
        );
    ...
    fn build_context_analysis_prompt(
        ...
        prompt.push_str("🧠 DYNAMIC TEMPLATE ANALYSIS\n\n");
        prompt.push_str(
            "Analyze the following context to generate insights for dynamic template creation:\n\n",
        );
        ...
        prompt.push_str(
            "1. DOMAIN_INSIGHTS: What domain-specific knowledge should influence the template?\n",
        );
        prompt.push_str(
            "2. OBSERVED_PATTERNS: What patterns do you see that could improve effectiveness?\n",
        );
        prompt.push_str(
            "3. IMPROVEMENT_SUGGESTIONS: How should the template be customized for this context?\n",
        );
        prompt
            .push_str("4. PROJECT_CONTEXT: What project-specific elements should be emphasized?\n");
        prompt.push_str(
            "5. LEARNED_PREFERENCES: What user/team preferences should be considered?\n\n",
        );
    ...
    fn create_base_template_content(
        &self,
        phase: &SPARCPhase,
        session_type: &SessionType,
    ) -> String {
    ...
    async fn enhance_with_ai_insights(
        &self,
        base_content: &str,
    ...
    fn apply_memory_patterns(
        ...
        for pattern in relevant_patterns.iter().filter(|p| p.success_score > 0.8) {
            ...
            if enhanced_content.contains("Consider:")
                && pattern.template_fragment.contains("Consider")
            {
                enhanced_content = enhanced_content.replace(
                    "Consider:",
                    &format!("Consider ({} pattern):", pattern.pattern_description),
                );
            }
        }
    ...
    fn get_relevant_patterns(
        ...
        phase: &SPARCPhase,
        session_type: &SessionType,
    ) -> Vec<SuccessCodePattern> {
        self.global_memory
            .iter()
            .filter(|pattern| {
                pattern.success_context.get("phase") == Some(&format!("{:?}", phase))
                    || pattern.success_context.get("session_type")
                        == Some(&format!("{:?}", session_type))
            })
            .cloned()
            .collect()
    }
    ...
    pub fn record_success(
        ...
        self.global_memory
            .sort_by(|a, b| b.success_score.partial_cmp(&a.success_score).unwrap());
    ...
    pub fn update_template_performance(&mut self, template_id: &str, new_score: f64) {
        if let Some(template) = self.template_cache.get_mut(template_id) {
            ...
            template.performance_score =
                alpha * new_score + (1.0 - alpha) * template.performance_score;
