...
impl Predict {
    ...
    fn get_global_settings_safe(&self) -> anyhow::Result<(Arc<dyn Adapter>, LM)> {
        ...
        let guard = GLOBAL_SETTINGS.try_read().map_err(|_| {
            anyhow::anyhow!("DSPy settings lock unavailable - extension may be initializing")
        })?;

        let settings = guard.as_ref().ok_or_else(|| {
            anyhow::anyhow!(
                "DSPy not configured - extension requires initialization via Moon config"
            )
        })?;
...
impl super::Predictor for Predict {
    ...
    async fn forward(&self, inputs: Example) -> anyhow::Result<Prediction> {
        ...
        self.execute_prediction_with_retry(adapter, &mut lm, inputs, 3)
            .await
    }
    ...
    async fn forward_with_config(
        ...
        self.execute_prediction_with_retry(Arc::new(ConversationHistoryAdapter), lm, inputs, 3)
            .await
    }
}
