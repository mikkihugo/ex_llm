...
impl ConfidenceScorer {
  pub fn new(fact_store: FactStorage) -> Self {
    ...
    Self { fact_store, neural_predictor }
  }

  /// Calculate confidence adjustment based on execution (with optional neural ML)
  pub async fn calculate_adjustment(&self, execution: &ExecutionResult) -> Result<ConfidenceAdjustment> {
    // Get current confidence
    let old_confidence = self.get_current_confidence(&execution.prompt_id).await?;

    // Extract features for ML prediction
    let features = self.extract_features(&execution.prompt_id, execution).await?;
    ...
    let (new_confidence, ml_confidence) = if let Some(predictor) = &self.neural_predictor {
      match predictor.predict(&features) {
        ...
        Err(_) => {
          ...
          (self.calculate_heuristic_confidence(execution, old_confidence), None)
        }
      }
    } else {
    ...
    // Generate reason
    let reason = self.generate_adjustment_reason(old_confidence, new_confidence, execution, ml_confidence);

    Ok(ConfidenceAdjustment { prompt_id: execution.prompt_id.clone(), old_confidence, new_confidence, adjustment_reason: reason, ml_confidence })
  }

  /// Extract features from execution history for ML prediction
  async fn extract_features(&self, prompt_id: &str, current_execution: &ExecutionResult) -> Result<PromptFeatures> {
    ...
    let success_rate = if execution_count > 0 { success_count as f64 / execution_count as f64 } else { current_execution.success_score };

    let avg_confidence = if execution_count > 0 { total_confidence / execution_count as f64 } else { current_execution.confidence };
  ...
  fn calculate_heuristic_confidence(&self, execution: &ExecutionResult, old_confidence: f64) -> f64 {
  ...
  pub async fn apply_adjustment(&self, prompt_id: &str, adjustment: ConfidenceAdjustment) -> Result<()> {
    // Store as feedback FACT
    let feedback = PromptFeedbackFact {
      prompt_id: prompt_id.to_string(),
      feedback_type: if adjustment.new_confidence > adjustment.old_confidence { FeedbackType::Quality } else { FeedbackType::Performance },
    ...
    self.fact_store.store(PromptFactType::PromptFeedback(feedback)).await?;
  ...
  async fn get_current_confidence(&self, prompt_id: &str) -> Result<f64> {
    ...
    Ok(if total_weight > 0.0 { total_confidence / total_weight } else { 0.5 })
  }

  /// Generate human-readable reason for adjustment
  fn generate_adjustment_reason(&self, old: f64, new: f64, execution: &ExecutionResult, ml_confidence: Option<f64>) -> String {
    ...
    if !execution.user_changes.is_empty() {
      reasons.push(format!("{} user modifications", execution.user_changes.len()));
    }

    if !execution.error_messages.is_empty() {
      reasons.push(format!("{} errors encountered", execution.error_messages.len()));
    }

    let method = if ml_confidence.is_some() { "ML-enhanced" } else { "Heuristic" };
    ...
    let negative_feedback_count = recent_feedback
      ...
      .filter(|f| {
        if let PromptFactType::PromptFeedback(feedback) = f {
          feedback.prompt_id == prompt_id && matches!(feedback.feedback_type, FeedbackType::Performance)
        } else {
          false
        }
      })
