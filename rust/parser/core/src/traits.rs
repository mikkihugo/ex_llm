...
use crate::{Class, Comment, Enum, Function, Import, LanguageMetrics, ParseError, AST};
...
pub trait LanguageParser: Send + Sync {
    ...
    /// Extract enum definitions from an AST
    fn get_enums(&self, _ast: &AST) -> Result<Vec<Enum>, ParseError> {
        Ok(Vec::new())
    }
...
pub trait ParserFactory: Send + Sync {
    ...
    fn create_language_parser(&self, language: &str)
        -> Result<Box<dyn LanguageParser>, ParseError>;

    /// Create a specialized parser
    fn create_specialized_parser(
        &self,
        parser_type: &str,
    ) -> Result<Box<dyn SpecializedParser>, ParseError>;
