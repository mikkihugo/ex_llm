...
type ProcDirPathsFunction<Config> = dyn Fn(&mut HashMap<String, Vec<PathBuf>>, &Path, &Config) + Send + Sync;
...
fn send_file<T>(path: PathBuf, cfg: &Arc<T>, sender: &JobSender<T>) -> Result<(), ConcurrentErrors> {
  sender.send(Some(JobItem { path, cfg: Arc::clone(cfg) })).map_err(|e| ConcurrentErrors::Sender(e.to_string()))
}
...
{
  let FilesData { mut paths, ref include, ref exclude } = files_data;
  ...
  for path in paths.drain(..) {
    ...
    if path.is_dir() {
      for entry in WalkDir::new(path).into_iter().filter_entry(|e| !is_hidden(e)) {
        ...
        if (include.is_empty() || include.is_match(&path)) && (exclude.is_empty() || !exclude.is_match(&path)) && path.is_file() {
          proc_dir_paths(&mut all_files, &path, cfg);
          send_file(path, cfg, sender)?;
        }
      }
    } else if (include.is_empty() || include.is_match(&path)) && (exclude.is_empty() || !exclude.is_match(&path)) && path.is_file() {
      proc_path(&path, cfg);
      send_file(path, cfg, sender)?;
    }
  }
...
impl<Config: 'static + Send + Sync> ConcurrentRunner<Config> {
    ...
    ProcFiles: 'static + Fn(PathBuf, &Config) -> std::io::Result<()> + Send + Sync,
  {
    ...
    Self { proc_files: Box::new(proc_files), proc_dir_paths: Box::new(null_proc_dir_paths), proc_path: Box::new(null_proc_path), num_jobs }
  }
  ...
  where
    ProcDirPaths: 'static + Fn(&mut HashMap<String, Vec<PathBuf>>, &Path, &Config) + Send + Sync,
  {
  ...
  pub fn run(self, config: Config, files_data: FilesData) -> Result<HashMap<String, Vec<PathBuf>>, ConcurrentErrors> {
    ...
    let producer = {
      ...
      match thread::Builder::new().name(String::from("Producer")).spawn(move || explore(files_data, &cfg, self.proc_dir_paths, self.proc_path, &sender)) {
    ...
    for i in 0..self.num_jobs {
      ...
      let t = match thread::Builder::new().name(format!("Consumer {i}")).spawn(move || {
    ...
    let all_files = match producer.join() {
      Ok(res) => res,
      Err(_) => {
        return Err(ConcurrentErrors::Producer("Child thread panicked".to_owned()));
      }
    };
    ...
    for receiver in receivers {
      if receiver.join().is_err() {
        return Err(ConcurrentErrors::Receiver("A thread used to process a file panicked".to_owned()));
      }
    }
