    ...
    pub fn analyze(&self, code: &str, file_path: &str) -> Vec<StructureOptimization> {
        ...
        for (line_num, line) in lines.iter().enumerate() {
            ...
            if self.has_excessive_nesting(code, line_num) {
                optimizations.push(StructureOptimization {
                    ...
                    description: "Excessive nesting detected - consider early returns or guard clauses".to_string(),
            ...
            if self.is_large_function(code, line_num) {
                optimizations.push(StructureOptimization {
                    ...
                    description: "Function is too large and should be split into smaller functions".to_string(),
                    current_structure: "Single large function with multiple responsibilities".to_string(),
                    suggested_structure: "Multiple focused functions with single responsibility".to_string(),
                    benefits: vec![
            ...
            if self.is_small_function(code, line_num) {
                optimizations.push(StructureOptimization {
                    ...
                    description: "Function is very small and might be better merged with caller".to_string(),
                    current_structure: "Multiple small functions with single line implementations".to_string(),
                    suggested_structure: "Inline simple functions or merge related functionality".to_string(),
                    benefits: vec![
        ...
        if self.is_large_class(code) {
            optimizations.push(StructureOptimization {
                ...
                description: "Class is too large and should be broken down into smaller classes".to_string(),
                current_structure: "Single large class with multiple responsibilities".to_string(),
                suggested_structure: "Multiple focused classes with single responsibility".to_string(),
                benefits: vec![
                    "Single responsibility principle".to_string(),
