...
impl MetricStats {
  fn from_values(values: &[f64], unit: MetricUnit) -> Self {
    if values.is_empty() {
      return Self { count: 0, sum: 0.0, min: 0.0, max: 0.0, avg: 0.0, p50: 0.0, p95: 0.0, p99: 0.0, unit };
    }
    ...
    Self { count, sum, min, max, avg, p50, p95, p99, unit }
  }
}
...
impl MetricsCollector {
  pub fn new() -> Self {
    Self { metrics: Arc::new(RwLock::new(HashMap::new())), start_time: Instant::now() }
  }
  ...
  pub fn record_with_metadata(&self, name: &str, value: f64, unit: MetricUnit, metadata: HashMap<String, String>) {
    let point = MetricPoint { timestamp: SystemTime::now(), value, unit, metadata };
  ...
  pub fn get_recent(&self, name: &str, count: usize) -> Vec<MetricPoint> {
    let metrics = self.metrics.read().unwrap();
    if let Some(points) = metrics.get(name) {
      let start_idx = if points.len() > count { points.len() - count } else { 0 };
  ...
  pub fn export_prometheus(&self) -> String {
    ...
    for (name, points) in metrics.iter() {
      ...
      let stats = MetricStats::from_values(&points.iter().map(|p| p.value).collect::<Vec<_>>(), points[0].unit.clone());

      // Export as Prometheus histogram
      output.push_str(&format!("# TYPE {} histogram\n", name.replace(".", "_")));
      output.push_str(&format!("{}_count {}\n", name.replace(".", "_"), stats.count));
      output.push_str(&format!("{}_sum {}\n", name.replace(".", "_"), stats.sum));
      output.push_str(&format!("{}_bucket{{le=\"{:.2}\"}} {}\n", name.replace(".", "_"), stats.p50, stats.count / 2));
      output.push_str(&format!("{}_bucket{{le=\"{:.2}\"}} {}\n", name.replace(".", "_"), stats.p95, (stats.count as f64 * 0.95) as usize));
...
impl MetricsSummary {
  pub fn new() -> Self {
    Self { metrics: HashMap::new(), uptime: Duration::default() }
  }
      ...
      if let Some(threshold) = thresholds.get_threshold(metric_name) {
        if stats.avg > threshold.warning_level {
          violations.push(ThresholdViolation {
            ...
            severity: if stats.avg > threshold.critical_level { Severity::Critical } else { Severity::Warning },
          });
        }
      }
    }
...
impl MetricThresholds {
  pub fn new() -> Self {
    ...
    thresholds.insert(
      "llm_request_duration".to_string(),
      Threshold {
