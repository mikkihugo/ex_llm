...
impl UniversalParserError {
  /// Check if this error is recoverable
  pub fn is_recoverable(&self) -> bool {
    match self {
      // Recoverable errors - can retry or use fallback
      UniversalParserError::Timeout { .. } => true,
      UniversalParserError::DependencyUnavailable { .. } => true,
      UniversalParserError::CacheError { .. } => true,
      UniversalParserError::TreeSitterError { .. } => true,
      UniversalParserError::TokeiError { .. } => true,
      UniversalParserError::RustCodeAnalysisError { .. } => true,
      UniversalParserError::IoError { .. } => true,

      // Non-recoverable errors - fundamental problems
      UniversalParserError::FileTooLarge { .. } => false,
      UniversalParserError::UnsupportedLanguage { .. } => false,
      UniversalParserError::ConfigurationError { .. } => false,
      UniversalParserError::MemoryLimitExceeded { .. } => false,
      UniversalParserError::SerializationError { .. } => false,

      // Analysis failed and generic could be either
      UniversalParserError::AnalysisFailed { .. } => true,
      UniversalParserError::Generic { .. } => true,
    }
  }
  ...
  pub fn severity(&self) -> ErrorSeverity {
    match self {
      UniversalParserError::Timeout { .. } => ErrorSeverity::Warning,
      UniversalParserError::FileTooLarge { .. } => ErrorSeverity::Error,
      UniversalParserError::UnsupportedLanguage { .. } => ErrorSeverity::Error,
      UniversalParserError::DependencyUnavailable { .. } => ErrorSeverity::Warning,
      UniversalParserError::AnalysisFailed { .. } => ErrorSeverity::Error,
      UniversalParserError::ConfigurationError { .. } => ErrorSeverity::Error,
      UniversalParserError::MemoryLimitExceeded { .. } => ErrorSeverity::Error,
      UniversalParserError::CacheError { .. } => ErrorSeverity::Warning,
      UniversalParserError::TreeSitterError { .. } => ErrorSeverity::Warning,
      UniversalParserError::TokeiError { .. } => ErrorSeverity::Warning,
      UniversalParserError::RustCodeAnalysisError { .. } => ErrorSeverity::Warning,
      UniversalParserError::IoError { .. } => ErrorSeverity::Error,
      UniversalParserError::SerializationError { .. } => ErrorSeverity::Error,
      UniversalParserError::Generic { .. } => ErrorSeverity::Info,
    }
  }
  ...
  pub fn category(&self) -> ErrorCategory {
    match self {
      UniversalParserError::Timeout { .. } => ErrorCategory::Performance,
      UniversalParserError::FileTooLarge { .. } => ErrorCategory::Resource,
      UniversalParserError::UnsupportedLanguage { .. } => ErrorCategory::Configuration,
      UniversalParserError::DependencyUnavailable { .. } => ErrorCategory::Dependency,
      UniversalParserError::AnalysisFailed { .. } => ErrorCategory::Analysis,
      UniversalParserError::ConfigurationError { .. } => ErrorCategory::Configuration,
      UniversalParserError::MemoryLimitExceeded { .. } => ErrorCategory::Resource,
      UniversalParserError::CacheError { .. } => ErrorCategory::Performance,
      UniversalParserError::TreeSitterError { .. } => ErrorCategory::Dependency,
      UniversalParserError::TokeiError { .. } => ErrorCategory::Dependency,
      UniversalParserError::RustCodeAnalysisError { .. } => ErrorCategory::Dependency,
      UniversalParserError::IoError { .. } => ErrorCategory::System,
      UniversalParserError::SerializationError { .. } => ErrorCategory::System,
      UniversalParserError::Generic { .. } => ErrorCategory::Unclassified,
    }
  }
...
/// Error implementation for UniversalParserError to enable anyhow compatibility
impl std::error::Error for UniversalParserError {
  fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
    None
  }
}

/// Display implementation for UniversalParserError to provide detailed error messages
impl std::fmt::Display for UniversalParserError {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    match self {
      UniversalParserError::Timeout { duration_ms } => {
        write!(f, "Analysis timeout after {duration_ms}ms - consider increasing timeout or optimizing input")
      }
      UniversalParserError::FileTooLarge { size_bytes, max_size_bytes } => {
        write!(f, "File too large: {size_bytes} bytes (max: {max_size_bytes} bytes) - consider splitting or excluding large files")
      }
      UniversalParserError::UnsupportedLanguage { language } => {
        write!(f, "Unsupported language: '{language}' - check available parsers or add custom parser")
      }
      UniversalParserError::DependencyUnavailable { dependency } => {
        write!(f, "Required dependency '{dependency}' unavailable - ensure dependency is installed and accessible")
      }
      UniversalParserError::AnalysisFailed { details } => {
        write!(f, "Analysis failed: {details} - check input validity and parser configuration")
      }
      UniversalParserError::ConfigurationError { message } => {
        write!(f, "Configuration error: {message} - verify parser settings and options")
      }
      UniversalParserError::MemoryLimitExceeded { requested_bytes, available_bytes } => {
        write!(
          f,
...
pub struct ErrorContext {
  /// The original error
  pub error: UniversalParserError,
...
impl ErrorContext {
  ...
  pub fn new(error: UniversalParserError) -> Self {
  ...
  pub fn recovery_strategy(&self) -> RecoveryStrategy {
    match &self.error {
      UniversalParserError::Timeout { .. } => RecoveryStrategy::Retry { max_attempts: 2, delay_ms: 1000 },
      UniversalParserError::DependencyUnavailable { .. } => RecoveryStrategy::Fallback,
      UniversalParserError::TreeSitterError { .. } => RecoveryStrategy::Fallback,
      UniversalParserError::TokeiError { .. } => RecoveryStrategy::Fallback,
      UniversalParserError::RustCodeAnalysisError { .. } => RecoveryStrategy::Fallback,
      UniversalParserError::CacheError { .. } => RecoveryStrategy::Skip,
      UniversalParserError::IoError { .. } => RecoveryStrategy::Retry { max_attempts: 3, delay_ms: 500 },
      UniversalParserError::AnalysisFailed { .. } => RecoveryStrategy::Fallback,
...
impl From<std::io::Error> for UniversalParserError {
  fn from(err: std::io::Error) -> Self {
    UniversalParserError::IoError { message: err.to_string() }
  }
}

impl From<serde_json::Error> for UniversalParserError {
  fn from(err: serde_json::Error) -> Self {
    UniversalParserError::SerializationError { message: err.to_string() }
  }
}

impl From<tree_sitter::LanguageError> for UniversalParserError {
  fn from(err: tree_sitter::LanguageError) -> Self {
    UniversalParserError::TreeSitterError { language: "language not specified".to_string(), message: err.to_string() }
  }
}
...
mod tests {
  ...
  fn test_error_classification() {
    let timeout_error = UniversalParserError::timeout(5000);
    assert!(timeout_error.is_recoverable());
    assert_eq!(timeout_error.severity(), ErrorSeverity::Warning);
    assert_eq!(timeout_error.category(), ErrorCategory::Performance);

    let unsupported_error = UniversalParserError::unsupported_language("unsupported language");
  ...
  fn test_error_context() {
    let error = UniversalParserError::analysis_failed("test failure");
  ...
  fn test_error_reporter() {
    ...
    let error1 = ErrorContext::new(UniversalParserError::timeout(1000)).with_parser("test-parser");
    let error2 = ErrorContext::new(UniversalParserError::unsupported_language("test")).with_parser("test-parser");
  ...
  fn test_error_conversions() {
    ...
    let universal_error: UniversalParserError = io_error.into();
    assert!(matches!(universal_error, UniversalParserError::IoError { .. }));

    let json_error = serde_json::from_str::<serde_json::Value>("{").unwrap_err();
    let universal_error: UniversalParserError = json_error.into();
    assert!(matches!(universal_error, UniversalParserError::SerializationError { .. }));
  }
}
