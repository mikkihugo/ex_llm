...
impl MetricsCollector {
  ...
  pub fn average(&self, metric: &str) -> Option<f64> {
    self.metrics.get(metric).map(|values| if values.is_empty() { 0.0 } else { values.iter().sum::<f64>() / values.len() as f64 })
  }
}
...
impl ExecutionTracker {
  pub fn new(fact_store: FactStorage) -> Self {
    Self { fact_store, metrics_collector: MetricsCollector::new() }
  }
  ...
  pub async fn track_execution(&mut self, execution: ExecutionResult) -> Result<String> {
    ...
    // Store FACT
    let fact_id = self.fact_store.store(PromptFactType::PromptExecution(fact)).await?;

    // Update metrics
    self.metrics_collector.record(&format!("execution_time_{}", execution.prompt_id), execution.duration.as_secs_f64());
    self.metrics_collector.record(&format!("success_rate_{}", execution.prompt_id), execution.success_score);
    self.metrics_collector.record(&format!("confidence_{}", execution.prompt_id), execution.confidence);
  ...
  async fn update_training_examples(&self, execution: ExecutionResult) -> Result<()> {
    ...
    metadata.insert("success_score".to_string(), execution.success_score.to_string());
    metadata.insert("confidence".to_string(), execution.confidence.to_string());
    metadata.insert("user_changes_count".to_string(), execution.user_changes.len().to_string());
    metadata.insert("context_hash".to_string(), self.hash_context(&execution.context));
    ...
    self.fact_store.store(PromptFactType::PromptExecution(training_fact)).await?;

    tracing::info!(
      "{} training example stored: {} (score: {:.2}, confidence: {:.2})",
      if execution.success_score > 0.9 { "Positive" } else { "Negative" },
      execution.prompt_id,
  ...
  pub async fn get_performance_summary(&self, prompt_id: &str) -> Result<PerformanceSummary> {
    ...
    Ok(PerformanceSummary {
      ...
      average_success_rate: if count > 0.0 { total_success / count } else { 0.0 },
      average_execution_time: if executions.is_empty() { Duration::ZERO } else { total_time / executions.len() as u32 },
      average_modifications: if executions.is_empty() { 0.0 } else { modification_count as f64 / count },
  ...
  fn calculate_confidence_trend(&self, executions: &[PromptFactType]) -> ConfidenceTrend {
    ...
    // Sort by timestamp
    let mut sorted_execs: Vec<_> = executions.iter().filter_map(|f| if let PromptFactType::PromptExecution(exec) = f { Some(exec) } else { None }).collect();
    ...
    let recent_avg = sorted_execs[sorted_execs.len() / 2..].iter().map(|e| e.confidence_score).sum::<f64>() / (sorted_execs.len() / 2) as f64;

    let older_avg = sorted_execs[..sorted_execs.len() / 2].iter().map(|e| e.confidence_score).sum::<f64>() / (sorted_execs.len() / 2) as f64;
