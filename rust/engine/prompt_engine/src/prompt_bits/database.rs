...
pub enum PromptBitSource {
  ...
  LLMGenerated { model: String, timestamp: chrono::DateTime<chrono::Utc> },
  CommunityContributed { author: String },
  Learned { from_feedback: String },
}
...
impl PromptBitDatabase {
  pub fn new(storage_path: PathBuf) -> Self {
    Self { storage_path, cache: HashMap::new(), llm_client: None }
  }
  ...
  pub async fn get_or_generate(&mut self, trigger: &PromptBitTrigger, category: &PromptBitCategory, context: &GenerationContext) -> Result<String> {
    // 1. Try to find in database
    let found_bit =
      if let Some(bits) = self.cache.get(trigger) { bits.iter().find(|b| &b.category == category).map(|b| (b.id.clone(), b.content.clone())) } else { None };
    ...
    if let Some(llm) = &self.llm_client {
      println!("⚠️  No prompt bit for {:?} + {:?}, generating with LLM...", trigger, category);
    ...
    for bits in self.cache.values_mut() {
      for bit in bits.iter_mut() {
        if bit.id == bit_id {
          ...
          let successes = (bit.success_rate * (total - 1.0)) + if success { 1.0 } else { 0.0 };
...
"#
        .to_string(),
        metadata: PromptBitMetadata { confidence: 0.95, last_updated: chrono::Utc::now(), versions: vec!["4.x".to_string()], related_bits: vec![] },
...
"#
        .to_string(),
        metadata: PromptBitMetadata { confidence: 0.95, last_updated: chrono::Utc::now(), versions: vec!["*".to_string()], related_bits: vec![] },
        source: PromptBitSource::Builtin,
        created_at: chrono::Utc::now(),
        usage_count: 0,
        success_rate: 1.0,
  ...
  pub fn get_statistics(&self) -> DatabaseStatistics {
    ...
    let llm_generated = self.cache.values().flatten().filter(|b| matches!(b.source, PromptBitSource::LLMGenerated { .. })).count();
    ...
    DatabaseStatistics { total_bits, builtin_bits: total_bits - llm_generated, llm_generated_bits: llm_generated, by_trigger }
  }
}
