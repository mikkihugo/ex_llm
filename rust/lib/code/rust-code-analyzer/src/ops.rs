...
fn compute_operators_and_operands<T: ParserTrait>(state: &mut State) {
  state.ops.operators =
    state.halstead_maps.operators.keys().filter(|k| !T::Checker::is_primitive(**k)).map(|k| T::Getter::get_operator_id_as_str(*k).to_owned()).collect();
  ...
  state.ops.operands = state.halstead_maps.operands.keys().map(|k| String::from_utf8(k.to_vec()).unwrap_or_else(|_| String::from("wrong_operands"))).collect();
}
...
pub fn operands_and_operators<'a, T: ParserTrait>(parser: &'a T, path: &'a Path) -> Option<Ops> {
  ...
  while let Some((node, level)) = stack.pop() {
    ...
    let new_level = if func_space {
      let state = State { ops: Ops::new::<T::Getter>(&node, code, kind), halstead_maps: HalsteadMaps::new(), primitive_types: HashSet::new() };
    ...
    if let Some(state) = state_stack.last_mut() {
      ...
      if T::Checker::is_primitive(node.kind_id()) {
        ...
        let primitive_string = String::from_utf8(code.to_vec()).unwrap_or_else(|_| String::from("primitive_type"));
...
mod tests {
  ...
  #[inline(always)]
  fn check_ops(lang: LANG, source: &str, file: &str, correct_operators: &mut [&str], correct_operands: &mut [&str]) {
  ...
  fn cpp_function_ops() {
    check_ops(
      ...
      "foo.c",
      &mut ["()", "{}", "int", "&", "=", "+", "/", ",", ";"],
      &mut ["main", "a", "b", "c", "avg", "scanf", "\"%d %d %d\"", "3", "printf", "\"avg = %d\""],
    );
  }
  ...
  fn javascript_ops() {
    check_ops(
      ...
      "foo.js",
      &mut ["()", "var", "let", "=", "+", "/", ",", ".", ";"],
      &mut ["a", "b", "c", "avg", "x", "1", "3", "5", "console.log", "console", "log", "\"{}\""],
    );
  }
  ...
  fn javascript_function_ops() {
    check_ops(
      ...
      "foo.js",
      &mut ["function", "()", "{}", "var", "let", "=", "+", "/", ",", ".", ";"],
      &mut ["main", "a", "b", "c", "avg", "x", "1", "3", "5", "console.log", "console", "log", "\"{}\""],
    );
  }
  ...
  fn typescript_ops() {
    check_ops(
      ...
      "var a, b, c, avg;
             ...
             console.log(\"{}\", avg);",
      "foo.ts",
      &mut ["()", "var", "let", "string", "number", "boolean", ":", "=", "+", "/", ",", ".", ";"],
      &mut ["a", "b", "c", "avg", "age", "name", "isUpdated", "32", "\"John\"", "true", "3", "5", "console.log", "console", "log", "\"{}\""],
    );
  }
  ...
  fn typescript_function_ops() {
    check_ops(
      ...
      "foo.ts",
      &mut ["function", "()", "{}", "var", "let", "string", "number", "boolean", ":", "=", "+", "/", ",", ".", ";"],
      &mut ["main", "a", "b", "c", "avg", "age", "name", "isUpdated", "32", "\"John\"", "true", "3", "5", "console.log", "console", "log", "\"{}\""],
    );
  }
  ...
  fn java_ops() {
    check_ops(
      ...
      "foo.java",
      &mut ["{}", "void", "()", "[]", ",", ";", "int", "=", "+", "/"],
      &mut ["Main", "main", "args", "a", "b", "c", "avg", "5", "3", "MessageFormat", "format", "\"{0}\""],
    );
  }
}
