  ...
  root.act_on_node(&mut |n| {
    if T::Checker::is_func(n) {
      ...
      if let Some(name) = T::Getter::get_func_name(n, code) {
        spans.push(FunctionSpan { name: name.to_string(), start_line, end_line, error: false });
      } else {
        spans.push(FunctionSpan { name: "".to_string(), start_line, end_line, error: true });
      }
    }
  });
...
fn dump_span(span: FunctionSpan, stdout: &mut StandardStreamLock, last: bool) -> std::io::Result<()> {
  // if !span.error {
  // return Ok(());
  // }
