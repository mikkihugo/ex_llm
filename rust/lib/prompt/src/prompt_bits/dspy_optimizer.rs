...
impl PromptBitDSPyOptimizer {
  ...
  pub async fn optimize_prompt_bit(&self, original_content: &str) -> Result<String> {
    ...
    let candidates = self.copro.generate_candidate_variations(original_content, 5).await?;
    ...
    let best_candidate = processed_candidates
      ...
      .unwrap_or_else(|| Candidate { score: 0.0, instruction: original_content.to_string(), prefix: "fallback".to_string(), depth: 0 });
  ...
  pub async fn optimize_bit(&self, bit: &StoredPromptBit, training_examples: Vec<OptimizationExample>) -> Result<OptimizedPromptBit> {
    // Build DSPy optimization task
    let task = DSPyOptimizationTask { original_content: bit.content.clone(), trigger: bit.trigger.clone(), category: bit.category.clone(), training_examples };
    ...
    // Select best variant
    let best = results.into_iter().max_by(|a, b| a.score.partial_cmp(&b.score).unwrap()).unwrap();
    ...
    Ok(OptimizedPromptBit {
      ...
      optimization_metadata: OptimizationMetadata {
        ...
        optimization_time_ms: std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_millis() as u64,
      },
    })
  }

  /// Generate N variants using DSPy
  async fn generate_variants(&self, task: &DSPyOptimizationTask) -> Result<Vec<PromptBitVariant>> {
    ...
    variants.push(PromptBitVariant { id: 1, content: self.make_concise(&task.original_content), rationale: "Made more concise".to_string() });

    // Variant 2: More detailed
    variants.push(PromptBitVariant { id: 2, content: self.make_detailed(&task.original_content), rationale: "Added more details".to_string() });
    ...
    if !task.training_examples.is_empty() {
      variants.push(PromptBitVariant { id: 4, content: self.learn_from_examples(task).await?, rationale: "Learned from successful examples".to_string() });
    }
  ...
  async fn evaluate_variants(&self, variants: Vec<PromptBitVariant>) -> Result<Vec<EvaluationResult>> {
    ...
    for variant in variants {
      ...
      results.push(EvaluationResult {
        ...
        score,
        metrics: EvaluationMetrics { success_count: 10, failure_count: 2, avg_execution_time_ms: 5000 },
      });
    }
  ...
  fn make_concise(&self, content: &str) -> String {
    ...
    content.lines().filter(|line| !line.trim().is_empty()).map(|line| line.trim()).collect::<Vec<_>>().join("\n")
  }
  ...
  async fn learn_from_examples(&self, task: &DSPyOptimizationTask) -> Result<String> {
    ...
    let successful_examples: Vec<_> = task.training_examples.iter().filter(|ex| ex.success).collect();
    ...
    let top_patterns: Vec<_> = pattern_vec.iter().take(5).map(|(pattern, count)| ((*pattern).clone(), **count)).collect();

    // Generate optimized instruction based on patterns
    let learned_instruction = if !top_patterns.is_empty() {
      let pattern_text = top_patterns.iter().map(|(pattern, count)| format!("{} ({}x)", pattern, count)).collect::<Vec<_>>().join(", ");

      format!("{}\n\nOptimized based on {} successful examples. Key patterns: {}", task.original_content, successful_examples.len(), pattern_text)
    } else {
      format!("{}\n\nOptimized based on {} successful examples", task.original_content, successful_examples.len())
    };
  ...
  pub async fn ab_test(&self, bit_a: &StoredPromptBit, bit_b: &StoredPromptBit, test_cases: Vec<TestCase>) -> Result<ABTestResult> {
    let mut a_wins = 0;
    let mut b_wins = 0;
    let mut ties = 0;
    ...
    Ok(ABTestResult { winner, a_wins, b_wins, ties, confidence: self.calculate_confidence(a_wins, b_wins, ties) })
  }

  async fn test_bit_on_case(&self, _bit: &StoredPromptBit, _test_case: &TestCase) -> Result<TestResult> {
  ...
  pub fn create_evaluation_task(&self, original_content: &str, trigger: PromptBitTrigger, category: PromptBitCategory) -> DSPyOptimizationTask {
  ...
  pub fn execute_optimization(&self, task: &DSPyOptimizationTask) -> Result<String> {
    ...
    let approach = match &task.trigger {
      PromptBitTrigger::Framework(framework) => format!("framework_optimization_{}", framework.to_lowercase()),
      PromptBitTrigger::Language(lang) => format!("language_optimization_{}", lang.to_lowercase()),
      PromptBitTrigger::BuildSystem(system) => format!("build_optimization_{}", system.to_lowercase()),
      PromptBitTrigger::Infrastructure(infra) => format!("infrastructure_optimization_{}", infra.to_lowercase()),
      PromptBitTrigger::CodePattern(pattern) => format!("pattern_optimization_{}", pattern.to_lowercase()),
      PromptBitTrigger::Custom(custom) => format!("custom_optimization_{}", custom.to_lowercase()),
    };
    ...
    Ok(format!("Executing {} optimization for {} domain. Content length: {} chars", approach, domain, task.original_content.len()))
  }

  /// Create evaluation result with comprehensive metrics
  #[allow(dead_code)]
  fn create_evaluation_result(&self, variant_id: usize, success_count: usize, failure_count: usize, avg_execution_time_ms: u64) -> EvaluationResult {
