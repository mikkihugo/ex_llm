...
impl fmt::Display for Stats {
  fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
    write!(f, "sum: {}, average: {}, min:{}, max: {}", self.cognitive(), self.cognitive_average(), self.cognitive_min(), self.cognitive_max())
  }
}
...
fn compute_booleans<T: std::cmp::PartialEq + std::convert::From<u16>>(node: &Node, stats: &mut Stats, typs1: T, typs2: T) {
  for child in node.children() {
    if typs1 == child.kind_id().into() || typs2 == child.kind_id().into() {
      stats.structural = stats.boolean_seq.eval_based_on_prev(child.kind_id(), stats.structural)
    }
  }
}
...
fn get_nesting_from_map(node: &Node, nesting_map: &mut HashMap<usize, (usize, usize, usize)>) -> (usize, usize, usize) {
...
fn increment_function_depth<T: std::cmp::PartialEq + std::convert::From<u16>>(depth: &mut usize, node: &Node, stop: T) {
...
impl Cognitive for PythonCode {
  fn compute(node: &Node, stats: &mut Stats, nesting_map: &mut HashMap<usize, (usize, usize, usize)>) {
    ...
    match node.kind_id().into() {
      ...
      BooleanOperator => {
        if node.count_specific_ancestors::<PythonParser>(|node| node.kind_id() == BooleanOperator, |node| node.kind_id() == Lambda) == 0 {
          stats.structural += node.count_specific_ancestors::<PythonParser>(
            |node| node.kind_id() == Lambda,
            |node| matches!(node.kind_id().into(), ExpressionList | IfStatement | ForStatement | WhileStatement),
          );
        }
      ...
      FunctionDefinition => {
        // Increase depth function nesting if needed
        increment_function_depth::<language_python::Python>(&mut depth, node, FunctionDefinition);
      }
      _ => {}
    }
