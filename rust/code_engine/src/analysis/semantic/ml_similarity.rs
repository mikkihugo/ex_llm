...
impl MLVectorizer {
  ...
  pub fn new_from_metadata(metadata_list: &[CodeMetadata], embedding_dim: usize) -> Result<Self> {
    ...
    Ok(Self { embeddings, vocabulary, embedding_dim, function_vocab, class_vocab, pattern_vocab, keyword_vocab })
  }
  ...
  fn tokenize(text: &str) -> Vec<String> {
    text
      ...
      .split_whitespace()
      .filter_map(|word| {
        let clean_word: String = word.chars().filter(|c| c.is_alphanumeric() || *c == '_').collect();
        if clean_word.len() >= 2 {
          Some(clean_word)
        } else {
          None
  ...
  fn extract_function_vocab(documents: &[(String, String, String)]) -> Vec<String> {
    documents
      .iter()
      .flat_map(|(_, text, _)| {
        text.split_whitespace().filter(|word| word.contains("fn") || word.contains("func") || word.contains("def")).map(|s| s.to_string())
      })
      .collect()
  }
  ...
  fn extract_class_vocab(documents: &[(String, String, String)]) -> Vec<String> {
    documents
      .iter()
      .flat_map(|(_, text, _)| {
        text.split_whitespace().filter(|word| word.contains("class") || word.contains("struct") || word.contains("interface")).map(|s| s.to_string())
      })
      .collect()
  }
  ...
  fn extract_pattern_vocab(documents: &[(String, String, String)]) -> Vec<String> {
    documents
      .iter()
      .flat_map(|(_, text, _)| {
        text.split_whitespace().filter(|word| word.contains("pattern") || word.contains("factory") || word.contains("builder")).map(|s| s.to_string())
      })
      .collect()
  }
  ...
  fn extract_keyword_vocab(documents: &[(String, String, String)]) -> Vec<String> {
    let keywords = ["async", "await", "impl", "trait", "enum", "match", "if", "for", "while"];
    documents.iter().flat_map(|(_, text, _)| text.split_whitespace().filter(|word| keywords.contains(word)).map(|s| s.to_string())).collect()
  }

  /// Create advanced vector representation
  pub fn create_advanced_vector(&self, id: String, text: &str, code_type: String) -> Result<MLCodeVector> {
    ...
    Ok(MLCodeVector { id, vector: vector.to_vec(), magnitude, code_type, semantic_features: Self::extract_semantic_features(text) })
  }
  ...
  fn extract_semantic_features(text: &str) -> SemanticFeatures {
    ...
    let unique_words = tokens.iter().collect::<std::collections::HashSet<_>>().len();
    let avg_word_length = if word_count > 0 { tokens.iter().map(|w| w.len()).sum::<usize>() as f32 / word_count as f32 } else { 0.0 };

    SemanticFeatures { word_count, unique_words, avg_word_length, complexity_score: Self::calculate_complexity(&tokens) }
  }
  ...
  fn calculate_complexity(tokens: &[String]) -> f32 {
    ...
    let complexity_count = tokens.iter().filter(|token| complexity_keywords.contains(&token.as_str())).count();

    complexity_count as f32 / tokens.len().max(1) as f32
  }

  /// Find similar vectors using advanced algorithms
  pub fn find_similar_advanced(&self, query: &MLCodeVector, candidates: &[(String, String, String)], top_k: usize) -> Vec<(String, f32)> {
    let mut similarities = Vec::new();

    for (id, text, _) in candidates {
      if id != &query.id {
        if let Ok(candidate) = self.create_advanced_vector(id.clone(), text, "function".to_string()) {
  ...
  fn advanced_similarity(a: &MLCodeVector, b: &MLCodeVector) -> f32 {
    ...
    let cosine_sim = if a.magnitude > 0.0 && b.magnitude > 0.0 {
      let dot_product: f32 = a.vector.iter().zip(b.vector.iter()).map(|(x, y)| x * y).sum();
  ...
  fn semantic_similarity(a: &SemanticFeatures, b: &SemanticFeatures) -> f32 {
    let word_count_sim = 1.0 - ((a.word_count as f32 - b.word_count as f32).abs() / (a.word_count.max(b.word_count) as f32 + 1.0));
    let unique_words_sim = 1.0 - ((a.unique_words as f32 - b.unique_words as f32).abs() / (a.unique_words.max(b.unique_words) as f32 + 1.0));
...
impl MultiModalFusion {
  pub fn new() -> Self {
    Self { text_weight: 0.6, structure_weight: 0.25, complexity_weight: 0.15 }
  }

  /// Fuse multiple modalities into final vector
  pub fn fuse_multimodal(&self, advanced_vector: &MLCodeVector, _text: &str, _file_path: &std::path::PathBuf) -> FusionResult {
...
mod tests {
  ...
  fn test_advanced_vectorizer() {
    let documents = vec![
      ("func1".to_string(), "function login user password".to_string(), "function".to_string()),
      ("func2".to_string(), "class User authentication method".to_string(), "class".to_string()),
    ];

    let vectorizer = AdvancedVectorizer::new(&documents, 50).unwrap();
    let vector = vectorizer.create_advanced_vector("test".to_string(), "user login auth", "function".to_string()).unwrap();
  ...
  fn test_semantic_features() {
    let features = AdvancedVectorizer::extract_semantic_features("function complex_algorithm if for while");
