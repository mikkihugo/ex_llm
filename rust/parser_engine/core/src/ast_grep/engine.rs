...
use super::languages::{
    BashLang, CLang, CppLang, DockerfileLang, ElixirLang, ErlangLang, GleamLang, GoLang, JavaLang, JavaScriptLang, JsonLang, LuaLang,
    MarkdownLang, PythonLang, RustLang, SqlLang, TomlLang, TypeScriptLang, YamlLang,
};
...
impl AstGrep {
    ...
    pub fn search(&mut self, source: &str, pattern: &Pattern) -> Result<Vec<SearchResult>, AstGrepError> {
        self.ensure_within_limits(source.len())?;

        let start = Instant::now();
        let results = self.dispatch_search(source, pattern)?;

        self.update_stats(results.len(), 1, source.len(), start.elapsed().as_millis() as u64);
        Ok(results)
    }
    ...
    pub fn search_multiple(
        ...
        self.update_stats(total_matches, patterns.len(), source.len(), start.elapsed().as_millis() as u64);
        Ok(results)
    }
    ...
    pub fn replace_with_options(
        ...
        let start = Instant::now();
        let (result, replacements) = self.dispatch_replace(source, pattern, replacement, options)?;

        self.update_stats(replacements, 1, source.len(), start.elapsed().as_millis() as u64);
        Ok(result)
    }
    ...
    pub fn lint(
        ...
        self.update_stats(violations.len(), rules.len(), source.len(), start.elapsed().as_millis() as u64);
        Ok(violations)
    }

    fn dispatch_search(&self, source: &str, pattern: &Pattern) -> Result<Vec<SearchResult>, AstGrepError> {
        match self.language {
    ...
    fn dispatch_search_multiple(
        ...
        match self.language {
            ...
            SupportedLanguage::JavaScript => self.search_multiple_with::<JavaScriptLang>(source, patterns),
            SupportedLanguage::TypeScript => self.search_multiple_with::<TypeScriptLang>(source, patterns),
            ...
            SupportedLanguage::Markdown => self.search_multiple_with::<MarkdownLang>(source, patterns),
            SupportedLanguage::Dockerfile => self.search_multiple_with::<DockerfileLang>(source, patterns),
    ...
    fn dispatch_replace(
        ...
        match self.language {
            SupportedLanguage::Elixir => self.replace_with::<ElixirLang>(source, pattern, replacement, options),
            SupportedLanguage::Erlang => self.replace_with::<ErlangLang>(source, pattern, replacement, options),
            SupportedLanguage::Gleam => self.replace_with::<GleamLang>(source, pattern, replacement, options),
            SupportedLanguage::Rust => self.replace_with::<RustLang>(source, pattern, replacement, options),
            SupportedLanguage::JavaScript => self.replace_with::<JavaScriptLang>(source, pattern, replacement, options),
            SupportedLanguage::TypeScript => self.replace_with::<TypeScriptLang>(source, pattern, replacement, options),
            SupportedLanguage::Python => self.replace_with::<PythonLang>(source, pattern, replacement, options),
            SupportedLanguage::Java => self.replace_with::<JavaLang>(source, pattern, replacement, options),
            SupportedLanguage::Go => self.replace_with::<GoLang>(source, pattern, replacement, options),
            SupportedLanguage::C => self.replace_with::<CLang>(source, pattern, replacement, options),
            SupportedLanguage::Cpp => self.replace_with::<CppLang>(source, pattern, replacement, options),
            SupportedLanguage::Bash => self.replace_with::<BashLang>(source, pattern, replacement, options),
            SupportedLanguage::Json => self.replace_with::<JsonLang>(source, pattern, replacement, options),
            SupportedLanguage::Yaml => self.replace_with::<YamlLang>(source, pattern, replacement, options),
            SupportedLanguage::Lua => self.replace_with::<LuaLang>(source, pattern, replacement, options),
            SupportedLanguage::Markdown => self.replace_with::<MarkdownLang>(source, pattern, replacement, options),
            SupportedLanguage::Dockerfile => self.replace_with::<DockerfileLang>(source, pattern, replacement, options),
            SupportedLanguage::Toml => self.replace_with::<TomlLang>(source, pattern, replacement, options),
            SupportedLanguage::Sql => self.replace_with::<SqlLang>(source, pattern, replacement, options),
        }
    }
        ...
        L: LanguageExt + Default + Clone,
    {
        ...
        let root = CoreAst::try_new(source, lang.clone())
        ...
        L: LanguageExt + Default + Clone,
    {
        ...
        let matcher = compile_pattern(pattern, &lang)?;
        let constraint_patterns = compile_constraints(pattern, &lang)?;
        let mut root = CoreAst::try_new(source, lang.clone())
            .map_err(|err| AstGrepError::ParseError {
                language: self.language.primary_alias().to_string(),
                details: err,
            })?;
        ...
        for node_match in root.root().find_all(matcher.clone()) {
            ...
            if let Ok(mut replacement_str) = String::from_utf8(edit.inserted_text.clone()) {
                ...
                if options.preserve_comments {
                    if let Some((comment_indent, comment_body)) = extract_trailing_comment(&original_text) {
                        if !replacement_str.trim_end().ends_with(comment_body.trim_end()) {
    ...
    fn lint_with<L>(&self, source: &str, rules: &[LintRule]) -> Result<Vec<LintViolation>, AstGrepError>
    where
        L: LanguageExt + Default + Clone,
    {
...
where
    L: LanguageExt + Clone,
{
    CorePattern::try_new(pattern.as_str(), lang.clone()).map_err(|source| AstGrepError::PatternCompilation {
        pattern: pattern.as_str().to_string(),
        source,
    })
}

fn compile_constraints<L>(pattern: &Pattern, lang: &L) -> Result<HashMap<String, CorePattern>, AstGrepError>
where
    L: LanguageExt + Clone,
{
    let mut compiled = HashMap::new();
    for (var, constraint) in &pattern.constraints {
        let core = CorePattern::try_new(constraint, lang.clone()).map_err(|source| AstGrepError::ConstraintCompilation {
            variable: var.clone(),
            source,
        })?;
    ...
    L: LanguageExt,
{
    ...
    let captures_map: HashMap<String, String> = {
        ...
        map.into_iter().collect::<BTreeMap<_, _>>().into_iter().collect()
    };
...
fn surrounding_lines(source: &str, range: std::ops::Range<usize>, before: usize, after: usize) -> (String, String) {
