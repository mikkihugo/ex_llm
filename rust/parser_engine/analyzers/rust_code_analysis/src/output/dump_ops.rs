...
fn dump_space(space: &Ops, prefix: &str, last: bool, stdout: &mut StandardStreamLock) -> std::io::Result<()> {
...
fn dump_space_ops(ops: &Ops, prefix: &str, last: bool, stdout: &mut StandardStreamLock) -> std::io::Result<()> {
  dump_ops_values("operators", &ops.operators, prefix, last, stdout)?;
  dump_ops_values("operands", &ops.operands, prefix, last, stdout)
}

fn dump_ops_values(name: &str, ops: &[String], prefix: &str, last: bool, stdout: &mut StandardStreamLock) -> std::io::Result<()> {
  let (pref_child, pref) = if last { ("   ", "`- ") } else { ("|  ", "|- ") };
