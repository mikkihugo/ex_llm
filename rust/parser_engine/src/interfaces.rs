...
use crate::{dependencies::UniversalDependencies, errors::PolyglotCodeParserError, languages::ProgrammingLanguage, AnalysisResult};
...
impl ParserFactory {
  ...
  /// parser crates are separate dependencies that implement PolyglotCodeParser.
  /// In practice, parsers should be instantiated directly from their respective crates.
  pub async fn create_parser(
    &self,
    language: ProgrammingLanguage,
  ) -> Result<Box<dyn PolyglotCodeParser<Config = HashMap<String, serde_json::Value>, ProgrammingLanguageSpecific = serde_json::Value>>> {
    ...
    match language {
      ...
      _ => Err(PolyglotCodeParserError::UnsupportedLanguage { language: language.to_string() }.into()),
    }
  }
  ...
  pub async fn create_parser_for_file<P: AsRef<Path>>(
    &self,
    file_path: P,
  ) -> Result<(ProgrammingLanguage, Box<dyn PolyglotCodeParser<Config = HashMap<String, serde_json::Value>, ProgrammingLanguageSpecific = serde_json::Value>>)> {
    ...
    if language == ProgrammingLanguage::LanguageNotSupported {
      ...
      if detected_language == ProgrammingLanguage::LanguageNotSupported {
        return Err(PolyglotCodeParserError::UnsupportedLanguage { language: "language not recognized or supported".to_string() }.into());
      }
...
#[async_trait]
impl PolyglotCodeParser for UniversalProgrammingLanguageParser {
...
// #[async_trait]
// impl<T> PolyglotCodeParser for SparcParserAdapter<T>
// where
...
// UniversalDependencies::new_with_config(PolyglotCodeParserFrameworkConfig::default()).unwrap()
// }),
// config: HashMap::new(),
// }
// }
