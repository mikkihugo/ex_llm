            ...
            for capture in m.captures {
                if capture.index == 0 {
                    let content = capture
                        ...
                        .to_owned();
                    let line = capture.node.start_position().row + 1;
                    let column = capture.node.start_position().column + 1;
...
impl GleamParser {
    /// Perform comprehensive BEAM analysis for Gleam code
    pub fn analyze_beam_patterns(&self, ast: &AST) -> Result<BeamAnalysisResult, ParseError> {
        ...
        let language_features = self.analyze_gleam_features(ast)?;
    ...
    /// Analyze Gleam-specific features (type system, functional programming, etc.)
    fn analyze_gleam_features(&self, _ast: &AST) -> Result<GleamFeatures, ParseError> {
        let type_analysis = TypeAnalysis {
            custom_types: Vec::new(), // TODO: implement custom type detection
            type_aliases: Vec::new(), // TODO: implement type alias detection
            type_features: std::collections::HashMap::new(),
        };
        ...
        let modern_features = ModernFeatures {
            language_features: std::collections::HashMap::new(),
        };
