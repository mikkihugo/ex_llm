...
async fn test_templates_load() -> Result<()> {
    ...
    assert!(!results.is_empty(), "Templates should have loaded and detected something");

    println!("✅ Templates loaded: {} technologies detected", results.len());
    Ok(())
}
...
async fn test_confidence_levels() -> Result<()> {
    ...
    for result in &results {
        assert!(result.confidence >= 0.0 && result.confidence <= 1.0,
            "Confidence must be 0-1, got: {}", result.confidence);
