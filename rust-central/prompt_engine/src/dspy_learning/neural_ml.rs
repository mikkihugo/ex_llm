    ...
    fn train_step(&self, inputs: &Tensor, targets: &Tensor, learning_rate: f32) -> Result<f32> {
        ...
        for var in var_map.all_vars().iter() {
            if let Some(grad) = grads.get(var) {
                ...
                let momentum = momentum_map
                    .entry(var_key.clone())
                    .or_insert_with(|| Tensor::zeros_like(var).unwrap());
                ...
                *momentum = momentum
                    .mul(&Tensor::new(beta1, &self.device).unwrap())?
                    .add(&grad.mul(&Tensor::new(1.0 - beta1, &self.device).unwrap())?)?;

                // Update velocity (exponential moving average of squared gradients)
                let grad_squared = grad.sqr()?;
                *velocity = velocity
                    .mul(&Tensor::new(beta2, &self.device).unwrap())?
                    .add(&grad_squared.mul(&Tensor::new(1.0 - beta2, &self.device).unwrap())?)?;

                // Bias correction
                let momentum_corrected = momentum
                    .div(&Tensor::new(1.0 - beta1.powi(*step as i32), &self.device).unwrap())?;
                let velocity_corrected = velocity
                    .div(&Tensor::new(1.0 - beta2.powi(*step as i32), &self.device).unwrap())?;

                // Compute update with adaptive learning rate
                let velocity_sqrt = velocity_corrected.sqrt()?;
                let denominator =
                    velocity_sqrt.add(&Tensor::new(epsilon, &self.device).unwrap())?;
                let update = momentum_corrected.div(&denominator)?;
                let scaled_update =
                    update.mul(&Tensor::new(learning_rate, &self.device).unwrap())?;
...
impl ConfidencePredictor {
                ...
                if let Some(path) = model_path {
                    if path.exists() {
                        if let Err(e) = model.load(path) {
                            tracing::warn!(
                                "Failed to load model weights: {}, using fresh model",
                                e
                            );
                        } else {
    ...
    pub fn train(
        &mut self,
        training_data: Vec<(PromptFeatures, f64)>,
        ...
        let model = self
            .model
            .as_ref()
            .ok_or_else(|| anyhow::anyhow!("No model available for training"))?;
        ...
        tracing::info!(
            "Training neural confidence predictor: {} train samples, {} validation samples",
            train_set.len(),
            val_set.len()
        );
        ...
        for epoch in 0..config.max_epochs {
            ...
            let val_loss = if !val_set.is_empty() {
                self.evaluate_batch(model, val_set)?
            } else {
                avg_train_loss
            };
