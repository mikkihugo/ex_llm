...
impl QueryExecutor {
  ...
  pub fn new() -> Self {
    ...
    executor.register_default_strategies();
  ...
  /// Execute a query
  #[must_use]
  pub fn execute(&self, query: &str) -> serde_json::Value {
    // Analyze query to determine strategy
    let strategy_name = self.analyze_query(query);

    self.strategies.get(&strategy_name).map_or_else(
      || {
        // Default processing
        serde_json::json!({
            "query": query,
      ...
      |strategy| Self::apply_strategy(strategy, query),
    )
  }
  ...
  fn analyze_query(&self, query: &str) -> String {
    ...
    for (name, strategy) in &self.strategies {
      if regex_lite::Regex::new(&strategy.pattern).is_match(&lower_query) {
        return name.clone();
      }
    }
...
mod tests {
  ...
  fn test_query_executor() {
    ...
    let result = executor.execute("What is the weather today?");
    assert_eq!(result["type"], "question");

    // Test command processing
    let result = executor.execute("Create a new document");
    assert_eq!(result["type"], "command");

    // Test analysis processing
    let result = executor.execute("Analyze the sales data");
