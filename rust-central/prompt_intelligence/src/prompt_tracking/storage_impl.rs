...
impl PromptTrackingStorage {
  ...
  pub fn new_global() -> Result<Self> {
    ...
    let global_cache = dirs::cache_dir().ok_or_else(|| anyhow::anyhow!("Could not find cache directory"))?.join("sparc-engine").join("global");
    ...
    Ok(Self { redb, json_dir, cache: Arc::new(RwLock::new(LruCache::new(1000))) })
  }
  ...
  pub async fn store(&self, fact: PromptFactType) -> Result<String> {
    ...
    match fact {
      ...
      PromptFactType::DetectedFramework(framework) => {
        self.store_json(&id, "detected_framework", &framework).await?;
      }
    }
  ...
  async fn store_feedback(&self, id: &str, feedback: PromptFeedbackFact) -> Result<()> {
    ...
    // Update index
    self.update_index("feedback", id, &feedback.prompt_id).await?;
  ...
  async fn store_context(&self, id: &str, context: ContextSignatureFact) -> Result<()> {
    ...
    self.update_index("context", id, &context.project_type).await?;
  ...
  async fn store_json<T: serde::Serialize>(&self, id: &str, category: &str, data: &T) -> Result<()> {
  ...
  async fn get_by_id(&self, id: &str) -> Result<Vec<PromptFactType>> {
    ...
    // Try JSON storage
    for category in &["code_index", "project_tech_stack", "patterns", "evolutions", "ab_tests"] {
