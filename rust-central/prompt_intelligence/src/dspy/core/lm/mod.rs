...
impl Default for SparcConfig {
  fn default() -> Self {
    Self { api_key: String::new(), model: "claude-3-sonnet".to_string(), base_url: None }
  }
}
...
pub async fn execute_ai_prompt<F, Fut>(context: &AIContext, conversation: &ConversationHistory, llm_executor: F) -> Result<Message>
where
  F: FnOnce(&str, &str) -> Fut,
  Fut: std::future::Future<Output = Result<String>>,
{
  // Convert conversation to prompt
  let prompt = conversation.messages.iter().map(|msg| format!("{}: {}", msg.role, msg.content)).collect::<Vec<_>>().join("\n");

  // Use the provided LLM executor function
  let response = llm_executor(&prompt, &context.model).await?;

  Ok(Message { role: "assistant".to_string(), content: response })
}
...
impl DirectAILM {
  ...
  pub fn new(session_id: String, config: SparcConfig) -> Self {
    Self { session_id, config, history: Vec::new() }
  }
  ...
  pub async fn call(&mut self, messages: ConversationHistory, signature: &str) -> Result<(Message, LanguageModelUsageMetrics)> {
    ...
    let context = AIContext { model: self.config.model.clone(), temperature: 0.1, max_tokens: 4096 };

    // Use existing execute_ai_prompt function with a placeholder executor
    let prompt = messages.messages.iter().map(|msg| format!("{}: {}", msg.role, msg.content)).collect::<Vec<_>>().join("\n");

    // TODO: Replace with actual LLM client call
    let response = format!("Response from {} for prompt: {}", context.model, prompt);
    let response_len = response.len();

    let message = Message { role: "assistant".to_string(), content: response };
    ...
    // Record in history
    self.history.push(LMResponse { chat: messages, output: message.clone(), config: self.config.clone(), signature: signature.to_string() });
  ...
  fn convert_chat_to_prompt(&self, chat: &ConversationHistory, signature: &str) -> String {
    ...
    for message in &chat.messages {
      match message {
        Message { role, content } if role == "system" => prompt_parts.push(format!("System: {}", content)),
        Message { role, content } if role == "user" => prompt_parts.push(format!("User: {}", content)),
        Message { role, content } if role == "assistant" => prompt_parts.push(format!("Assistant: {}", content)),
...
impl DummyLM {
  ...
  pub async fn call(&mut self, messages: ConversationHistory, signature: &str, prediction: String) -> Result<(Message, LanguageModelUsageMetrics)> {
    ...
    let enhanced_prediction = self.generate_contextual_response(&messages, signature, &prediction);

    self.history.push(LMResponse {
      chat: messages.clone(),
      output: Message { role: "assistant".to_string(), content: enhanced_prediction.clone() },
      config: self.config.clone(),
      signature: signature.to_string(),
    });

    Ok((
      Message { role: "assistant".to_string(), content: enhanced_prediction },
  ...
  fn generate_contextual_response(&self, messages: &ConversationHistory, signature: &str, base_prediction: &str) -> String {
    // Analyze the conversation context
    let last_message = messages.messages.last().map(|m| m.content.as_str()).unwrap_or("");
