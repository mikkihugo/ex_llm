...
impl PromptSelector {
  ...
  pub fn new(fact_store: FactStorage) -> Self {
    ...
    Self { fact_store, embedder }
  }

  /// Select optimal prompt for task and context (with neural similarity if available)
  pub async fn select_optimal_prompt(&mut self, task: &Task, context: &RepositoryContext, candidates: Vec<PromptBit>) -> Result<SelectedPrompt> {
    // 1. Generate context signature
    let signature = self.analyze_context(task, context).await?;

    // 2. Query similar successful executions
    let similar_executions = self.fact_store.query(FactQuery::Similar(signature.clone())).await?;

    // 3. Score each candidate based on historical performance
    let mut scored_candidates = Vec::new();
    for candidate in candidates {
      let score = self.score_candidate(&candidate, &signature, &similar_executions).await?;
    ...
    if let Some((best_prompt, score)) = scored_candidates.first() {
      ...
      let reasoning = self.generate_reasoning(task, context, best_prompt, &similar_executions).await?;
      ...
      Ok(SelectedPrompt {
        ...
        similar_executions: similar_executions
          .iter()
          .filter_map(|f| if let PromptFactType::PromptExecution(exec) = f { Some(exec.prompt_id.clone()) } else { None })
          .collect(),
        similarity_scores,
      })
    } else {
  ...
  async fn analyze_context(&self, task: &Task, context: &RepositoryContext) -> Result<ContextSignatureFact> {
    ...
    let fingerprint = format!("{}-{}-{}", context.workspace_type, context.build_system, context.languages.join(","));

    Ok(ContextSignatureFact {
      signature_hash: fingerprint,
      project_tech_stack: [context.languages.clone(), context.frameworks.clone(), context.databases.clone()].concat(),
  ...
  async fn score_candidate(&self, candidate: &PromptBit, signature: &ContextSignatureFact, similar_executions: &[PromptFactType]) -> Result<ScoredPrompt> {
    let mut score = ScoredPrompt { prompt_id: candidate.id.clone(), historical_score: 0.0, relevance_score: 0.0, confidence: 0.0 };
    ...
    let matching_tech = candidate_tech.iter().filter(|t| signature_tech.contains(t)).count();
  ...
  fn extract_tech_from_prompt(&self, prompt: &PromptBit) -> Vec<String> {
    ...
    let known_tech = ["rust", "typescript", "javascript", "python", "react", "nextjs", "express", "postgres", "mongodb", "redis", "kafka", "nats"];
  ...
  async fn generate_reasoning(&self, task: &Task, context: &RepositoryContext, selected: &PromptBit, similar_executions: &[PromptFactType]) -> Result<String> {
    // Use Chain of Thought to generate reasoning
    let mut reasoning = format!("Selected prompt '{}' for task '{}' because:\n", selected.id, task.description);

    // Add historical performance reason
    let exec_count =
      similar_executions.iter().filter(|f| if let PromptFactType::PromptExecution(exec) = f { exec.prompt_id == selected.id } else { false }).count();

    if exec_count > 0 {
      reasoning.push_str(&format!("- It has been successfully used {} times in similar contexts\n", exec_count));
    }

    // Add tech stack match reason
    let prompt_tech = self.extract_tech_from_prompt(selected);
    let matching_tech: Vec<_> =
      prompt_tech.iter().filter(|t| context.languages.contains(t) || context.frameworks.contains(t) || context.databases.contains(t)).collect();

    if !matching_tech.is_empty() {
      reasoning.push_str(&format!("- It matches your tech stack: {}\n", matching_tech.iter().map(|t| t.as_str()).collect::<Vec<_>>().join(", ")));
    }

    // Add confidence reason
    reasoning.push_str(&format!("- Confidence level: {:.1}%", selected.metadata.confidence * 100.0));
  ...
  pub async fn score_prompts(&self, prompt_ids: Vec<String>) -> Result<Vec<ScoredPrompt>> {
    ...
    for prompt_id in prompt_ids {
      ...
      let historical_score = self.get_historical_performance(&prompt_id).await.unwrap_or(0.5);
      ...
      let scored_prompt = ScoredPrompt::new(prompt_id, historical_score, relevance_score, confidence);
...
impl ScoredPrompt {
  ...
  pub fn new(prompt_id: String, historical_score: f64, relevance_score: f64, confidence: f64) -> Self {
    Self { prompt_id, historical_score, relevance_score, confidence }
  }
