...
use crate::{
  prompt_bits::{
    database::{PromptBitCategory, PromptBitMetadata, PromptBitSource, PromptBitTrigger, StoredPromptBit},
    types::*,
  },
...
impl PromptBitAssembler {
  ...
  pub fn new_with_llm(analysis: RepositoryAnalysis, llm_interface: Box<dyn std::fmt::Debug + Send + Sync>) -> Self {
    Self {
      analysis,
      detected_frameworks: Vec::new(),
      project_tech_stack_facts: Vec::new(),
  ...
  pub async fn new_with_framework_detection(analysis: RepositoryAnalysis, project_path: &Path) -> Result<Self> {
    ...
    let project_tech_stack_facts = Self::get_project_tech_stack_facts_from_engine(&detected_frameworks).await?;

    Ok(Self { analysis, detected_frameworks, project_tech_stack_facts, framework_prompt_bits, llm_interface: None })
  }
  ...
  pub async fn new_with_framework_detection_and_llm(
    ...
    let framework_prompt_bits = Self::load_framework_prompt_bits_with_llm(&detected_frameworks, &llm_interface).await?;

    // Get tech stack facts from the engine
    let project_tech_stack_facts = Self::get_project_tech_stack_facts_from_engine(&detected_frameworks).await?;

    Ok(Self { analysis, detected_frameworks, project_tech_stack_facts, framework_prompt_bits, llm_interface: Some(llm_interface) })
  }
  ...
  async fn detect_frameworks_from_engine(project_path: &Path) -> Result<Vec<DetectedFramework>> {
    ...
    let output =
      std::process::Command::new("sparc-engine").arg("detect-frameworks").arg(project_path.to_string_lossy().as_ref()).arg("--format=json").output()?;

    if !output.status.success() {
      return Err(anyhow::anyhow!("Framework detection failed: {}", String::from_utf8_lossy(&output.stderr)));
    }
    ...
    let frameworks = detection_result["frameworks"]
      ...
      .map(|f| DetectedFramework {
        ...
        framework_type: f["framework_type"].as_str().unwrap_or("unknown").to_string(),
        confidence: f["confidence"].as_f64().unwrap_or(0.0),
        detection_method: f["detection_method"].as_str().unwrap_or("unknown").to_string(),
        detected_files: f["detected_files"].as_array().unwrap_or(&vec![]).iter().map(|file| file.as_str().unwrap_or("").to_string()).collect(),
      })
      .collect();
  ...
  async fn load_framework_prompt_bits(frameworks: &[DetectedFramework]) -> Result<std::collections::HashMap<String, Vec<StoredPromptBit>>> {
  ...
  async fn generate_prompt_bits_with_detector_llm(framework: &DetectedFramework) -> Result<Vec<StoredPromptBit>> {
  ...
  pub fn generate_contextual_prompt(&self, base_prompt: &str) -> String {
    let tech_context = self.project_tech_stack_facts.iter().map(|fact| format!("- {}: {}", fact.technology, fact.version)).collect::<Vec<_>>().join("\n");
  ...
  async fn get_fallback_prompt_bits(framework: &DetectedFramework) -> Result<Vec<StoredPromptBit>> {
  ...
  /// Get tech stack facts from the engine
  async fn get_project_tech_stack_facts_from_engine(frameworks: &[DetectedFramework]) -> Result<Vec<ProjectTechStackFact>> {
  ...
  pub fn get_all_framework_prompt_bits(&self) -> &std::collections::HashMap<String, Vec<StoredPromptBit>> {
  ...
  pub fn generate(&self, task: TaskType) -> Result<GeneratedPrompt> {
    ...
    Ok(GeneratedPrompt { task_type: task, content, categories, confidence, timestamp: chrono::Utc::now(), repo_fingerprint })
  }
  ...
  fn analyze_repository_structure(&self, service_name: &str) -> String {
    match self.analysis.workspace_type {
      WorkspaceType::Monorepo => match &self.analysis.build_system {
        ...
        BuildSystem::Npm | BuildSystem::Pnpm | BuildSystem::Yarn => format!("./packages/{}/", service_name),
        BuildSystem::Maven | BuildSystem::Gradle => format!("./modules/{}/", service_name),
        BuildSystem::Other(name) if name.eq_ignore_ascii_case("moon") || name.eq_ignore_ascii_case("lerna") => {
          format!("./packages/{}/", service_name)
        }
        BuildSystem::Other(name) if name.eq_ignore_ascii_case("nx") => format!("./apps/{}/", service_name),
  ...
  fn generate_dependencies(&self) -> String {
    ...
    let shared: Vec<_> = packages
      .iter()
      .filter(|p| {
        p.name.contains("foundation") || p.name.contains("shared") || p.name.contains("common") || p.name.contains("core") || p.name.contains("database")
      })
      .collect();
    ...
    let auth_existing: Vec<_> = auth_packages.iter().filter(|p| p.name.contains("auth") || p.name.contains("user")).collect();
  ...
  fn generate_naming(&self, _service_name: &str) -> String {
    ...
    section.push_str(&format!("Use **{}** (detected from {} existing packages)\n\n", convention, examples_packages.len()));

    let examples: Vec<_> = examples_packages.iter().take(examples_packages.len().min(3)).map(|p| format!("`{}`", p.name)).collect();
  ...
  fn generate_infrastructure_integration(&self) -> String {
    ...
    for db in &self.analysis.databases {
      if let DatabaseSystem::PostgreSQL = db {
        ...
        let packages: Vec<Package> = vec![];
        if let Some(db_pkg) = packages.iter().find(|p| p.name.contains("database") || p.name.contains("db")) {
          section.push_str(&format!("Use connection from: `{}` at {}\n\n", db_pkg.name, db_pkg.path.display()));
        } else {
  ...
  fn generate_code_examples(&self) -> String {
    ...
    if let Some(first_pkg) = packages.first() {
      if first_pkg.primary_language == Language::Rust {
        ...
        section.push_str("async fn login(body: web::Json<LoginRequest>) -> impl Responder {\n");
