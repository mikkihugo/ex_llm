...
fn dump_space(space: &FuncSpace, prefix: &str, last: bool, stdout: &mut StandardStreamLock) -> std::io::Result<()> {
...
fn dump_metrics(metrics: &CodeMetrics, prefix: &str, last: bool, stdout: &mut StandardStreamLock) -> std::io::Result<()> {
  let (pref_child, pref) = if last { ("   ", "`- ") } else { ("|  ", "|- ") };
...
fn dump_cognitive(stats: &cognitive::Stats, prefix: &str, last: bool, stdout: &mut StandardStreamLock) -> std::io::Result<()> {
  let (pref_child, pref) = if last { ("   ", "`- ") } else { ("|  ", "|- ") };
...
fn dump_cyclomatic(stats: &cyclomatic::Stats, prefix: &str, last: bool, stdout: &mut StandardStreamLock) -> std::io::Result<()> {
  let (pref_child, pref) = if last { ("   ", "`- ") } else { ("|  ", "|- ") };
...
fn dump_halstead(stats: &halstead::Stats, prefix: &str, last: bool, stdout: &mut StandardStreamLock) -> std::io::Result<()> {
  let (pref_child, pref) = if last { ("   ", "`- ") } else { ("|  ", "|- ") };
  ...
  dump_value("estimated program length", stats.estimated_program_length(), &prefix, false, stdout)?;
...
fn dump_loc(stats: &loc::Stats, prefix: &str, last: bool, stdout: &mut StandardStreamLock) -> std::io::Result<()> {
  let (pref_child, pref) = if last { ("   ", "`- ") } else { ("|  ", "|- ") };
...
fn dump_nom(stats: &nom::Stats, prefix: &str, last: bool, stdout: &mut StandardStreamLock) -> std::io::Result<()> {
  let (pref_child, pref) = if last { ("   ", "`- ") } else { ("|  ", "|- ") };
...
fn dump_mi(stats: &mi::Stats, prefix: &str, last: bool, stdout: &mut StandardStreamLock) -> std::io::Result<()> {
  let (pref_child, pref) = if last { ("   ", "`- ") } else { ("|  ", "|- ") };
  ...
  dump_value("mi_visual_studio", stats.mi_visual_studio(), &prefix, true, stdout)
}

fn dump_nargs(stats: &nargs::Stats, prefix: &str, last: bool, stdout: &mut StandardStreamLock) -> std::io::Result<()> {
  let (pref_child, pref) = if last { ("   ", "`- ") } else { ("|  ", "|- ") };
...
fn dump_nexits(stats: &exit::Stats, prefix: &str, last: bool, stdout: &mut StandardStreamLock) -> std::io::Result<()> {
...
fn dump_abc(stats: &abc::Stats, prefix: &str, last: bool, stdout: &mut StandardStreamLock) -> std::io::Result<()> {
  let (pref_child, pref) = if last { ("   ", "`- ") } else { ("|  ", "|- ") };
  ...
  dump_value("assignments", stats.assignments_sum(), &prefix, false, stdout)?;
...
fn dump_wmc(stats: &wmc::Stats, prefix: &str, last: bool, stdout: &mut StandardStreamLock) -> std::io::Result<()> {
  ...
  dump_value("interfaces", stats.interface_wmc_sum(), &prefix, false, stdout)?;
  dump_value("total", stats.total_wmc(), &prefix, true, stdout)
}

fn dump_npm(stats: &npm::Stats, prefix: &str, last: bool, stdout: &mut StandardStreamLock) -> std::io::Result<()> {
  if stats.is_disabled() {
    return Ok(());
  }
  ...
  dump_value("interfaces", stats.interface_npm_sum(), &prefix, false, stdout)?;
  dump_value("total", stats.total_npm(), &prefix, false, stdout)?;
  dump_value("average", stats.total_coa(), &prefix, true, stdout)
}

fn dump_npa(stats: &npa::Stats, prefix: &str, last: bool, stdout: &mut StandardStreamLock) -> std::io::Result<()> {
  if stats.is_disabled() {
    return Ok(());
  }
  ...
  dump_value("interfaces", stats.interface_npa_sum(), &prefix, false, stdout)?;
  dump_value("total", stats.total_npa(), &prefix, false, stdout)?;
  dump_value("average", stats.total_cda(), &prefix, true, stdout)
}

fn dump_value(name: &str, val: f64, prefix: &str, last: bool, stdout: &mut StandardStreamLock) -> std::io::Result<()> {
  let pref = if last { "`- " } else { "|- " };
