...
#[inline(always)]
fn get_regex<'a>(once_lock: &OnceLock<Regex>, line: &'a [u8], regex: &'a str) -> Option<regex::bytes::Captures<'a>> {
  once_lock.get_or_init(|| Regex::new(regex).unwrap()).captures_iter(line).next()
}
...
pub fn guess_language<'a, P: AsRef<Path>>(buf: &[u8], path: P) -> (Option<LANG>, &'a str) {
  let ext = path.as_ref().extension().map(|e| e.to_str().unwrap()).map(|e| e.to_lowercase()).unwrap_or_else(|| "".to_string());
  ...
  if let Some(lang_ext) = from_ext {
    if let Some(lang_mode) = from_mode {
      if lang_ext == lang_mode {
        (Some(lang_mode), fake::get_true(&ext, &mode).unwrap_or_else(|| lang_mode.get_name()))
      } else {
        // we should probably rely on extension here
        (Some(lang_ext), lang_ext.get_name())
      }
    } else {
      (Some(lang_ext), fake::get_true(&ext, &mode).unwrap_or_else(|| lang_ext.get_name()))
    }
...
#[cfg(test)]
pub(crate) fn check_func_space<T: crate::ParserTrait, F: Fn(crate::FuncSpace)>(source: &str, filename: &str, check: F) {
...
#[cfg(test)]
pub(crate) fn check_metrics<T: crate::ParserTrait>(source: &str, filename: &str, check: fn(crate::CodeMetrics) -> ()) {
...
mod tests {
  ...
  fn test_guess_language() {
    ...
    assert_eq!(guess_language(buf, "foo.py"), (Some(LANG::Python), "python"));
    ...
    assert_eq!(guess_language(buf, "foo.mm"), (Some(LANG::Cpp), "obj-c/c++"));
  }
}
