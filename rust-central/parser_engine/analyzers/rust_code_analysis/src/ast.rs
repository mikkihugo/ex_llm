...
impl AstNode {
  pub fn new(r#type: &'static str, value: String, span: Span, children: Vec<AstNode>) -> Self {
    Self { r#type, value, span, children }
  }
}
  ...
  loop {
    ...
    if cursor.goto_first_child() {
      ...
      loop {
        ...
        if let Some(node) = T::Checker::get_ast_node(&ts_node, code, child_stack.pop().unwrap(), span, comment) {
...
impl Callback for AstCallback {
  ...
  fn call<T: ParserTrait>(cfg: Self::Cfg, parser: &T) -> Self::Res {
    AstResponse { id: cfg.id, root: build(parser, cfg.span, cfg.comment) }
  }
}
