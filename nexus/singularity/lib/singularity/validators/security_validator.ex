defmodule Singularity.Validators.SecurityValidator do
  ...
  def validate(code, opts \\ []) when is_binary(code) do
    ...
    violations = check_sasl_security(code, violations, opts)
    violations = check_authentication_patterns(code, violations, opts)
  ...
  defp check_sasl_security(code, violations, opts) do
    # Check for SASL security violations
    sasl_violations = [
      {~r/:crypto\.hash_equals/i, "Missing crypto.hash_equals for password comparison"},
      {~r/String\.to_atom.*user/i, "Potential atom exhaustion via user input"},
      {~r/:erlang\.binary_to_term.*user/i, "Unsafe deserialization of user data"},
      {~r/Repo\.query.*\$/i, "Potential SQL injection in database queries"}
    ]

    Enum.reduce(sasl_violations, violations, fn {pattern, message}, acc ->
      if Regex.match?(pattern, code) do
        acc ++ ["SASL Security: #{message}"]
      else
        acc
      end
    end)
  ...
  defp check_authentication_patterns(code, violations, opts) do
    # Check for authentication security patterns
    auth_violations = [
      {~r/password.*=.*"[^"]*"/i, "Hardcoded password detected"},
      {~r/api_key.*=.*"[^"]*"/i, "Hardcoded API key detected"},
      {~r/secret.*=.*"[^"]*"/i, "Hardcoded secret detected"},
      {~r/token.*=.*"[^"]*"/i, "Hardcoded token detected"},
      {~r/auth.*=.*"[^"]*"/i, "Hardcoded authentication credential detected"}
    ]

    Enum.reduce(auth_violations, violations, fn {pattern, message}, acc ->
      if Regex.match?(pattern, code) do
        acc ++ ["Authentication Security: #{message}"]
      else
        acc
      end
    end)
