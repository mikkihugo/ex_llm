defmodule Singularity.PromptEngine do
  ...
  def load_and_optimize_template(template_name) do
    if :erlang.function_exported(:prompt_engine, :load_and_optimize_template, 1) do
  ...
  def generate_prompt_from_template(template_name, input) when is_binary(input) do
    if :erlang.function_exported(:prompt_engine, :generate_prompt_from_template, 2) do
  ...
  def generate_prompt_from_template(template_name, input) when is_map(input) do
    generate_prompt_from_template(template_name, Map.get(input, :context, inspect(input)))
  end
  ...
  def sync_template_from_global(template_name) do
    if :erlang.function_exported(:prompt_engine, :sync_template_from_global, 1) do
  ...
  def invalidate_template(template_name) do
    if :erlang.function_exported(:prompt_engine, :invalidate_template, 1) do
      :prompt_engine.invalidate_template(template_name)
    else
      # Fallback: Clear from local cache
      Logger.debug("prompt_engine NIF not available, clearing local cache")
      cache_clear()
      {:ok, :invalidated}
    end
  end
