defmodule Singularity.BuildInfo do
  ...
  def version do
    case Application.spec(:singularity, :vsn) do
      ...
      _ -> 
        ...
        root = Path.join([__DIR__, "..", "..", "..", "..", "VERSION"])
        |> Path.expand()
  ...
  defp read_git_commit do
    repo_root = find_repo_root(__DIR__)
    case System.cmd("git", ["rev-parse", "--short", "HEAD"], cd: repo_root, stderr_to_stdout: true) do
  ...
  defp read_git_tag do
    repo_root = find_repo_root(__DIR__)
    case System.cmd("git", ["describe", "--tags", "--exact-match", "HEAD"], cd: repo_root, stderr_to_stdout: true) do
