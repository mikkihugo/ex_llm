...
This project keeps the most valuable reliability patterns from the pre-PGFlow codebase. The
following modules were ported from the old `master` branch and adapted to the current `nexus`
layout so we can retain the hardening efforts while using PostgreSQL/PGMQ + PGFlow:
...
- `Singularity.Infrastructure.PidManager` – “adopt vs. kill” process management for local
  services such as PostgreSQL and the PGFlow notifier.
- `Singularity.Infrastructure.Overseer` – centralized status poller for PGFlow workflow
...
- `Singularity.Infrastructure.Resilience` – retry, timeout, bulkhead, and circuit breaker helpers
  used across PGFlow pipelines and integration points.
- `Singularity.Monitoring.AgentTaskTracker` – telemetry-based task lifecycle broadcasts the
  dashboards rely on.
- `Singularity.ProcessRegistry` – LiveDashboard-friendly keywords for PGFlow supervision trees
  while still exposing the actual registry used by agents (via `{:via, Registry, ...}` tuples).

PGFlow workflows now lean on these primitives: the embedding and architecture training
...
talk to PostgreSQL, PGFlow queues, or GPU-backed Axon trainers.

Integration tests in `test/integration/infrastructure_patterns_test.exs` exercise the core
behaviour of these modules to ensure the cherry-picked logic remains healthy as PGFlow evolves.
