defmodule CentralCloud.Models.MLComplexityTrainer do
  ...
  defp build_dnn_model do
    ...
    # Output layer for complexity score (0.0 to 1.0)
    output = hidden2
  ...
  defp prepare_training_data_axon(training_data) do
    ...
    features = Enum.map(training_data, fn sample ->
      [
        ...
        sample.user_satisfaction,
        sample.completion_time,
        sample.context_length,
        sample.model_capabilities,
        sample.task_type_score,
        sample.success_rate,
        sample.quality_score
      ]
    end)

    labels = Enum.map(training_data, fn sample ->
      [sample.actual_complexity]
    end)

    # Convert to Nx tensors
    x_train = Nx.tensor(features)
    y_train = Nx.tensor(labels)
  ...
  defp train_model_axon(model, x_train, y_train) do
    try do
      ...
      loop = Axon.Loop.trainer(loss_fn, optimizer, :mean_squared_error)
