defmodule CentralCloud.Engines.EmbeddingEngine do
  ...
  def embed_text(text, opts \\ []) do
    ...
    with :ok <- QuantumFlow.Notifications.send_with_notify("embedding.request", request, CentralCloud.Repo, expect_reply: false) do
  ...
  defp wait_for_embedding_response(timeout) do
    ...
    case QuantumFlow.Notifications.send_with_notify("embedding.request", %{"request_id" => request_id}, CentralCloud.Repo, expect_reply: true, timeout: timeout) do
      {:ok, response} ->
        # Update tracking
        :ets.insert(:embedding_requests, {request_id, %{
          timestamp: DateTime.utc_now(),
