defmodule CentralCloud.Evolution.Consensus.Engine do
  ...
  CentralCloud.Evolution.Consensus.Engine:
    ...
    execute_if_consensus/1:
      ...
      - broadcasts change via ex_pgflow
  ...
  - **DO NOT** broadcast without ex_pgflow - durable delivery required
  ...
  governance, ex_pgflow_broadcast, change_execution, collective_intelligence,
  ...
  via ex_pgflow for synchronized execution.
  ...
  defp broadcast_for_voting(change_id, code_change, metadata) do
    # In production: Publish to ex_pgflow queue "evolution_voting_requests"
    Logger.info("[ConsensusEngine] Broadcasting for voting",
      change_id: change_id,
      change_type: code_change[:change_type]
    )

    # TODO: Implement ex_pgflow broadcast
    :ok
  end
  ...
  defp broadcast_change(change_id, code_change, metadata) do
    ...
    # In production: Publish to ex_pgflow queue "evolution_approved_changes"
