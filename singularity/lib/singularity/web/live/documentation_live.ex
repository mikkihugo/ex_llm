defmodule Singularity.Web.DocumentationLive do
    ...
    case DocumentationPipeline.run_incremental_pipeline(modified_files) do
      {:ok, :incremental_pipeline_started} ->
        socket =
          ...
          |> put_flash(
            :info,
            "Incremental documentation upgrade started for #{length(modified_files)} files"
          )
          |> assign(:upgrade_in_progress, true)
      ...
      {:error, reason} ->
        socket =
          put_flash(socket, :error, "Failed to start incremental pipeline: #{inspect(reason)}")
  ...
  def handle_event("schedule_automatic_upgrades", %{"interval" => interval}, socket) do
    ...
    case DocumentationPipeline.schedule_automatic_upgrades(interval_minutes) do
      :ok ->
        socket =
          put_flash(
            socket,
            :info,
            "Automatic upgrades scheduled every #{interval_minutes} minutes"
          )
      ...
      {:error, reason} ->
        socket =
          put_flash(socket, :error, "Failed to schedule automatic upgrades: #{inspect(reason)}")
  ...
  def handle_event("run_self_awareness", _params, socket) do
    case Singularity.SelfImprovingAgent.run_self_awareness_pipeline() do
      {:ok, :pipeline_started} ->
        socket =
          put_flash(
            socket,
            :info,
            "Self-awareness pipeline started - analyzing codebase, detecting issues, and generating fixes (with emergency Claude CLI fallback)"
          )
      ...
      {:error, reason} ->
        socket =
          put_flash(socket, :error, "Failed to start self-awareness pipeline: #{inspect(reason)}")
  ...
  defp update_status(socket) do
    ...
    pipeline_status =
      case DocumentationPipeline.get_pipeline_status() do
        {:ok, status} -> status
        {:error, _reason} -> %{}
      end
