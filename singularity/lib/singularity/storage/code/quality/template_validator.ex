defmodule Singularity.Code.Quality.TemplateValidator do
  ...
  def validate(code, quality_template, language, _opts \\ []) do
    ...
    with {:ok, temp_file} <- write_temp_file(code, language),
      ...
      Logger.info(
        "Code validation: #{if compliant, do: "PASS", else: "FAIL"} (score: #{Float.round(score, 2)})"
      )
  ...
  defp check_documentation(parse_result, requirements) do
    ...
    if doc_req == %{} do
      ...
      violations =
        if doc_req["moduledoc"]["required"] && !has_moduledoc do
      ...
      violations =
        if doc_req["doc"]["required_for"] && !has_function_docs do
  ...
  defp check_observability(code, requirements) do
    ...
    cond do
      telemetry_req && logging_req ->
        ...
        violations =
          if !has_telemetry, do: ["Missing telemetry events" | violations], else: violations

        violations =
          if !has_logging, do: ["Missing Logger statements" | violations], else: violations
