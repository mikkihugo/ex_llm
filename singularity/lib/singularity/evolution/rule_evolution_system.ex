defmodule Singularity.Evolution.RuleEvolutionSystem do
  ...
  **Quorum Gate** - Threshold before publishing (default: 0.85):
  ...
  {:ok, published} = RuleEvolutionSystem.publish_confident_rules(min_confidence: 0.90)
  ...
  ## Example
      ...
      iex> RuleEvolutionSystem.analyze_and_propose_rules(
      ...>   task_type: :architect,
      ...>   time_range: :last_week
      ...> )
      {:ok, [
  ...
  ## Returns
  - `{:ok, published_count}` - Number of rules published
  - `{:error, reason}` - Publishing failed

  ## Example

      iex> RuleEvolutionSystem.publish_confident_rules(min_confidence: 0.90)
  ...
  def publish_confident_rules(opts \\ []) do
    ...
    try do
      case analyze_and_propose_rules(%{}, min_confidence: 0.0, limit: limit + 10) do
        {:ok, all_rules} ->
          ...
          published_count =
            rules_to_publish
            |> Enum.map(&publish_rule_to_genesis(&1, namespace))
            |> Enum.count(&(&1 == :ok))

          Logger.info("RuleEvolutionSystem: Published #{published_count} rules to Genesis",
            namespace: namespace,
            adaptive_threshold: adaptive_threshold
          )
      ...
      error ->
        Logger.error("RuleEvolutionSystem: Error publishing rules",
          error: inspect(error)
        )
  ...
  defp publish_rule_to_genesis(rule, namespace) do
