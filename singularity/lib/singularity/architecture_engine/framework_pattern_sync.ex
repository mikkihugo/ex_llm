defmodule Singularity.ArchitectureEngine.FrameworkPatternSync do
  ...
  def get_pattern(framework_name) do
    case :ets.lookup(@ets_table, framework_name) do
      ...
      [] ->
        ...
        case Singularity.ArchitectureEngine.FrameworkPatternStore.get_pattern(framework_name) do
          {:ok, pattern} ->
  ...
  def handle_cast({:learn_pattern, detection_result}, state) do
    ...
    case Singularity.ArchitectureEngine.FrameworkPatternStore.learn_pattern(detection_result) do
      {:ok, _id} ->
        # 2. Update ETS cache
