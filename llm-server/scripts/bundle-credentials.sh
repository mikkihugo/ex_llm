...
echo "🔐 Bundling AI Provider Credentials..."
echo ""
...
# Gemini ADC
echo "📦 Bundling Gemini ADC credentials..."
...
echo "📦 Bundling Claude credentials..."
...
# Cursor OAuth
echo "📦 Bundling Cursor credentials..."
...
# GitHub Token
echo "📦 Bundling GitHub token..."
if command -v gh &> /dev/null; then
    GH_TOKEN=$(gh auth token 2>/dev/null || echo "")
    if [ -n "$GH_TOKEN" ]; then
        echo "GH_TOKEN=$GH_TOKEN" >> "$OUTPUT_FILE"
        echo "✓ GitHub token bundled"
    else
        echo "⚠️  Warning: GitHub token not found"
        echo "  Run: gh auth login"
    fi
else
    echo "⚠️  Warning: gh CLI not installed"
    echo "  Manually add: GH_TOKEN=your_token_here"
fi
echo ""

# Gemini Code Project
echo "📦 Adding Gemini Code project..."
echo "GEMINI_CODE_PROJECT=gemini-code-473918" >> "$OUTPUT_FILE"
echo "✓ Gemini Code project added"
echo ""

# Port
echo "📦 Adding default port..."
echo "PORT=3000" >> "$OUTPUT_FILE"
echo "✓ Port added"
echo ""

echo "✅ Credentials bundled to: $OUTPUT_FILE"
echo ""
...
echo "  3. Or use with Docker: docker run --env-file $OUTPUT_FILE llm-server"
echo ""
