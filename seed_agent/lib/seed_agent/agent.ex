defmodule SeedAgent.Agent do
  ...
  alias SeedAgent.{CodeStore, Control, HotReload, ProcessRegistry}
  alias SeedAgent.Autonomy.Decider
  alias SeedAgent.Autonomy.Limiter
  alias SeedAgent.Control.QueueCrdt
  alias SeedAgent.DynamicCompiler
  alias MapSet
  ...
  def handle_info({:validate_improvement, version}, state) do
    ...
    current = SeedAgent.Telemetry.snapshot()
  ...
  defp emit_improvement_event(agent_id, event, measurements, metadata) do
    ...
    :telemetry.execute([:seed_agent, :improvement, event], measurements, meta)
  end
end
