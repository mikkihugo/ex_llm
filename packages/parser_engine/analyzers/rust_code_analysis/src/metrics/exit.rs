...
impl Default for Stats {
  fn default() -> Self {
    Self { exit: 0, exit_sum: 0, total_space_functions: 1, exit_min: usize::MAX, exit_max: 0 }
  }
}
...
impl fmt::Display for Stats {
  fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
    write!(f, "sum: {}, average: {} min: {}, max: {}", self.exit_sum(), self.exit_average(), self.exit_min(), self.exit_max())
  }
}
...
impl Exit for RustCode {
  fn compute(node: &Node, stats: &mut Stats) {
    if matches!(node.kind_id().into(), Rust::ReturnExpression | Rust::TryExpression) || Self::is_func(node) && node.child_by_field_name("return_type").is_some()
    {
      stats.exit += 1;
    }
  }
...
implement_metric_trait!(Exit, KotlinCode, PreprocCode, CcommentCode, ElixirCode, ErlangCode, GleamCode, LuaCode);
