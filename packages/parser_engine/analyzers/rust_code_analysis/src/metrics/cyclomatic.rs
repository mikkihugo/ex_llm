...
impl Default for Stats {
  fn default() -> Self {
    Self { cyclomatic_sum: 0., cyclomatic: 1., n: 1, cyclomatic_max: 0., cyclomatic_min: f64::MAX }
  }
}
...
impl fmt::Display for Stats {
  fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
    write!(f, "sum: {}, average: {}, min: {}, max: {}", self.cyclomatic_sum(), self.cyclomatic_average(), self.cyclomatic_min(), self.cyclomatic_max())
  }
}
...
impl Cyclomatic for PythonCode {
  fn compute(node: &Node, stats: &mut Stats) {
    ...
    match node.kind_id().into() {
      ...
      Else => {
        if node.has_ancestors(|node| matches!(node.kind_id().into(), ForStatement | WhileStatement), |node| node.kind_id() == ElseClause) {
          stats.cyclomatic += 1.;
        }
      }
      _ => {}
...
implement_metric_trait!(Cyclomatic, KotlinCode, PreprocCode, CcommentCode, ElixirCode, ErlangCode, GleamCode, LuaCode);
