...
impl Default for Sloc {
  fn default() -> Self {
    Self { start: 0, end: 0, unit: false, sloc_min: usize::MAX, sloc_max: 0 }
  }
}
...
impl Sloc {
  ...
  pub fn sloc(&self) -> f64 {
    ...
    let sloc = if self.unit { self.end - self.start } else { (self.end - self.start) + 1 };
    sloc as f64
  }
...
impl Default for Ploc {
  fn default() -> Self {
    Self { lines: HashSet::default(), ploc_min: usize::MAX, ploc_max: 0 }
  }
}
...
impl Default for Cloc {
  fn default() -> Self {
    Self { only_comment_lines: 0, code_comment_lines: 0, comment_line_end: Option::default(), cloc_min: usize::MAX, cloc_max: 0 }
  }
}
...
impl Default for Lloc {
  fn default() -> Self {
    Self { logical_lines: 0, lloc_min: usize::MAX, lloc_max: 0 }
  }
}
...
impl Default for Stats {
  fn default() -> Self {
    Self { sloc: Sloc::default(), ploc: Ploc::default(), cloc: Cloc::default(), lloc: Lloc::default(), space_count: 1, blank_min: usize::MAX, blank_max: 0 }
  }
}
...
impl Loc for MozjsCode {
  fn compute(node: &Node, stats: &mut Stats, is_func_space: bool, is_unit: bool) {
    ...
    match node.kind_id().into() {
      ...
      ExpressionStatement | ExportStatement | ImportStatement | StatementBlock | IfStatement | SwitchStatement | ForStatement | ForInStatement
      | WhileStatement | DoStatement | TryStatement | WithStatement | BreakStatement | ContinueStatement | DebuggerStatement | ReturnStatement
      | ThrowStatement | EmptyStatement | StatementIdentifier => {
        stats.lloc.logical_lines += 1;
      }
      _ => {
        check_comment_ends_on_code_line(stats, start);
...
impl Loc for RustCode {
  fn compute(node: &Node, stats: &mut Stats, is_func_space: bool, is_unit: bool) {
    ...
    match node.kind_id().into() {
      ...
      Statement | EmptyStatement | ExpressionStatement | LetDeclaration | AssignmentExpression | CompoundAssignmentExpr => {
        stats.lloc.logical_lines += 1;
      }
      _ => {
        check_comment_ends_on_code_line(stats, start);
...
impl Loc for CppCode {
  fn compute(node: &Node, stats: &mut Stats, is_func_space: bool, is_unit: bool) {
    ...
    match node.kind_id().into() {
      RawStringLiteral | StringLiteral | DeclarationList | FieldDeclarationList | TranslationUnit => {}
      Comment => {
        add_cloc_lines(stats, start, end);
      }
      WhileStatement | SwitchStatement | CaseStatement | IfStatement | ForStatement | ReturnStatement | BreakStatement | ContinueStatement | GotoStatement
      ...
      Declaration => {
        if node.count_specific_ancestors::<CppParser>(
          |node| matches!(node.kind_id().into(), WhileStatement | ForStatement | IfStatement),
...
impl Loc for JavaCode {
  fn compute(node: &Node, stats: &mut Stats, is_func_space: bool, is_unit: bool) {
    ...
    match kind_id {
      ...
      AssertStatement | BreakStatement | ContinueStatement | DoStatement | EnhancedForStatement | ExpressionStatement | ForStatement | IfStatement
      | ReturnStatement | SwitchExpression | ThrowStatement | TryStatement | WhileStatement => {
        stats.lloc.logical_lines += 1;
      }
      LocalVariableDeclaration => {
        if node.count_specific_ancestors::<JavaParser>(|node| node.kind_id() == ForStatement, |node| node.kind_id() == Block) == 0 {
...
mod tests {
  ...
  fn java_statement_inline_loc() {
    check_metrics::<JavaParser>("for (int i = 0; i < 100; i++) { System.out.println(\"hello\"); }", "foo.java", |metric| {
      // Spaces: 1
      insta::assert_json_snapshot!(
          metric.loc,
          @r###"
