    ...
    match node.kind_id().into() {
      ClassBody => {
        stats.class_na += node
          ...
          .map(|declaration| {
            let attributes = declaration.children().filter(|n| matches!(n.kind_id().into(), VariableDeclarator)).count();
            ...
            if declaration.child(0).is_some_and(|modifiers| {
              ...
              matches!(modifiers.kind_id().into(), Modifiers) && modifiers.first_child(|id| id == Public).is_some()
            }) {
