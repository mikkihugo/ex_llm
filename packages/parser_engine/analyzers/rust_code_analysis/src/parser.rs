...
#[derive(Debug)]
pub struct Parser<T: LanguageInfo + Alterator + Checker + Getter + Abc + Cognitive + Cyclomatic + Exit + Halstead + Loc + Mi + NArgs + Nom + Npa + Npm + Wmc> {
  code: Vec<u8>,
  tree: Tree,
...
#[inline(always)]
fn get_fake_code<T: LanguageInfo>(code: &[u8], path: &Path, pr: Option<Arc<PreprocResults>>) -> Option<Vec<u8>> {
  if let Some(pr) = pr {
    match T::get_lang() {
      LANG::Cpp => {
...
impl<T: 'static + LanguageInfo + Alterator + Checker + Getter + Abc + Cognitive + Cyclomatic + Exit + Halstead + Loc + Mi + NArgs + Nom + Npa + Npm + Wmc>
  ParserTrait for Parser<T>
{
  ...
  fn new(code: Vec<u8>, path: &Path, pr: Option<Arc<PreprocResults>>) -> Self {
    ...
    let code = if let Some(fake) = fake_code { fake } else { code };

    let tree = Tree::new::<T>(&code);

    Self { code, tree, phantom: PhantomData }
  }
      ...
      match f {
        ...
        _ => {
          if let Ok(n) = f.parse::<u16>() {
            ...
            let f = f.to_owned();
            res.push(Box::new(move |node: &Node| -> bool { node.kind().contains(&f) }));
          }
        }
      }
    }
