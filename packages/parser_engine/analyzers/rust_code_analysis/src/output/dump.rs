...
pub fn dump_node(code: &[u8], node: &Node, depth: i32, line_start: Option<usize>, line_end: Option<usize>) -> std::io::Result<()> {
  let stdout = StandardStream::stdout(ColorChoice::Always);
  let mut stdout = stdout.lock();
  let ret = dump_tree_helper(code, node, "", true, &mut stdout, depth, &line_start, &line_end);
...
fn dump_tree_helper(
  ...
  if count != 0 {
    ...
    loop {
      i -= 1;
      dump_tree_helper(code, &cursor.node(), &prefix, i == 0, stdout, depth - 1, line_start, line_end)?;
...
impl Callback for Dump {
  ...
  fn call<T: ParserTrait>(cfg: Self::Cfg, parser: &T) -> Self::Res {
    dump_node(parser.get_code(), &parser.get_root(), -1, cfg.line_start, cfg.line_end)
  }
}
