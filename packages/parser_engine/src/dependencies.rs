...
use crate::{
  errors::PolyglotCodeParserError, languages::ProgrammingLanguage, optimizations::AnalysisCache, AnalysisResult, ComplexityMetrics, HalsteadMetrics, LineMetrics,
  MaintainabilityMetrics, PolyglotCodeParserFrameworkConfig,
...
impl Default for UniversalDependencies {
  fn default() -> Self {
    Self::new().unwrap_or_else(|_| Self {
      ...
      config: PolyglotCodeParserFrameworkConfig::default(),
    })
  }
}
...
impl TokeiAnalyzer {
  ...
  pub async fn analyze(&self, content: &str, language: ProgrammingLanguage) -> Result<LineMetrics> {
    let tokei_lang = language
      .to_tokei_language()
      .ok_or_else(|| PolyglotCodeParserError::UnsupportedLanguage { language: language.to_string() })?;
    ...
    let metrics = tokio::task::spawn_blocking(move || -> Result<LineMetrics> {
      ...
      if let Some(language_stats) = languages.get(&lang_for_tokei) {
        ...
        Err(PolyglotCodeParserError::tokei_error(format!(
