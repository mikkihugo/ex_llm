//! Scala parser implemented with tree-sitter and the parser-framework traits.
...
impl ScalaParser {
    ...
    fn get_classes(&self, ast: &AST) -> Result<Vec<Class>, ParseError> {
        ...
        // Also collect objects (similar to classes in Scala)
        visit_nodes(ast.tree.root_node(), "object_definition", &mut |node| {
            let name = node
                .child_by_field_name("name")
                .and_then(|n| n.utf8_text(ast.content.as_bytes()).ok())
                .unwrap_or_default()
...
impl Default for ScalaParser {
    fn default() -> Self {
        Self::new().expect("Scala parser initialisation must succeed")
    }
}
...
impl LanguageParser for ScalaParser {
    fn parse(&self, content: &str) -> Result<AST, ParseError> {
        let tree = self
            .parse_tree(content)
            .ok_or_else(|| ParseError::ParseError("failed to parse Scala code".into()))?;
    ...
    fn get_metrics(&self, ast: &AST) -> Result<LanguageMetrics, ParseError> {
        ...
        let (complexity_score, _sloc, ploc, cloc, blank_lines) =
            parser_core::calculate_rca_complexity(&ast.content, "scala").unwrap_or((
                1.0,
...
mod tests {
    ...
    fn parses_basic_function() {
        let parser = ScalaParser::default();
        let source = r#"
...
"#;
        ...
        let ast = parser.parse(source).expect("scala parse");
