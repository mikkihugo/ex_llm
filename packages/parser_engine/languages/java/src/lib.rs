//! Java parser implemented with tree-sitter.
...
/// Java language parser backed by tree-sitter.
...
impl Default for JavaParser {
    fn default() -> Self {
        Self::new().expect("Java parser initialisation must succeed")
    }
}
...
impl LanguageParser for JavaParser {
    fn parse(&self, content: &str) -> Result<AST, ParseError> {
        ...
        let tree = parser
            .parse(content, None)
            .ok_or_else(|| ParseError::ParseError("failed to parse Java code".into()))?;
    ...
    fn get_metrics(&self, ast: &AST) -> Result<LanguageMetrics, ParseError> {
        ...
        let (complexity_score, _sloc, ploc, cloc, blank_lines) =
            parser_core::calculate_rca_complexity(&ast.content, "java").unwrap_or((
                1.0,
