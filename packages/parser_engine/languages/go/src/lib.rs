//! Go parser implemented with tree-sitter.
...
/// Go language parser backed by tree-sitter.
pub struct GoParser {
    parser: Mutex<Parser>,
}
...
impl Default for GoParser {
    fn default() -> Self {
        Self::new().expect("Go parser initialisation must succeed")
    }
}
...
impl LanguageParser for GoParser {
    fn parse(&self, content: &str) -> Result<AST, ParseError> {
        ...
        let tree = parser
            .parse(content, None)
            .ok_or_else(|| ParseError::ParseError("failed to parse Go code".into()))?;
    ...
    fn get_metrics(&self, ast: &AST) -> Result<LanguageMetrics, ParseError> {
        ...
        let (complexity_score, _sloc, ploc, cloc, blank_lines) =
            parser_core::calculate_rca_complexity(&ast.content, "go").unwrap_or((
                1.0,
