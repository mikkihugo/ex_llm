//! Swift parser implemented with tree-sitter and the parser-framework traits.
...
impl Default for SwiftParser {
    fn default() -> Self {
        Self::new().expect("Swift parser initialisation must succeed")
    }
}
...
impl LanguageParser for SwiftParser {
    fn parse(&self, content: &str) -> Result<AST, ParseError> {
        let tree = self
            .parse_tree(content)
            .ok_or_else(|| ParseError::ParseError("failed to parse Swift code".into()))?;
    ...
    fn get_metrics(&self, ast: &AST) -> Result<LanguageMetrics, ParseError> {
        ...
        let (complexity_score, _sloc, ploc, cloc, blank_lines) =
            parser_core::calculate_rca_complexity(&ast.content, "swift").unwrap_or((
                1.0,
...
"#;
        ...
        let ast = parser.parse(source).expect("swift parse");
