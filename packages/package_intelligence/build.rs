...
fn main() {
  ...
  for path in &json_files {
    ...
    // Read JSON content
    let json_content = fs::read_to_string(path).unwrap_or_else(|_| {
      panic!("Failed to read template file: {}", path.display())
    });
    ...
    writeln!(
      generated_code,
      "/// Generated template function for {template_id}"
    )
    .unwrap();
    writeln!(
      generated_code,
      "pub fn create_{function_name}_template() -> Template {{"
    )
    .unwrap();
    ...
    generated_code.push_str(
      "    let template_data: Value = serde_json::from_str(json_content)\n",
    );
    generated_code
      .push_str("        .expect(\"Failed to parse template JSON\");\n\n");

    generated_code.push_str("    Template {\n");
    generated_code.push_str(
      "        id: template_data[\"id\"].as_str().unwrap().to_string(),\n",
    );
    generated_code.push_str(
      "        name: template_data[\"name\"].as_str().unwrap().to_string(),\n",
    );
  ...
  println!(
    "cargo:warning=Generated AI templates: {}",
