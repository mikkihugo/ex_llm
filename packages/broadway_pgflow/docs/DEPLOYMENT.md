...
This document contains practical production deployment guidance for the `Broadway.PgflowProducer` package. It covers required environment configuration, database and queue setup, monitoring, scaling guidance, troubleshooting steps, and rollback procedures.

## Overview

The producer integrates Broadway with PGFlow-backed workflows using PostgreSQL as the durable queue. Production deployments must ensure durable storage, correct PGFlow configuration, resource lock handling (e.g., GPUs), monitoring, and an operational rollout/rollback plan.

## Prerequisites

- PostgreSQL (primary) reachable by the app and PGFlow.
- PGFlow library installed and available to your runtime.
...
- Ensure PGFlow has permissions to read/update queue tables and use advisory locks if required.
...
- PGFlow workflow health: restarts, failures, running workflows count
...
- Producer fails to start (workflow failed):
  - Check PGFlow and DB connectivity and permissions.
...
- Messages not yielded:
  ...
  - Check PGFlow workflows are running and not stuck.
...
- Post-deploy validation:
  - Validate workflow registration in PGFlow.
...
- For end-to-end performance (DB, PGFlow, GPU-bound tasks) run integration benchmarks in a staging environment that mirrors production hardware and DB.
