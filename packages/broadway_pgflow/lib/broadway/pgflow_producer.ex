defmodule Broadway.PgflowProducer do
  @moduledoc """
  Broadway producer adapter backed by PGFlow workflows for durable message production.

  This producer integrates with PGFlow to fetch messages from a PostgreSQL queue in a durable,
  ...
  def handle_demand(demand, _index, %{workflow_pid: workflow_pid} = state) do
    ...
    # Enqueue PGFlow workflow to fetch up to demand messages and include
  ...
  @doc """
  Handle external ack notifications by informing the PGFlow workflow.
  ...
  defp measure_recent_ack_latency() do
    ...
    telemetry_mod = Application.get_env(:broadway_pgflow, :telemetry_module, Singularity.Telemetry)
