defmodule Broadway.PgflowProducerErrorTest do
  ...
  describe "database failures and update errors" do
    test "handle_failure continues gracefully when Workflow.update returns db error", %{workflow_pid: workflow_pid} do
      ...
      assert {:noreply, ^state} = PgflowProducer.handle_failure({:basic, :db_error}, batch, nil, state)
    end
  end

  describe "pgflow timeouts and enqueue failures" do
    test "handle_demand tolerates PGFlow enqueue timeout", %{workflow_pid: workflow_pid} do
      ...
      assert {:noreply, [], ^state} = PgflowProducer.handle_demand(50, 0, state)
    end
  end
  ...
  describe "resource lock conflicts and recovery" do
    test "start_link returns error when workflow cannot acquire resource lock" do
      ...
      assert {:error, :resource_locked} = PgflowProducer.start_link(opts)
    end
    ...
    test "producer recovers when a transient resource conflict is represented as enqueue error", %{workflow_pid: workflow_pid} do
      ...
      assert {:noreply, [], ^state} = PgflowProducer.handle_demand(20, 0, state)
    end
  end
  ...
  describe "graceful degradation and eventual recovery" do
    test "multiple transient errors do not crash the GenServer", %{workflow_pid: workflow_pid} do
      ...
      # First call: timeout
      assert {:noreply, [], ^state} = PgflowProducer.handle_demand(5, 0, state)
      # Second call: success
      assert {:noreply, [], ^state} = PgflowProducer.handle_demand(5, 0, state)
    end
  end
