defmodule Broadway.PgflowProducerTest do
  ...
  describe "start_link/1" do
    test "starts GenServer and PGFlow workflow child", %{workflow_pid: _pid} do
      opts = [
        workflow_name: "test_producer",
        queue_name: "test_queue",
        concurrency: 5,
        batch_size: 10,
        pgflow_config: [timeout_ms: 300_000],
        resource_hints: [gpu: true]
      ]

      assert {:ok, pid} = PgflowProducer.start_link(opts)
      assert is_pid(pid)
    ...
    test "fails with missing required opts" do
      ...
      assert {:error, {:badarg, _}} = PgflowProducer.start_link(opts)
    end
  ...
  describe "handle_demand/3" do
    test "enqueues workflow with demand and returns empty list", %{workflow_pid: workflow_pid} do
      ...
      assert {:noreply, [], ^state} = PgflowProducer.handle_demand(10, 0, state)
    end
    ...
    test "handles high concurrency demand", %{workflow_pid: workflow_pid} do
      ...
      assert {:noreply, [], ^state} = PgflowProducer.handle_demand(100, 0, state)
    end
  end
  ...
  describe "handle_failure/4" do
    test "updates workflow to requeue failed messages", %{workflow_pid: workflow_pid} do
      ...
      assert {:noreply, ^state} = PgflowProducer.handle_failure({:basic, :test_error}, batch, nil, state)
    end
  ...
  describe "resource hints" do
    test "passes resource hints to workflow state", %{workflow_pid: workflow_pid} do
      ...
      expect(Workflow, :start_link, fn "test_producer", Broadway.PgflowProducer.Workflow, state ->
        assert state.resource_hints == [gpu: true, cpu_cores: 4]
        {:ok, workflow_pid}
      end)

      assert {:ok, _pid} = PgflowProducer.start_link(opts)
    end
  ...
  describe "error scenarios" do
    test "handles workflow start failure", %{workflow_pid: _workflow_pid} do
      ...
      assert {:error, :workflow_failed} = PgflowProducer.start_link(opts)
    end
