defmodule Broadway.PgflowProducerStressTest do
  ...
  describe "performance benchmarks vs DummyProducer" do
    test "benchmarks throughput: 10000 messages", %{workflow_pid: workflow_pid} do
      ...
      # Benchmark PgflowProducer
      {pgflow_time, _} = :timer.tc(fn ->
        ...
        expect(Broadway, :push_messages, 20, fn Broadway.PgflowProducer, messages ->
      ...
      # Log results
      IO.puts("PgflowProducer time: #{pgflow_time / 1000}ms")
      IO.puts("DummyProducer time: #{dummy_time / 1000}ms")
      IO.puts("Throughput ratio: #{dummy_time / pgflow_time}")

      # Pgflow should be reasonably close (within 2x) due to orchestration overhead
      assert pgflow_time < dummy_time * 3
