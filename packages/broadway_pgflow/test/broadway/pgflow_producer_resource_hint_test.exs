defmodule Broadway.PgflowProducerResourceHintTest do
  ...
  setup do
    ...
    Application.put_env(:broadway_pgflow, :repo, Singularity.RepoMock)
    :ok
  end
  ...
  test "acquire_hint reserves DB row and caches token" do
    ...
    assert {:next, :wait_acks, new_state} = Broadway.PgflowProducer.Workflow.yield_and_commit(state)
  ...
  test "failed acquire_hint prevents message send and job not yielded" do
    ...
    batches = [
      [
        %Broadway.Message{
          data: {2, %{}},
          metadata: %{resource_key: "gpu"},
          acknowledger: {Broadway.PgflowProducer.Workflow, 2}
        }
      ]
    ]
    ...
    assert {:halt, {:error, {:resource_unavailable, "gpu"}}, ^state} = Broadway.PgflowProducer.Workflow.yield_and_commit(state)
  end
  ...
  test "ack releases hint and marks job completed" do
    ...
    assert {:ok, new_state} = Broadway.PgflowProducer.Workflow.handle_update(:ack, %{id: 3}, state)
