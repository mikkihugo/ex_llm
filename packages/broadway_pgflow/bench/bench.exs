...
# Lightweight bench script comparing a simple Pgflow-style push vs Dummy push.
...
#   the Pgflow adapter path and a DummyProducer path. This focuses on
#   measuring throughput/latency of the push logic rather than full DB/PGFlow I/O.
# - CPU/GPU measurements are best-effort: it queries os_mon (if available)
#   and `nvidia-smi` (if present) for GPU utilization.

defmodule PgflowBench do
  def get_gpu_util do
  ...
  # Simulate the PGFlow producer path: create messages (maps) and loop through them
  # doing minimal work to emulate "yielding" and bookkeeping.
  def simulate_pgflow(count) when is_integer(count) and count > 0 do
...
suite = Benchee.run(
  %{
    "pgflow_sim" => fn count -> PgflowBench.simulate_pgflow(count) end,
    "dummy_sim" => fn count -> PgflowBench.simulate_dummy(count) end
  },
  inputs: inputs,
  time: 2,
  memory_time: 0.5
...
mem = :erlang.memory()
cpu = PgflowBench.get_cpu_util()
gpu = PgflowBench.get_gpu_util()
...
## Notes
- The benchmark simulates in-memory push loops and does not perform DB or PGFlow network I/O.
- For end-to-end production benchmarking (DB, PGFlow workflows, resource locks, GPU-bound work),
  run a tailored integration benchmark in a staging environment where the DB, PGFlow, and GPU hardware
  are available.

"""

result_path = Path.join([File.cwd!(), "packages", "broadway_pgflow", "bench", "results.md"])
