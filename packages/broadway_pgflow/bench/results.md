...
## Summary / Intent
The included benchmark script `packages/broadway_pgflow/bench/bench.exs` uses Benchee to run short microbenchmarks comparing the in-memory push/iteration paths that mimic:

- Broadway.PgflowProducer-like path (simulate_pgflow)  
...
- packages/broadway_pgflow/bench/bench.exs (Benchee script)
...
3. After successful run, `packages/broadway_pgflow/bench/results.md` will be overwritten with a timestamped summary.
...
- The included script is an in-memory microbenchmark. For production-grade benchmarking you must run an integration benchmark in a staging environment with:
  ...
  - PGFlow workflows enabled and using the same workflow config as production
...
- Capture results into CI artifacts and into `packages/broadway_pgflow/bench/results.md` (the bench script already writes a markdown summary).
