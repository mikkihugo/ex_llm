...
Broadway-compatible producer adapter backed by [PGFlow](https://hex.pm/packages/pgflow) workflows for durable message production and orchestration in PostgreSQL.

This package provides a reliable producer for Broadway pipelines, using PGFlow to manage message fetching, batching, and state updates (ack/nack) with built-in retries and resource hints (e.g., GPU locks).

## Features

- **Durability**: Messages are fetched and tracked via PGFlow workflows for fault-tolerant production.
...
Add `:broadway_pgflow` to your list of dependencies in `mix.exs`:
...
def deps do
  [
    ...
    {:pgflow, "~> 0.1"},
    {:ecto_sql, "~> 3.10"},
    {:broadway_pgflow, path: "../packages/broadway_pgflow"}  # For local development
  ]
end
```
...
### Broadway.PgflowProducer.start_link/1

Starts the producer GenServer with an attached PGFlow workflow.

**Options**:
- `workflow_name`: (required) Unique name for the PGFlow workflow.
...
- `pgflow_config`: (optional) PGFlow-specific config (e.g., `[timeout_ms: 300_000, retries: 3]`).
- `resource_hints`: (optional) Hints like `[gpu: true]` for resource acquisition.

**Example**:
```elixir
{:ok, _pid} = Broadway.PgflowProducer.start_link([
...
The integrated workflow (`Broadway.PgflowProducer.Workflow`) handles:
...
To migrate `Singularity.Embedding.BroadwayEmbeddingPipeline` from `Broadway.DummyProducer` to `Broadway.PgflowProducer`:
...
For GPU hints, ensure resource acquisition logic in `Broadway.PgflowProducer.Workflow.acquire_gpu_lock/1` is implemented (e.g., PG advisory locks).
...
- Check PGFlow dependency is properly installed and configured.
...
- Verify PGFlow workflow is running: check logs for workflow initialization.
...
# (PGFlow provides workflow inspection tools)

# Check Broadway producer state
:sys.get_state(Broadway.PgflowProducer)
```
...
- [ ] PGFlow configured with production timeouts and retries
...
- PGFlow workflow health: running workflows, failures, restarts

Recommended integrations:
- Prometheus metrics for Broadway and PGFlow (or instrument counters in your app)
...
- Implement liveness/readiness checks that validate DB connectivity and PGFlow workflow registration.
- On shutdown, allow in-flight batches to finish or be requeued by PGFlow within configured timeouts.
...
### PGFlow-specific recommendations
...
- Use PGFlow workflow inspection tools in staging to validate lifecycle behavior.
...
Tests use Mox for mocking PGFlow and database interactions.
