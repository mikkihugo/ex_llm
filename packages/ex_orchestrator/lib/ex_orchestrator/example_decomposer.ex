defmodule Pgflow.Orchestrator.ExampleDecomposer do
  @moduledoc """
  ...
  ## Usage
      ...
      {:ok, result} = Pgflow.WorkflowComposer.compose_from_goal(
        ...
        &Pgflow.Orchestrator.ExampleDecomposer.simple_decompose/1,
        step_functions,
        MyApp.Repo
      )
      ...
      {:ok, result} = Pgflow.WorkflowComposer.compose_from_goal(
        ...
        &Pgflow.Orchestrator.ExampleDecomposer.microservices_decompose/1,
        step_functions,
        MyApp.Repo
      )
  """
    ...
    tasks = cond do
      String.match?(goal_string, ~r/auth|authentication|login/i) ->
        [
          ...
          %{id: "hash_password", description: "Hash user password", depends_on: ["validate_input"]},
      ...
      String.match?(goal_string, ~r/deploy|deployment/i) ->
        [
          ...
          %{id: "build_artifacts", description: "Build deployment artifacts", depends_on: ["check_prerequisites"]},
          %{id: "deploy_services", description: "Deploy services to environment", depends_on: ["build_artifacts"]},
          %{id: "run_tests", description: "Run integration tests", depends_on: ["deploy_services"]},
          %{id: "verify_deployment", description: "Verify deployment success", depends_on: ["run_tests"]}
        ]

      String.match?(goal_string, ~r/process|analyze|data/i) ->
        [
          %{id: "fetch_data", description: "Fetch data from sources", depends_on: []},
          %{id: "validate_data", description: "Validate data quality", depends_on: ["fetch_data"]},
          %{id: "process_data", description: "Process and transform data", depends_on: ["validate_data"]},
          %{id: "save_results", description: "Save processed results", depends_on: ["process_data"]}
        ]
        
      true ->
        [
          %{id: "analyze_goal", description: "Analyze goal requirements", depends_on: []},
          %{id: "plan_execution", description: "Plan execution strategy", depends_on: ["analyze_goal"]},
          %{id: "execute_plan", description: "Execute the plan", depends_on: ["plan_execution"]},
          %{id: "verify_completion", description: "Verify goal completion", depends_on: ["execute_plan"]}
        ]
    end
    ...
    tasks = cond do
      String.match?(goal_string, ~r/microservices|microservice/i) ->
        [
          ...
          %{id: "deploy_auth_service", description: "Deploy authentication service", 
            depends_on: ["setup_database", "setup_redis"]},
          %{id: "deploy_api_gateway", description: "Deploy API gateway", 
            depends_on: ["deploy_auth_service"]},
          %{id: "deploy_user_service", description: "Deploy user management service", 
            depends_on: ["setup_database", "deploy_auth_service"]},
          %{id: "deploy_notification_service", description: "Deploy notification service", 
            depends_on: ["setup_message_queue", "deploy_auth_service"]},
          
          # Integration tasks
          %{id: "configure_services", description: "Configure service communication", 
            depends_on: ["deploy_api_gateway", "deploy_user_service", "deploy_notification_service"]},
          %{id: "setup_load_balancer", description: "Setup load balancer", 
            depends_on: ["configure_services"]},
          %{id: "run_integration_tests", description: "Run integration tests", 
            depends_on: ["setup_load_balancer"]},
          %{id: "verify_deployment", description: "Verify complete deployment", 
    ...
    tasks = cond do
      String.match?(goal_string, ~r/data|pipeline|etl|extract|transform|load/i) ->
        [
          ...
          %{id: "validate_data", description: "Validate extracted data", 
            depends_on: ["extract_users", "extract_orders", "extract_products"]},
          %{id: "clean_data", description: "Clean and normalize data", 
            depends_on: ["validate_data"]},
          %{id: "transform_data", description: "Transform data for target format", 
            depends_on: ["clean_data"]},
          
          # Loading tasks
          %{id: "load_to_warehouse", description: "Load data to data warehouse", 
            depends_on: ["transform_data"]},
          %{id: "create_aggregates", description: "Create data aggregates", 
            depends_on: ["load_to_warehouse"]},
          %{id: "update_analytics", description: "Update analytics dashboards", 
    ...
    tasks = cond do
      String.match?(goal_string, ~r/ml|machine learning|model|training|prediction/i) ->
        [
          ...
          %{id: "clean_data", description: "Clean and preprocess data", depends_on: ["collect_data"]},
          %{id: "split_data", description: "Split data into train/validation/test", depends_on: ["clean_data"]},
          
          # Model development
          %{id: "feature_engineering", description: "Engineer features", depends_on: ["split_data"]},
          %{id: "train_model", description: "Train machine learning model", depends_on: ["feature_engineering"]},
          %{id: "validate_model", description: "Validate model performance", depends_on: ["train_model"]},
          
          # Model deployment
          %{id: "package_model", description: "Package model for deployment", depends_on: ["validate_model"]},
          %{id: "deploy_model", description: "Deploy model to production", depends_on: ["package_model"]},
          %{id: "monitor_model", description: "Setup model monitoring", depends_on: ["deploy_model"]}
        ]
