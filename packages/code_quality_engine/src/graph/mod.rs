...
impl Graph {
    ...
    fn extract_semantic_features(&self, vectors: &[String]) -> SemanticFeatures {
        ...
        for vector in vectors {
            ...
            } else if vector.contains("pattern:")
                || vector.contains("behavior:")
                || vector.contains("structure:")
            {
                patterns.push(vector.clone());
            } else if vector.contains("functionality")
                || vector.contains("cognitive")
                || vector.contains("functional")
            {
                features.push(vector.clone());
            } else if vector.contains("business")
                || vector.contains("financial")
                || vector.contains("ecommerce")
            {
                business_context.push(vector.clone());
            } else if vector.contains("performance")
                || vector.contains("cached")
                || vector.contains("optimized")
            {
                performance.push(vector.clone());
            } else if vector.contains("security")
                || vector.contains("authenticated")
                || vector.contains("encrypted")
            {
    ...
    pub fn calculate_similarity(&mut self, file1: &str, file2: &str) -> f64 {
        ...
        let similarity = if let (Some(node1), Some(node2)) =
            (self.file_to_node.get(file1), self.file_to_node.get(file2))
        {
    ...
    fn is_microservice_communication(&self, file1: &str, file2: &str) -> bool {
        ...
        ms_patterns
            .iter()
            .any(|pattern| file1.contains(pattern) && file2.contains(pattern))
    }
    ...
    fn generate_context(&self, file1: &str, file2: &str, similarity: f64) -> String {
        format!(
            "Files {} and {} have {:.1}% similarity based on vector analysis",
            file1,
            file2,
            similarity * 100.0
        )
    ...
    pub fn get_stats(&self) -> DAGStats {
        DAGStats {
            ...
            average_relationships_per_file: if self.graph.node_count() > 0 {
    ...
    pub fn find_similar_files(&self, file_path: &str, threshold: f64) -> Vec<(String, f64)> {
        ...
        if let Some(node_index) = self.file_to_node.get(file_path) {
            ...
            for (path, other_node_index) in &self.file_to_node {
                if path != file_path {
                    let similarity = self.calculate_vector_similarity(
                        &source_node.vectors,
                        &self.graph[*other_node_index].vectors,
                    );
    ...
    pub fn optimize_traversal(&self, start_file: &str, target_file: &str) -> Option<Vec<String>> {
        if let (Some(start_node), Some(target_node)) = (
            self.file_to_node.get(start_file),
            self.file_to_node.get(target_file),
        ) {
    ...
    fn find_shortest_path(&self, start: NodeIndex, target: NodeIndex) -> Option<Vec<String>> {
        ...
        if let Some(_distance) = distances.get(&target) {
            ...
            Some(vec![
                self.graph[start].file_path.clone(),
                self.graph[target].file_path.clone(),
            ])
        } else {
            None
        }
