...
use crate::analysis::multilang::{
    CrossLanguageCodePattern, CrossLanguageCodePatternsDetector, LanguageAnalysis,
    LanguageAnalyzer, LanguageSpecificRulesAnalyzer, RuleViolation,
};
use crate::analysis::semantic::search_index::CodeMetadata;

// Import parser types
use parser_core::{AnalysisResult, ClassInfo, FunctionInfo, RcaMetrics};
...
impl CodebaseAnalyzer {
    ...
    pub fn check_language_rules(&self, code: &str, language_hint: &str) -> Vec<RuleViolation> {
        ...
        let tokenizer =
            crate::analysis::semantic::custom_tokenizers::SemanticTokenizer::new(language_hint);
    ...
    pub fn detect_cross_language_patterns(
        ...
        let tokens_by_file: Vec<Vec<crate::analysis::semantic::custom_tokenizers::DataToken>> =
            files
                .iter()
                .map(|(hint, code)| {
                    let tokenizer =
                        crate::analysis::semantic::custom_tokenizers::SemanticTokenizer::new(hint);
    ...
    pub async fn build_call_graph(
        ...
        graph_builder
            .build_call_graph(metadata_cache)
            .await
    ...
    pub async fn build_import_graph(
        ...
        let graph_builder = CodeGraphBuilder::new(working_directory);
        graph_builder
            .build_import_graph(metadata_cache)
            .await
    ...
    pub fn analyze_files_with_parser(
        &self,
    ...
    /// Calculate aggregate quality metrics from structural data.
    ///
    /// Provides lightweight heuristics that can be enriched with RCA metrics when available.
    pub fn calculate_quality_metrics(
        ...
        let size_penalty = if file_size > 10_000 { 0.1 } else { 0.0 };
    ...
    /// Calculate cyclomatic and cognitive complexity heuristics.
    ...
    /// Rank similarity results in descending order and truncate to a limit.
