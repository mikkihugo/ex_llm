...
impl LanguageSpecificRulesAnalyzer {
  ...
  fn initialize_rules_from_registry(&mut self) {
    ...
    self.add_family_rules("BEAM", vec![
      LanguageSpecificRule {
        id: "beam_snake_case".to_string(),
        name: "Snake case for functions and variables".to_string(),
        description: "BEAM languages use snake_case for function and variable names".to_string(),
        ...
        suggested_fix: Some("Convert to snake_case: use_underscores_between_words".to_string()),
      },
      LanguageSpecificRule {
        id: "beam_module_organization".to_string(),
        name: "Module organization and documentation".to_string(),
        description: "BEAM modules should have @moduledoc and clear section structure".to_string(),
        applies_to: "BEAM".to_string(),
        rule_type: LanguageRuleType::BestPractice,
        severity: RuleSeverity::Info,
        pattern: "defmodule".to_string(),
        suggested_fix: Some("Add @moduledoc with module description at top of file".to_string()),
      },
    ]);

    // Systems languages (Rust, C, C++)
    self.add_family_rules("Systems", vec![
      LanguageSpecificRule {
        id: "systems_pascalcase_types".to_string(),
        name: "PascalCase for types and structs".to_string(),
        description: "Systems languages use PascalCase for type, struct, and enum names".to_string(),
      ...
      LanguageSpecificRule {
        ...
        description: "Systems languages require explicit error handling (Result, Option)".to_string(),
        applies_to: "Systems".to_string(),
        rule_type: LanguageRuleType::BestPractice,
        severity: RuleSeverity::Error,
        pattern: "unwrap|panic|abort".to_string(),
        suggested_fix: Some("Use Result<T> or Option<T> instead of unwrap()".to_string()),
      },
    ]);

    // Web languages (JavaScript, TypeScript, JSON)
    self.add_family_rules("Web", vec![
      LanguageSpecificRule {
        id: "web_camelcase_naming".to_string(),
        name: "Camel case for functions and variables".to_string(),
        description: "Web languages use camelCase for function and variable names".to_string(),
      ...
      LanguageSpecificRule {
        ...
        description: "Web code should use async/await or promises, not callbacks".to_string(),
        applies_to: "Web".to_string(),
        rule_type: LanguageRuleType::BestPractice,
        severity: RuleSeverity::Warning,
        pattern: "callback.*function|promise.*chaining".to_string(),
        suggested_fix: Some("Use async/await syntax for cleaner asynchronous code".to_string()),
      },
    ]);

    // Dynamic languages (Python, Lua, Bash)
    self.add_family_rules("Scripting", vec![
      LanguageSpecificRule {
        id: "scripting_snake_case".to_string(),
        name: "Snake case for functions".to_string(),
        description: "Scripting languages use snake_case for function and variable names".to_string(),
      ...
      LanguageSpecificRule {
        ...
        description: "Modern Python code should include type hints for clarity".to_string(),
        applies_to: "Scripting".to_string(),
        rule_type: LanguageRuleType::BestPractice,
        severity: RuleSeverity::Info,
  ...
  pub fn analyze_rules(&self, code: &str, language_hint: &str, tokens: &[DataToken]) -> Vec<RuleViolation> {
  ...
  fn check_rule(&self, code: &str, tokens: &[DataToken], rule: &LanguageSpecificRule) -> Vec<RuleViolation> {
    let mut violations = vec![];
    ...
    match rule.rule_type {
          ...
          for (line_num, line) in code.lines().enumerate() {
            if re.is_match(line) {
              violations.push(RuleViolation {
                ...
                details: format!("Potential naming convention violation in: {}", line.trim()),
              });
            }
          }
        }
      ...
      LanguageRuleType::CodeStyle => {
        ...
        for (line_num, line) in code.lines().enumerate() {
          if line.len() > 100 {  // Example: line too long
            violations.push(RuleViolation {
              rule: rule.clone(),
              location: format!("line {}", line_num + 1),
              details: format!("Line is {} characters (recommended max 100)", line.len()),
            });
          }
        }
      }
      LanguageRuleType::BestPractice | LanguageRuleType::PerformanceRule | LanguageRuleType::SecurityRule => {
  ...
  pub fn add_rule(&mut self, language_id: String, rule: LanguageSpecificRule) {
    self.rules.entry(language_id).or_insert_with(Vec::new).push(rule);
  }
