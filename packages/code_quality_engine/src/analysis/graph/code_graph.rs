...
impl CodeDependencyGraph {
                ...
                for (node_id, &node_index) in &self.node_lookup {
                    if node_id != query_node_id {
                        if let Some(node) = self.graph.node_weight(node_index) {
                            if let (Some(node_vector), Some(node_magnitude)) =
                                (&node.vector, node.vector_magnitude)
                            {
                                // Use advanced similarity combining multiple metrics
                                let cosine_sim = Self::cosine_similarity(
                                    query_vector,
                                    node_vector,
                                    query_magnitude,
                                    node_magnitude,
                                ...
                                let jaccard_sim =
                                    Self::jaccard_similarity(&query_node.name, &node.name);
                                ...
                                let combined_similarity =
                                    0.6 * cosine_sim + 0.25 * jaccard_sim + 0.15 * structural_sim;
    ...
    fn analyze_naming_patterns(&self) -> Vec<NamingCodePattern> {
        ...
        let mut name_groups: std::collections::HashMap<String, Vec<String>> =
        ...
        for (pattern, nodes) in name_groups {
            if nodes.len() > 2 {
                patterns.push(NamingCodePattern {
                    pattern,
                    count: nodes.len(),
...
impl CodeGraphBuilder {
                    ...
                    for (other_path, other_metadata) in metadata_cache {
                        ...
                        if other_path.to_string_lossy().contains(import) {
                            ...
                            for callee_name in &other_metadata.functions {
                                let callee_id =
                                    format!("{}::{}", other_path.display(), callee_name);
    ...
    pub async fn build_advanced_codebase_graph(
        &self,
        metadata_cache: &HashMap<PathBuf, CodeMetadata>,
    ) -> Result<CodeGraph> {
        ...
        for (file_path, metadata) in metadata_cache {
            for function in &metadata.functions {
                ...
                let advanced_vector = vectorizer.create_advanced_vector(
                    node_id.clone(),
                    &text,
                    "function".to_string(),
                )?;
