...
use crate::{
    analysis::results::{CodePatternDatabase, FileAnalysisResult},
...
impl CodeInsightsEngine {
    ...
    pub async fn analyze_file_stellar(
        ...
        let insights = self
            .insight_engine
            .generate_insights(&basic_analysis, content);

        // Detect code patterns
        let detected_patterns = self.pattern_database.detect_patterns(content);
        let patterns: Vec<CodeCodePattern> = detected_patterns
            .into_iter()
            .map(|p| CodeCodePattern {
                name: p.pattern_name,
        ...
        let recommendations =
            self.generate_architectural_recommendations(&basic_analysis, &patterns);
    ...
    /// Generate architectural recommendations
    fn generate_architectural_recommendations(
        &self,
        analysis: &FileAnalysisResult,
        patterns: &[CodeCodePattern],
    ) -> Vec<ArchitecturalRecommendation> {
    ...
    fn calculate_performance_metrics(&self, content: &str) -> PerformanceMetrics {
        ...
        PerformanceMetrics {
            ...
            performance_score: self
                .calculate_performance_score(cyclomatic_complexity, maintainability_index),
        }
    }
    ...
    fn check_quality_gates(
        ...
        gates.push(QualityGate {
            ...
            status: if metrics.cyclomatic_complexity <= 10.0 {
                GateStatus::Passed
            } else {
                GateStatus::Failed
            },
        ...
        gates.push(QualityGate {
            ...
            status: if security.security_score >= 80.0 {
                GateStatus::Passed
            } else {
                GateStatus::Failed
            },
            ...
            message: if security.security_score >= 80.0 {
                "Security score meets requirements".to_string()
            } else {
                "Security score below threshold".to_string()
            },
        });
    ...
    fn calculate_technical_debt_ratio(&self, content: &str) -> f64 {
        ...
        let todo_lines =
            content.matches("TODO").count() as f64 + content.matches("FIXME").count() as f64;
        if total_lines > 0.0 {
