    ...
    fn extract_keywords(&self, data: &DataType) -> Result<Vec<String>> {
        ...
        Ok(tokens
            ...
            .filter(|token| {
                matches!(
                    token.token_type,
                    TokenType::FunctionName | TokenType::ClassName | TokenType::VariableName
                )
            })
...
impl CodeTokenizer {
    fn tokenize_code(&self, language: &str, content: &str) -> Result<Vec<DataToken>> {
        ...
        let lang_info = LANGUAGE_REGISTRY
            .get_language(language)
            .or_else(|| LANGUAGE_REGISTRY.get_language_by_alias(language));

        if let Some(lang) = lang_info {
            // Use registry-driven tokenization with language-specific keywords
            self.tokenize_with_keywords(content, &lang.tokenization_profile.keywords)
        } else {
            // Fallback to generic tokenization for unknown languages
    ...
    fn tokenize_with_keywords(&self, content: &str, keywords: &[String]) -> Result<Vec<DataToken>> {
        let mut tokens = Vec::new();
        ...
        for part in content.split(
            ...
            if cleaned.len() > 1 && !cleaned.chars().all(|c| c.is_numeric()) {
                let token_type = if keywords.iter().any(|k| k == &cleaned) {
                    TokenType::Keyword
                } else {
    ...
    fn tokenize(&self, data: &DataType) -> Result<Vec<DataToken>> {
        match data {
            DataType::Prompt {
                sections,
                variables,
            } => self.tokenize_prompt(sections, variables),
    ...
    fn extract_keywords(&self, data: &DataType) -> Result<Vec<String>> {
        ...
        Ok(tokens
            ...
            .filter(|token| {
                matches!(
                    token.token_type,
                    TokenType::SectionHeader | TokenType::Variable
                )
            })
            .map(|token| token.text)
            .collect())
    ...
    fn tokenize(&self, data: &DataType) -> Result<Vec<DataToken>> {
        match data {
            DataType::Fact {
                entities,
                relationships,
            } => self.tokenize_fact(entities, relationships),
    ...
    fn extract_keywords(&self, data: &DataType) -> Result<Vec<String>> {
        ...
        Ok(tokens
            ...
            .filter(|token| {
                matches!(
                    token.token_type,
                    TokenType::Entity | TokenType::Relationship
                )
            })
            .map(|token| token.text)
            .collect())
...
impl FactTokenizer {
    fn tokenize_fact(
        ...
        for relationship in relationships {
            tokens.push(DataToken {
                text: relationship.clone(),
                token_type: TokenType::Relationship,
                weight: 0.9,
    ...
    fn extract_keywords(&self, data: &DataType) -> Result<Vec<String>> {
        ...
        Ok(tokens
            ...
            .filter(|token| {
                matches!(
                    token.token_type,
                    TokenType::ContextKeyword | TokenType::CodeBlock
                )
            })
            .map(|token| token.text)
            .collect())
    ...
    fn tokenize_snippet(&self, context: &str, content: &str) -> Result<Vec<DataToken>> {
        ...
        for word in context.split_whitespace() {
            tokens.push(DataToken {
                ...
                token_type: TokenType::ContextKeyword,
                weight: 1.0,
    ...
    fn tokenize(&self, data: &DataType) -> Result<Vec<DataToken>> {
        match data {
            DataType::Session {
                project_path,
                metadata,
            } => self.tokenize_session(project_path, metadata),
    ...
    fn extract_keywords(&self, data: &DataType) -> Result<Vec<String>> {
        ...
        Ok(tokens
            ...
            .filter(|token| {
                matches!(
                    token.token_type,
                    TokenType::ProjectPath | TokenType::FileExtension
                )
            })
            .map(|token| token.text)
            .collect())
        ...
        for part in project_path.split('/') {
            if !part.is_empty() {
                tokens.push(DataToken {
                    ...
                    token_type: TokenType::ProjectPath,
                    weight: 1.0,
...
mod tests {
    ...
    fn test_code_tokenizer_rust() {
        ...
        let data = DataType::Code {
            ...
            content: "fn get_user_name() -> String { user.name }".to_string(),
        };
    ...
    fn test_code_tokenizer_python() {
        ...
        let data = DataType::Code {
            ...
            content: "def calculate_total(items): return sum(items)".to_string(),
        };
    ...
    fn test_code_tokenizer_typescript() {
        ...
        let data = DataType::Code {
            ...
            content: "function greet(name: string): void { console.log(name); }".to_string(),
        };
    ...
    fn test_code_tokenizer_javascript() {
        ...
        let data = DataType::Code {
            ...
            content: "const getValue = (key) => data[key];".to_string(),
        };
    ...
    fn test_code_tokenizer_java() {
        ...
        let data = DataType::Code {
            ...
            content: "public class User { private String name; }".to_string(),
        };
    ...
    fn test_code_tokenizer_go() {
        ...
        let data = DataType::Code {
            ...
            content: "func main() { fmt.Println(\"Hello\") }".to_string(),
        };
    ...
    fn test_code_tokenizer_csharp() {
        ...
        let data = DataType::Code {
            ...
            content: "public class Program { static void Main() { } }".to_string(),
        };
    ...
    fn test_code_tokenizer_elixir() {
        ...
        let data = DataType::Code {
            ...
            content: "defmodule User do def get_name(user), do: user.name end".to_string(),
        };
    ...
    fn test_code_tokenizer_erlang() {
        ...
        let data = DataType::Code {
            ...
            content: "-module(hello). -export([world/0]). world() -> ok.".to_string(),
        };
    ...
    fn test_code_tokenizer_gleam() {
        ...
        let data = DataType::Code {
            ...
            content: "pub fn main() { let x = 42 x }".to_string(),
        };
    ...
    fn test_prompt_tokenizer() {
        ...
        let data = DataType::Prompt {
            sections: vec!["Authentication".to_string()],
    ...
    fn test_fact_tokenizer() {
        ...
        let data = DataType::Fact {
            entities: vec!["User".to_string()],
            relationships: vec!["has_profile".to_string()],
        };
    ...
    fn test_keyword_weights() {
        ...
        let data = DataType::Code {
            language: "rust".to_string(),
            content: "fn main() -> i32 { let x = 42; x }".to_string(),
        };

        let tokens = tokenizer.tokenize(&data).unwrap();

        // Verify weight distribution
        let keyword_weights: Vec<f32> = tokens
            .iter()
            .filter(|t| matches!(t.token_type, TokenType::Keyword))
            .map(|t| t.weight)
            .collect();
