...
impl ArchitectureAnalyzer {
    ...
    pub async fn detect_architecture_patterns(
    ...
    ) -> Result<Vec<ArchitectureCodePattern>> {
        ...
        if packages.len() > 1
            && packages.len() < 10
            && !patterns.contains(&ArchitectureCodePattern::Microservices)
        {
    ...
    fn is_microservices(
        ...
        let has_api_gateway = packages.iter().any(|p| {
    ...
    pub async fn detect_communication_patterns(
        ...
        if infrastructure
            .message_brokers
            .iter()
            .any(|b| b.name == "Kafka" || b.name == "RabbitMQ")
        {
        ...
        // Streaming
        // Phase 6: Dynamic message broker names
        if api_protocols.contains(&ApiProtocol::WebSocket)
            || infrastructure
                .message_brokers
                .iter()
                .any(|b| b.name == "Kafka")
    ...
    async fn extract_dependencies(&self, package: &Package) -> Result<Vec<PackageId>> {
        ...
        match manifest_name {
            "Cargo.toml" => {
                self.extract_cargo_dependencies(&package.manifest_path)
                    .await
            }
    ...
    pub async fn detect_api_protocols(
        &self,
        packages: &[Package],
        project_tech_stacks: &HashMap<PackageId, TechStack>,
    ) -> Result<Vec<ApiProtocol>> {
        let mut protocols = HashSet::new();

        for package in packages {
            if let Some(project_tech_stack) = project_tech_stacks.get(&package.id) {
                // REST
                if project_tech_stack.frameworks.iter().any(|f| {
                    f.contains("Express")
                        || f.contains("Fastify")
                        || f.contains("Actix")
                        || f.contains("Axum")
    ...
    pub async fn detect_event_systems(&self, packages: &[Package]) -> Result<Vec<EventSystem>> {
        ...
        // Check for event bus
        let has_eventbus = packages.iter().any(|p| {
            p.name.contains("event") || p.name.contains("bus") || p.name.contains("eventbus")
        });
        ...
        let has_event_sourcing = packages
            .iter()
            .any(|p| p.name.contains("event-store") || p.name.contains("eventsourcing"));

        if has_event_sourcing {
            systems.push(EventSystem::EventSourcing {
                aggregates: Vec::new(),
            });
        }
...
mod tests {
    ...
    async fn test_microservices_detection() {
        ...
        let patterns = analyzer
            .detect_architecture_patterns(&packages, &infrastructure)
            .await
            .unwrap();
