...
impl TechStackAnalyzer {
  ...
  pub async fn analyze_project_tech_stacks(&self, packages: &[Package]) -> Result<HashMap<PackageId, TechStack>> {
  ...
  async fn analyze_package_project_tech_stack(&self, package: &Package) -> Result<TechStack> {
    ...
    match language {
      ...
      Language::TypeScript | Language::JavaScript => self.analyze_node_project_tech_stack(&package.path).await,
  ...
  fn detect_language(&self, manifest_path: &PathBuf) -> Result<Language> {
    let manifest_name = manifest_path.file_name().and_then(|n| n.to_str()).unwrap_or("");
  ...
  async fn analyze_rust_project_tech_stack(&self, path: &Path) -> Result<TechStack> {
    ...
    Ok(TechStack { frameworks, libraries, build_tools, runtime: "Rust".to_string() })
  }
  ...
  async fn analyze_node_project_tech_stack(&self, path: &Path) -> Result<TechStack> {
    ...
    if let Ok(json) = serde_json::from_str::<serde_json::Value>(&contents) {
      let deps = json.get("dependencies").and_then(|d| d.as_object()).map(|o| o.keys().cloned().collect::<Vec<_>>()).unwrap_or_default();

      let dev_deps = json.get("devDependencies").and_then(|d| d.as_object()).map(|o| o.keys().cloned().collect::<Vec<_>>()).unwrap_or_default();
      ...
      if all_deps.contains(&"nest".to_string()) || all_deps.contains(&"@nestjs/core".to_string()) {
    ...
    let runtime = if path.join("tsconfig.json").exists() { "TypeScript/Node.js".to_string() } else { "Node.js".to_string() };

    Ok(TechStack { frameworks, libraries, build_tools, runtime })
  }
  ...
  async fn analyze_go_project_tech_stack(&self, path: &Path) -> Result<TechStack> {
    ...
    Ok(TechStack { frameworks, libraries, build_tools: vec!["go".to_string()], runtime: "Go".to_string() })
  }
  ...
  async fn analyze_python_project_tech_stack(&self, path: &Path) -> Result<TechStack> {
    ...
    Ok(TechStack { frameworks, libraries, build_tools, runtime: "Python".to_string() })
  }
  ...
  async fn analyze_elixir_project_tech_stack(&self, path: &Path) -> Result<TechStack> {
    ...
    Ok(TechStack { frameworks, libraries, build_tools: vec!["mix".to_string()], runtime: "Elixir/BEAM".to_string() })
  }
  ...
  async fn analyze_erlang_project_tech_stack(&self, path: &Path) -> Result<TechStack> {
    ...
    Ok(TechStack { frameworks, libraries, build_tools: vec!["rebar3".to_string()], runtime: "Erlang/OTP".to_string() })
  }
  ...
  async fn analyze_java_project_tech_stack(&self, path: &Path) -> Result<TechStack> {
    ...
    Ok(TechStack { frameworks, libraries, build_tools, runtime: "JVM".to_string() })
  }
  ...
  pub async fn analyze_tool_stack(&self) -> Result<ToolStack> {
    ...
    // Check for linters
    if self.root_path.join(".eslintrc").exists() || self.root_path.join(".eslintrc.json").exists() || self.root_path.join(".eslintrc.js").exists() {
    ...
    Ok(ToolStack { linters, formatters, ci_cd, testing_frameworks })
  }
}

impl Default for TechStack {
  fn default() -> Self {
    Self { frameworks: Vec::new(), libraries: Vec::new(), build_tools: Vec::new(), runtime: "Unknown".to_string() }
  }
}
...
serde = "1.0"
    ...
    let project_tech_stack = analyzer.analyze_rust_project_tech_stack(temp.path()).await.unwrap();

    assert!(project_tech_stack.frameworks.contains(&"Actix Web".to_string()));
      ...
      r#"
{
  "dependencies": {
    ...
    let analyzer = TechStackAnalyzer::new(temp.path().to_path_buf());
    let project_tech_stack = analyzer.analyze_node_project_tech_stack(temp.path()).await.unwrap();

    assert!(project_tech_stack.frameworks.contains(&"Next.js".to_string()));
