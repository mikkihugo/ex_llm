...
impl DomainBoundaryAnalyzer {
    ...
    pub fn new(domains: &[Domain], dependency_graph: &DependencyGraph) -> Self {
        ...
        for domain in domains {
            let node = domain_graph.add_node(DomainNode {
                ...
                packages: domain.packages.clone(),
            });
    ...
    fn build_domain_edges(
        ...
        for edge in &dependency_graph.edges {
            if let (Some(from_domain), Some(to_domain)) = (
                package_to_domain.get(&edge.from),
                package_to_domain.get(&edge.to),
            ) {
        ...
        for ((from_domain, to_domain), count) in domain_interactions {
            if let (Some(&from_node), Some(&to_node)) =
                (domain_index.get(&from_domain), domain_index.get(&to_domain))
            {
    ...
    pub fn compute_coupling_scores(&self) -> HashMap<String, f64> {
        ...
        for node_idx in self.domain_graph.node_indices() {
            ...
            let coupling = if connection_count > 0.0 {
    ...
    pub fn suggest_improvements(&self) -> Vec<String> {
        ...
        if !cycles.is_empty() {
            suggestions.push(format!(
                "⚠️  Found {} circular domain dependencies - consider breaking cycles: {:?}",
                cycles.len(),
                cycles
            ));
        }
        ...
        if !isolated.is_empty() {
            suggestions.push(format!(
                "ℹ️  Found {} isolated domains (no interactions): {:?}",
                isolated.len(),
                isolated
            ));
        }
        ...
        let high_coupling: Vec<_> = coupling_scores
            .iter()
        ...
        if !high_coupling.is_empty() {
            suggestions.push(format!(
                "⚠️  High coupling detected in {} domains - consider decoupling",
                high_coupling.len()
            ));
        }
        ...
        if let Some((domain, degree)) = central.first() {
            if *degree > 5 {
                suggestions.push(format!(
                    "⚠️  Domain '{}' is a central bottleneck ({} connections) - consider splitting",
                    domain, degree
                ));
            }
        }
    ...
    pub fn to_dot(&self) -> String {
        ...
        format!(
            "{:?}",
            Dot::with_config(&self.domain_graph, &[Config::EdgeNoLabel])
        )
    }
}
...
mod tests {
    ...
    fn test_domain_boundary_analysis() {
        let domains = vec![
            Domain {
                ...
                packages: vec!["auth-service".to_string()],
                confidence: 0.9,
            },
            Domain {
                name: "payments".to_string(),
                description: "Payment processing".to_string(),
                packages: vec!["payment-service".to_string()],
                confidence: 0.9,
            },
        ];
        ...
        let dependency_graph = DependencyGraph {
            ...
            edges: vec![Dependency {
                from: "payment-service".to_string(),
    ...
    fn test_cycle_detection() {
        let domains = vec![
            Domain {
                name: "a".to_string(),
                description: "Domain A".to_string(),
                packages: vec!["pkg-a".to_string()],
                confidence: 0.9,
            },
            Domain {
                name: "b".to_string(),
                description: "Domain B".to_string(),
