...
use tantivy::{
    collector::TopDocs, query::QueryParser, schema::*, Index, ReloadPolicy, TantivyDocument,
};
...
impl TantivySearchEngine {
    ...
    pub fn new() -> Result<Self> {
        ...
        let architecture_pattern_field =
            schema_builder.add_text_field("architecture_pattern", TEXT);
    ...
    pub async fn search(
        ...
        // Perform search
        let top_docs =
            searcher.search(&tantivy_query, &TopDocs::with_limit(options.max_results))?;
    ...
    pub async fn search_by_business_domain(
        &self,
        domain: &str,
        query: &str,
    ) -> Result<TantivySearchResponse> {
        ...
        let query_parser = QueryParser::for_index(
            &self.index,
            vec![self.content_field, self.business_domain_field],
        );
        let tantivy_query =
            query_parser.parse_query(&format!("{} AND business_domain:{}", query, domain))?;
    ...
    pub async fn search_by_architecture_pattern(
        &self,
        pattern: &str,
        query: &str,
    ) -> Result<TantivySearchResponse> {
        ...
        let query_parser = QueryParser::for_index(
            &self.index,
            vec![self.content_field, self.architecture_pattern_field],
        );
        let tantivy_query =
            query_parser.parse_query(&format!("{} AND architecture_pattern:{}", query, pattern))?;
