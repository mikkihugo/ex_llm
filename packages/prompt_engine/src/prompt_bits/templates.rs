...
impl TemplateExpander {
    ...
    pub fn find_matching_templates(
        ...
        self.templates
            .iter()
            .filter(|template| self.matches_context(template, task, context))
            .collect()
    }
    ...
    fn matches_context(
        ...
        let task_match = match (&template.trigger, task) {
            ...
            (PromptBitTrigger::CodePattern(p), TaskType::AddAuthentication) => {
                p.contains("Authentication")
            }
            (PromptBitTrigger::Infrastructure(infra), TaskType::AddDatabase) => context
                .databases
                .iter()
                .any(|db| format!("{:?}", db) == *infra),
            (PromptBitTrigger::Infrastructure(infra), TaskType::AddMessageBroker) => context
                .message_brokers
                .iter()
                .any(|mb| format!("{:?}", mb) == *infra),
            (PromptBitTrigger::Language(lang), _) => context
                .languages
                .iter()
                .any(|l| format!("{:?}", l) == *lang),
            (PromptBitTrigger::BuildSystem(build), _) => {
                format!("{:?}", context.build_system) == *build
            }
    ...
    fn replace_build_commands(&self, content: &str, build_system: &BuildSystem) -> String {
        match build_system {
            ...
            BuildSystem::Npm => content
                .replace("pnpm add", "npm install")
                .replace("pnpm install", "npm install")
                .replace("pnpm --filter", "npm run --workspace"),
            BuildSystem::Yarn => content
                .replace("pnpm add", "yarn add")
                .replace("pnpm install", "yarn install")
                .replace("pnpm --filter", "yarn workspace"),
