...
async fn test_assembler_with_framework_detection() {
    ...
    match PromptBitAssembler::new_with_framework_detection(analysis, project_path).await {
        Ok(assembler) => {
            println!(
                "✅ Assembler created successfully with {} frameworks detected",
                assembler.get_detected_frameworks().len()
            );

            // Check if we have any framework prompt bits
            let total_bits = assembler
                .get_all_framework_prompt_bits()
                .values()
                .map(|bits| bits.len())
                .sum::<usize>();
        ...
        Err(e) => {
            println!(
                "⚠️  Framework detection failed (expected if sparc-engine not available): {}",
                e
            );
