...
pub use neural_ml::{
    ConfidencePredictor, PromptEmbedder, PromptFeatures, TrainingConfig, TrainingMetrics,
};
...
pub async fn initialize_learning_system_default(
...
impl LearningSystem {
    ...
    pub async fn learning_cycle(&mut self, execution: ExecutionResult) -> Result<()> {
        ...
        let _fact_id = self
            .execution_tracker
            .track_execution(execution.clone())
            .await?;

        // 2. Adjust confidence scores
        let adjustment = self
            .confidence_scorer
            .calculate_adjustment(&execution)
            .await?;

        self.confidence_scorer
            .apply_adjustment(&execution.prompt_id, adjustment.clone())
            .await?;
    ...
    async fn trigger_reoptimization(&self, prompt_id: &str) -> Result<()> {
        ...
        if executions.len() < 5 {
            tracing::warn!(
                "Insufficient data for reoptimization: {} executions (minimum 5 required)",
                executions.len()
            );
            return Ok(());
        }
        ...
        tracing::info!(
            "Current performance: {:.1}% success, {:.2} confidence across {} executions",
            success_rate * 100.0,
            avg_confidence,
            executions.len()
        );
