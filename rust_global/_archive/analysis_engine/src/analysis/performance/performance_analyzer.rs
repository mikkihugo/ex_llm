...
impl SystemMonitor {
  ...
  pub fn with_config(config: MonitorConfig) -> Self {
    Self { stats_history: Arc::new(RwLock::new(VecDeque::new())), is_running: Arc::new(std::sync::atomic::AtomicBool::new(false)), config }
  }
  ...
  pub fn get_average_stats(&self, duration: Duration) -> Option<SystemStats> {
    ...
    let relevant_stats: Vec<&SystemStats> = history.iter().filter(|stats| stats.timestamp >= cutoff_time).collect();
  ...
  pub fn detect_anomalies(&self) -> Vec<PerformanceAnomaly> {
    let recent_stats = self.get_recent_stats(Duration::from_secs(5 * 60));
    ...
    let recent_disk_activity: u64 = recent_stats.iter().map(|s| s.disk_read_bytes + s.disk_write_bytes).sum();

    let disk_mb_per_sec = (recent_disk_activity as f64 / (1024.0 * 1024.0)) / recent_stats.len() as f64;

    if disk_mb_per_sec > 50.0 {
      // 50MB/s threshold
      anomalies.push(PerformanceAnomaly {
  ...
  fn get_recent_stats(&self, duration: Duration) -> Vec<SystemStats> {
    ...
    history.iter().filter(|stats| stats.timestamp >= cutoff_time).cloned().collect()
  }

  fn collect_system_stats() -> SystemStats {
    use memory_stats::memory_stats;

    let memory_info = memory_stats().unwrap_or(memory_stats::MemoryStats { physical_mem: 0, virtual_mem: 0 });
...
impl DashboardData {
  pub fn new(monitor: &SystemMonitor) -> Self {
    ...
    let recent_history = monitor.get_recent_stats(Duration::from_secs(5 * 60));
    ...
    Self { current_stats, recent_history, anomalies, performance_score, recommendations }
  }
...
impl AlertSubscriber for ConsoleAlertSubscriber {
  fn on_alert(&self, anomaly: &PerformanceAnomaly) {
    println!("[{}] {:?}: {}", chrono::Utc::now().format("%Y-%m-%d %H:%M:%S"), anomaly.severity, anomaly.description);
  }
}
