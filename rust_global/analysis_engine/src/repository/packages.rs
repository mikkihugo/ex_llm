...
impl PackageDiscovery {
  pub fn new(root_path: PathBuf, workspace_type: WorkspaceType, build_system: BuildSystem) -> Self {
    Self { root_path, workspace_type, build_system }
  }
  ...
  fn scan_directory_for_mix_apps(&self, dir: &Path) -> Result<Vec<Package>> {
    ...
    for entry in std::fs::read_dir(dir)? {
      ...
      if path.is_dir() {
        ...
        if mix_exs.exists() {
          packages.push(Package { id: self.generate_package_id(&path), name: self.extract_package_name(&path)?, path: path.clone(), manifest_path: mix_exs });
        }
      }
    }
  ...
  fn find_manifest(&self, path: &Path) -> Result<PathBuf> {
    let manifests = vec!["Cargo.toml", "package.json", "go.mod", "mix.exs", "rebar.config", "pyproject.toml", "setup.py"];
  ...
  fn generate_package_id(&self, path: &Path) -> PackageId {
    path.strip_prefix(&self.root_path).unwrap_or(path).to_string_lossy().replace('\\', "/").replace('/', "-").trim_start_matches('-').to_string()
  }
  ...
  fn extract_package_name(&self, path: &Path) -> Result<String> {
    ...
    if let Ok(manifest) = self.find_manifest(path) {
      if manifest.file_name().map(|n| n == "Cargo.toml").unwrap_or(false) {
      ...
      } else if manifest.file_name().map(|n| n == "package.json").unwrap_or(false) {
        if let Ok(contents) = std::fs::read_to_string(&manifest) {
...
mod tests {
  ...
  fn test_discover_single_package() {
    ...
    std::fs::write(temp.path().join("Cargo.toml"), "[package]\nname = \"test-pkg\"").unwrap();

    let discovery = PackageDiscovery::new(temp.path().to_path_buf(), WorkspaceType::SingleRepo, BuildSystem::Cargo);
