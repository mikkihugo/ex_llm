...
impl Default for PageRankConfig {
  fn default() -> Self {
    Self { damping_factor: 0.85, max_iterations: 100, convergence_threshold: 1e-6, enable_caching: true }
  }
}
...
impl CentralPageRank {
  ...
  pub fn new(config: PageRankConfig) -> Self {
    Self { graph: HashMap::new(), scores: HashMap::new(), config, cache: std::cell::RefCell::new(HashMap::new()) }
  }
  ...
  pub fn build_from_dependencies(&mut self, dependencies: &HashMap<String, Vec<String>>) {
    ...
    debug!("Graph built: {} nodes, {} total edges", self.graph.len(), self.graph.values().map(|v| v.len()).sum::<usize>());
  }
  ...
  pub fn calculate_pagerank(&mut self) -> Result<PageRankMetrics> {
    if self.graph.is_empty() {
      ...
      return Ok(PageRankMetrics { total_nodes: 0, total_edges: 0, average_degree: 0.0, density: 0.0, iterations_to_converge: 0, converged: false });
    }
    ...
    info!("Starting PageRank calculation: {} nodes, {} edges", node_count, total_edges);
  ...
  pub fn get_top_nodes(&self, n: usize) -> Vec<PageRankResult> {
    let mut results: Vec<_> = self.scores.iter().map(|(node_id, score)| (node_id.clone(), *score)).collect();
    ...
    results
      ...
      .map(|(rank, (node_id, score))| PageRankResult {
        node_id,
        score,
        normalized_score: if max_score > 0.0 { score / max_score } else { 0.0 },
        rank: rank + 1,
      })
      .collect()
  }
  ...
  pub fn get_stats(&self) -> HashMap<String, f64> {
    ...
    stats.insert("edges".to_string(), self.graph.values().map(|v| v.len()).sum::<usize>() as f64);

    if !self.scores.is_empty() {
      let scores: Vec<f64> = self.scores.values().cloned().collect();
      stats.insert("max_score".to_string(), scores.iter().fold(0.0, |a, &b| a.max(b)));
      stats.insert("min_score".to_string(), scores.iter().fold(1.0, |a, &b| a.min(b)));
      stats.insert("avg_score".to_string(), scores.iter().sum::<f64>() / scores.len() as f64);
    }
...
pub trait PageRankIntegration {
  ...
  async fn get_file_importance_rank(&self, file_path: &Path, pagerank: &CentralPageRank) -> Result<Option<usize>> {
    ...
    Ok(results.iter().find(|result| result.node_id == path_str).map(|result| result.rank))
  }
}
