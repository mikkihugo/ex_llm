...
**All inter-service communication goes through NATS**:
- No direct database access (except `db_service`)
- Pub/Sub for events, Request/Reply for RPC
- Subject hierarchy: `singularity.{domain}.{operation}`

```elixir
# Elixir: Publish to NATS
Singularity.PlatformIntegration.NatsConnector.publish(
  "db.insert.codebase_snapshots",
  Jason.encode!(snapshot)
)

# Elixir: Request/Reply
{:ok, response} = Singularity.PlatformIntegration.NatsConnector.request(
  "llm.analyze",
...
```rust
// Rust: Publish to NATS
let nats = async_nats::connect(&nats_url).await?;
nats.publish("db.insert.codebase_snapshots", payload.into()).await?;

// Rust: Request/Reply
let response = nats.request("llm.analyze", request.into()).await?;
let result: AnalysisResult = serde_json::from_slice(&response.payload)?;
```

### Database Access Pattern

**Only `db_service` touches PostgreSQL**:
- All other services use NATS subjects (`db.query`, `db.execute`, `db.insert.*`)
- Enables horizontal scaling
- Simplified testing (mock NATS instead of database)
