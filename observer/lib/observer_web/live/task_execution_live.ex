defmodule ObserverWeb.TaskExecutionLive do
  ...
  def mount(_params, _session, socket) do
    Logger.info("TaskExecutionLive: Mounting")
  ...
  defp fetch_metrics(socket) do
    try do
      {:ok, dashboard} = Singularity.Execution.TaskExecutionMetricsDashboard.get_dashboard()
      ...
      error ->
        Logger.error("TaskExecutionLive: Error fetching metrics",
          error: inspect(error)
        )

        socket
        |> assign(:loading, false)
        |> assign(:error, "Failed to load task execution metrics: #{inspect(error)}")
